...
neck_aaroga_rajju = [413, 22]
neck_avaroga_rajju = [6, 15, 24]
stomach_aaroga_rajju = [3, 12, 21]
stomach_avaroga_rajju = [7, 16, 25]
waist_aaroga_rajju = [2, 11, 20]
waist_avaroga_rajju = [8, 17, 26]
foot_aaroga_rajju = [1, 10, 19]
foot_avaroga_rajju = [9, 18, 27]
...
gana_array = [
    ...
    [5, 6, 0],
    [1, 0, 6],
]  ## Based on saravali.de (Maitri) Transposed in V3.1.1
...
raasi_adhipathi_array_south = [
    ...
    [0, 0, 0, 1, 1, 1, 0],
]  # Saturn => Mercury, Juipter and Venus
raasi_adhipathi_results = {
    0.0: "Very Bad",
    0.5: "Bad",
    1.0: "Below Average",
    3.0: "Above Average",
...
class Ashtakoota:
    ...
    def __init__(
        ...
        boy_paadham_number: int,
        girl_nakshatra_number: int,
        girl_paadham_number: int,
        method: str = "North",
    ):
        ...
        self.boy_raasi_number = self._raasi_from_nakshatra_pada(
            boy_nakshatra_number, boy_paadham_number
        )
        self.girl_raasi_number = self._raasi_from_nakshatra_pada(
            girl_nakshatra_number, girl_paadham_number
        )
        self.count_from_girl = count_stars(
            self.girl_nakshatra_number, self.boy_nakshatra_number
        )
        self.count_from_boy = count_stars(
            self.boy_nakshatra_number, self.girl_nakshatra_number
        )
        self.method = method
    ...
    def varna_porutham(self):
        ...
        bv = vasiya_raasi_list
        gv = bv
        bvk = bv[self.boy_raasi_number - 1]
        gvk = gv[self.girl_raasi_number - 1]
    ...
    def _vasiya_porutham_new(self, use_astroyogi_method=False):  # vasiya porutham
        chatushpada = (
            lambda n, r, p: r in [1, 2]
            or (r == 9)
            and ((n == 20 and p in [3, 4]) or (n == 21 and p in [1, 2]))
            or (r == 10)
            and ((n == 21 and p in [3, 4]) or (n == 22 and p in [1, 2]))
        )
        manava = (
            lambda n, r, p: r in [3, 6, 7, 11]
            or (r == 9)
            and ((n == 19) or (n == 20 and p in [1, 2]))
        )
        jalachara = (
            lambda n, r, p: r in [4, 12]
            or (r == 10)
            and ((n == 22 and p in [3, 4]) or (n == 23 and p in [1, 2]))
        )
        vanachara = lambda n, r, p: r == 5
        keeta = lambda n, r, p: r == 8
        if chatushpada(
            self.boy_raasi_number, self.boy_nakshatra_number, self.boy_paadham_number
        ):
            Bvkpoint = 0
        elif manava(
            self.boy_raasi_number, self.boy_nakshatra_number, self.boy_paadham_number
            ...
            Bvkpoint = 4
        if chatushpada(
            self.girl_raasi_number, self.girl_nakshatra_number, self.girl_paadham_number
        ):
            Gvkpoint = 0
        elif manava(
            self.girl_raasi_number, self.girl_nakshatra_number, self.girl_paadham_number
    ...
    def dina_porutham_south(self):
        ...
        count = (
            self.count_from_boy
        )  # ProKerala reads from Girl - whereas Tamil Panchangam says countr from by star
        if count in [2, 4, 6, 8, 9, 11, 13, 15, 17, 18, 20, 21, 24, 25, 26]:
            return True  # 17,21,25 are extra in Tamil Panchangam
        exception_dict = {"12": [2, 3, 4], "14": [1, 2, 3], "16": [1, 2, 4]}
        if any(
            [
                self.girl_nakshatra_number == int(k) and self.girl_paadham_number in vl
                for k, vl in exception_dict.items()
            ]
        ):
            return True
        if self.girl_nakshatra_number == self.boy_nakshatra_number:
            if self.girl_nakshatra_number in [1, 3, 5, 10, 13, 15, 20, 23] and (
                self.girl_raasi_number < self.boy_raasi_number
                or self.girl_paadham_number < self.boy_paadham_number
            ):
                return True
            elif (
                self.girl_raasi_number != self.boy_raasi_number
                and self.boy_raasi_number < self.girl_raasi_number
            ):  # Same star diff rasi
                return True
        if (
            self.girl_raasi_number == self.boy_raasi_number
            and self.boy_nakshatra_number < self.girl_nakshatra_number
        ):
            return True
        exception_22_list = [
            (4, 25),
        ...
        if (self.boy_nakshatra_number, self.girl_nakshatra_number) in exception_22_list:
            return True
        return False
    ...
    def gana_porutham_south(self):
        if (
            (
                self.boy_nakshatra_number in gana_south_deva
                and self.girl_nakshatra_number in gana_south_deva
            )
            or (
                (
                    self.boy_nakshatra_number in gana_south_manushya
                    and self.girl_nakshatra_number in gana_south_manushya
                )
                or self.boy_nakshatra_number in gana_south_deva
                and self.girl_nakshatra_number in gana_south_manushya
            )
            or (
                self.boy_nakshatra_number in gana_south_manushya
            ...
            or (
                self.boy_nakshatra_number in gana_south_rakshasa
                and self.girl_nakshatra_number in gana_south_rakshasa
                and self.girl_nakshatra_number > const.gana_threshold_south
            )
        ):
            return True
        return False
    ...
    def yoni_porutham_south(self):
        ga = yoni_mappings[self.girl_nakshatra_number - 1]
        ba = yoni_mappings[self.boy_nakshatra_number - 1]
    ...
    def yoni_porutham(self):  # Yoni Porutham
        ...
        return (
            YoniArray[yoni_mappings[self.girl_nakshatra_number - 1]][
                yoni_mappings[self.boy_nakshatra_number - 1]
            ],
            yoni_max_score,
        )
    ...
    def raasi_adhipathi_porutham_south(self):  # Raasi adhipathi porutham
        ...
        return (
            raasi_adhipathi_array_south[
                raasi_adhipathi_mappings[self.girl_raasi_number - 1]
            ][raasi_adhipathi_mappings[self.boy_raasi_number - 1]]
            == 1
        )
    ...
    def raasi_adhipathi_porutham(self):  # Raasi adhipathi porutham
        ...
        return (
            raasi_adhipathi_array[raasi_adhipathi_mappings[self.girl_raasi_number - 1]][
                raasi_adhipathi_mappings[self.boy_raasi_number - 1]
            ],
            raasi_adhipathi_max_score,
        )
    ...
    def raasi_porutham_south(self):
        return (
            count_rasis(self.girl_raasi_number, self.boy_raasi_number)
            > const.raasi_threshold_south
        )
    ...
    def raasi_porutham(self):  # Raasi Porutham
        ...
        return (
            raasi_array[self.girl_raasi_number - 1][self.boy_raasi_number - 1],
            raasi_max_score,
        )
    ...
    def naadi_porutham(self):
        ...
        gvk = bvk
        bv = bvk[self.boy_nakshatra_number - 1]
        gv = gvk[self.girl_nakshatra_number - 1]
        if "south" in self.method.lower():
    ...
    def mahendra_porutham(self):
        ...
        rem = (
            self.count_from_girl
        )  # ((self.boy_nakshatra_number + 27 - self.girl_nakshatra_number) % 27)+1
    ...
    def rajju_porutham(self):
        ...
        rp = (
            (bn in head_rajju)
            and (gn in head_rajju)
            or (bn in neck_rajju)
            and (gn in neck_rajju)
            or (bn in stomach_rajju)
            and (gn in stomach_rajju)
            or (bn in waist_rajju)
            and (gn in waist_rajju)
            or (bn in foot_rajju)
            and (gn in foot_rajju)
        )
        return not rp
    ...
    def rajju_porutham_south(self):
        ...
        bn_aaroga = (
            bn
            in neck_aaroga_rajju
            + foot_aaroga_rajju
            + waist_aaroga_rajju
            + stomach_aaroga_rajju
        )
        gn_aaroga = (
            gn
            in neck_aaroga_rajju
        ...
        if (bn_aaroga and not gn_aaroga) or (gn_aaroga and not bn_aaroga):
            return True
        rp = (
            (bn in head_rajju)
            and (gn in head_rajju)
    ...
    def compatibility_score(self):
        ...
        dina_porutham = (
            self.dina_porutham()
        )  # nakshathra_porutham is same as tara porutham
        ...
        def _is_there_minimum_tamil_porutham(
            skip_varna_checking=const.skip_using_girls_varna_for_minimum_tamil_porutham,
        ):  # V4.5.5
    ...
    def _raasi_from_nakshatra_pada(self, nakshatra_number, paadha_number):
        nakshatra_duration = 360 / 27.0
        raasi_duration = 360 / 12.0
        #        print('nakshatra_duration',nakshatra_duration)
        paadha_duration = nakshatra_duration / 4.0
        #        print('paadha_duration',paadha_duration)
        total_duration = (
            ((nakshatra_number - 1) * nakshatra_duration)
            + ((paadha_number - 1) * paadha_duration)
            + 0.5 * paadha_duration
        )
...
class Match:
    def __init__(
        ...
        minimum_score: float = const.compatibility_minimum_score_north,
    ...
    def get_matching_partners(self):
        boy_nak_given = (
            self.boy_nakshatra_number != None
            and self.boy_nakshatra_number >= 1
            and self.boy_nakshatra_number <= 27
        )
        boy_pad_given = (
            self.boy_paadham_number != None
            and self.boy_paadham_number >= 1
            and self.boy_paadham_number <= 4
        )
        # boy_info_given = boy_nak_given or boy_pad_given
        girl_nak_given = (
            self.girl_nakshatra_number != None
            and self.girl_nakshatra_number >= 1
            and self.girl_nakshatra_number <= 27
        )
        girl_pad_given = (
            self.girl_paadham_number != None
            and self.girl_paadham_number >= 1
            and self.girl_paadham_number <= 4
        )
        ...
        if boy_nak_given:
            ...
            search_criteria = search_criteria & (
                self.match_db[_BOY_STAR_COL] == self.boy_nakshatra_number
            )
            if boy_pad_given:
                search_criteria = search_criteria & (
                    self.match_db[_BOY_PAD_COL] == self.boy_paadham_number
                )
        if girl_nak_given:
            self._gender = "Female"
            search_criteria = search_criteria & (
                self.match_db[_GIRL_STAR_COL] == self.girl_nakshatra_number
            )
            if girl_pad_given:
                search_criteria = search_criteria & (
                    self.match_db[_GIRL_PAD_COL] == self.girl_paadham_number
                )
        # print('search_results',self.match_db.index[search_criteria].tolist())
        if self.check_for_mahendra_porutham == True:
            search_criteria = search_criteria & (
                self.match_db[_MAHEN_COL] == self.check_for_mahendra_porutham
            )
            # print('mahen search_results',self.match_db.index[search_criteria].tolist())
        if self.check_for_vedha_porutham == True:
            search_criteria = search_criteria & (
                self.match_db[_VEDHA_COL] == self.check_for_vedha_porutham
            )
            # print('vedha search_results',self.match_db.index[search_criteria].tolist())
        if self.check_for_rajju_porutham == True:
            search_criteria = search_criteria & (
                self.match_db[_RAJJU_COL] == self.check_for_rajju_porutham
            )
            # print('rajju search_results',self.match_db.index[search_criteria].tolist())
        if self.check_for_shreedheerga_porutham == True:
            search_criteria = search_criteria & (
                self.match_db[_SHREE_COL] == self.check_for_shreedheerga_porutham
            )
...
if __name__ == "__main__":
    ...
    for p, porutham in enumerate(ettu_poruthham_list):
        print(
            ...
            % (
                porutham,
                str(ettu_porutham_results[p][0]),
                str(ettu_porutham_results[p][1]),
            )
        )
    ...
    print(
        ...
        % (
            ...
            str(compatibility_score) + " out of " + str(max_compatibility_score),
            "",
        )
    )
    exit()
