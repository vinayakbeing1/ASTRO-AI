...
def trinal_aspects_of_the_raasi(house_planet_dict, raasi):
    ...
    aspected_planets = [
        house_planet_dict[th].split("/")
        for th in aspected_houses
        if house_planet_dict[th] != ""
    ]
...
def conjunction_aspects_of_the_raasi(house_planet_dict, raasi):
    ...
    aspected_planets = [
        house_planet_dict[th].replace(str(planet), "").split("/")
        for th in aspected_houses
        if house_planet_dict[th] != ""
    ]
    aspected_planets = sum(aspected_planets, [])
...
def sixty_hour_chart(
    jd_at_dob, place, divisional_chart_factor=1, years=1, months=1, sixty_hour_count=1
):
    """
...
def _get_lord_candidates(
    planet_positions, years_from_dob, natal_lagna_house, night_time_birth
):
    tajaka_chart_p_to_h = utils.get_planet_house_dictionary_from_planet_positions(
        planet_positions
    )
    tajaka_chart_h_to_p = utils.get_house_to_planet_dict_from_planet_to_house_dict(
        tajaka_chart_p_to_h
    )
    ...
    if night_time_birth:
        candidates.append(
            house.house_owner_from_planet_positions(
                planet_positions, tajaka_chart_p_to_h[1]
            )
        )
    else:
        candidates.append(
            house.house_owner_from_planet_positions(
                planet_positions, tajaka_chart_p_to_h[0]
            )
        )
    candidates.append(
        house.house_owner_from_planet_positions(planet_positions, natal_lagna_house)
    )
    asc_house = tajaka_chart_p_to_h[const._ascendant_symbol]
    m_house = muntha_house(asc_house, years_from_dob)
    candidates.append(
        house.house_owner_from_planet_positions(planet_positions, m_house)
    )
    candidates.append(
    ...
    candidates = list(
        sorted(set(candidates), key=candidates.index)
    )  # Remove duplicates and keep order same
    return candidates
...
def _get_the_lord_of_tajaka_chart(jd, place, candidates):
    ...
    candidates_shortlisted = [
        ...
        if planet_has_benefic_aspect_on_house(tajaka_chart_h_to_p, candidate, asc_house)
    ]
    ...
    if len(candidates_shortlisted) == 0:
        candidates_shortlisted = [
            candidate
            for candidate in candidates
            if planet_has_malefic_aspect_on_house(
                tajaka_chart_h_to_p, candidate, asc_house
            )
        ]
...
def lord_of_the_year(jd_at_dob, place, years_from_dob):  # ,night_time_birth=False):
    ...
    candidates = _get_lord_candidates(
        rasi_chart, years_from_dob, natal_lagna_house, night_time_birth
    )
...
def both_planets_within_their_deeptamsa(planet_positions, planet1, planet2):
    ...
    ithasala_type = None
    planet1_deeptamsa_start, planet1_deeptamsa_end = utils.deeptaamsa_range_of_planet(
        planet1, planet1_long_within_raasi
    )
    planet2_deeptamsa_start, planet2_deeptamsa_end = utils.deeptaamsa_range_of_planet(
        planet2, planet2_long_within_raasi
    )
    chk1 = (
        planet1_long_within_raasi >= planet2_deeptamsa_start
        and planet1_long_within_raasi <= planet2_deeptamsa_end
    )
    chk1_1 = False
    if not chk1:  # Check for bhavishya ithasala
        chk1_1 = (
            abs(planet1_long_within_raasi - planet2_deeptamsa_start) <= 1.0
            or abs(planet1_long_within_raasi - planet2_deeptamsa_end) <= 1.0
        )
    ...
    chk2 = (
        planet2_long_within_raasi >= planet1_deeptamsa_start
        and planet2_long_within_raasi <= planet1_deeptamsa_end
    )
    chk2_1 = False
    if not chk2:  # Check for bhavishya ithasala
        chk2_1 = (
            abs(planet2_long_within_raasi - planet1_deeptamsa_start) <= 1.0
            or abs(planet2_long_within_raasi - planet1_deeptamsa_end) <= 1.0
        )
...
def both_planets_approaching(planet_positions, planet1, planet2):
    ...
    if const.order_of_planets_by_speed.index(
        planet1
    ) < const.order_of_planets_by_speed.index(planet2):
