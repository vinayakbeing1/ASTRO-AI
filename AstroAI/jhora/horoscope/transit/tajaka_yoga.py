...
def ishkavala_yoga(planet_to_house_dict):
    ...
    sph = list(
        set(
            [
                house.get_relative_house_of_planet(asc_house, h) - 1
                for h in list(planet_to_house_dict.values())[:-1]
            ]
        )
    )
...
def ithasala_yoga(planet_positions, planet1, planet2):
    ...
    """
    house_planet_dict = utils.get_house_planet_list_from_planet_positions(
        planet_positions
    )
    # print(house_planet_dict)
    chk1 = tajaka.planets_have_aspects(house_planet_dict, planet1, planet2)
    chk2, ithasala_type = tajaka.both_planets_within_their_deeptamsa(
...
def _check_nakta_yoga_1(planet_positions, planet1, planet2):
    ...
    yp = yp and int(planet1) not in tajaka.aspects_of_the_planet(
        house_planet_dict, int(planet2)
    )
    return yp
...
def _get_nakta_triples(ithasala_pairs):
    ...
    iy_list = [
        (k, list(itertools.combinations(lst, 2)))
        for k, lst in iy_list.items()
        if len(lst) > 1
    ]
...
def get_nakta_yoga_planet_triples(planet_positions):
    ...
    for planet, p_list in _nakta_triples:
        ...
        for p1, p2 in p_list:
            ...
            if (
                not tajaka.planets_have_aspects(house_planet_dict, p1, p2)
                and p_long < p1_long
                and p_long < p2_long
            ):
...
def get_manahoo_yoga_planet_pairs(planet_positions):
    ...
    iy_pairs = get_ithasala_yoga_planet_pairs(planet_positions)
    my = []
    mars_or_saturn_houses = [
        planet_positions[3 + 1][1][0],
        planet_positions[6 + 1][1][0],
    ]
    for p1, p2, _ in iy_pairs:  # ithasala tuple is p1,p2,othasal_type
        ...
        (
            faster_planet_deeptaamsa_start,
            faster_planet_deeptaamsa_end,
        ) = utils.deeptaamsa_range_of_planet(faster_planet, faster_planet_long)
        if faster_planet_house in mars_or_saturn_houses:
            ...
            if (
                m_s_long > faster_planet_deeptaamsa_start
                and m_s_long < faster_planet_deeptaamsa_end
            ):
...
def get_kamboola_yoga_planet_pairs(planet_positions):
    ...
    iy_ky_pairs = [
        (x, y) for kyp in ky_planets for x, y in iy_pairs if kyp == x or kyp == y
    ]
...
def get_radda_yoga_planet_pairs(planet_positions):
    ...
    chk.append(
        [
            x in charts.planets_in_combustion(planet_positions)
            or y in charts.planets_in_combustion(planet_positions)
            for x, y in iy_pairs
        ]
    )
    chk.append(
        [
            x in charts.planets_in_retrograde(planet_positions)
            or y in charts.planets_in_retrograde(planet_positions)
            for x, y in iy_pairs
        ]
    )
    chk.append(
        [
            const.house_strengths_of_planets[x][p_to_h[x]] < const._NEUTRAL_SAMAM
            or const.house_strengths_of_planets[y][p_to_h[y]] < 2
            for x, y in iy_pairs
        ]
    )
    import numpy as np
...
def get_duhphali_kutta_yoga_planet_pairs(jd, place):
    ...
    for x, y in iy_pairs:
        ...
        if const.order_of_planets_by_speed.index(
            x
        ) < const.order_of_planets_by_speed.index(y):
            faster_planet = y
            slower_planet = x
        print("ithasala pair", faster_planet, slower_planet)
        chk = (
            const.house_strengths_of_planets[faster_planet][p_to_h[faster_planet]]
            > const._FRIEND
        )
        print(
            "chk1",
        ...
        if not chk:
            continue
        chk = chk & (
            const.house_strengths_of_planets[slower_planet][p_to_h[slower_planet]]
...
if __name__ == "__main__":
    ...
    chart_67_rasi = charts.divisional_chart(
        jd_at_years, place, divisional_chart_factor=1
    )
    ...
    exit()
    chart_67_rasi = charts.divisional_chart(
        jd_at_years, place, ayanamsa_mode, divisional_chart_factor=1
    )
    print(chart_67_rasi)
