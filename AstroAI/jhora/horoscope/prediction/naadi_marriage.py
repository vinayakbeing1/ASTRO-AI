...
def _check_yoga_1(planet_positions: list = None, gender: int = 0):
    ...
    planet_to_house_dict = utils.get_planet_to_house_dict_from_chart(
        house_to_planet_list
    )
    planet = 5  # Venus
    if gender == 1:
        planet = 2  # Mars
    aspected_planets = house.aspected_planets_of_the_planet(
        house_to_planet_list, planet
    )
    y1 = any([p1 in aspected_planets for p1 in [4, 6]])
    y1msg = "Obstacles in marriage, if married, disputes with spouse"
    if y1:
        y1msg = "Marriage will happen"
    print(
        "4/6 in aspected planets graha drishti of planet",
        planet,
        aspected_planets,
        y1,
        y1msg,
...
def _check_yoga_3(planet_positions: list = None, gender: int = 0):
    ...
    y3msg = ""
    y3 = planet in aspected_planets
    if y3:
        y3msg = "Obstacles in marriage, If married disputes with spouse."
...
def _check_yoga_4(planet_positions: list = None, gender: int = 0):
    ...
    y4msg = ""
    y4 = planet in aspected_planets
    if y4:
        y4msg = "Marriage delayed or if married, will not like the spouse."
...
if __name__ == "__main__":
    ...
    pp = charts.divisional_chart(
        utils.julian_day_number((1996, 12, 7), (10, 34, 0)),
        drik.Place("Chennai,India", 13.0878, 80.2785, 5.5),
