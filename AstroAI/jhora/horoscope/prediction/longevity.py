...
def _baladrishta_checks(jd, place, divisional_chart_factor=1):
    ...
    bc = any(
        [
            p_to_h[1] in q
            for q in [
                house.quadrants_of_the_raasi(p_to_h[m]) for m in const.natural_malefics
            ]
        ]
    )
    bd_checks.append(bc)
    ...
    bc = (
        (_relative_house(p_to_h[1]) in [7, 8, 12])
        and any([p_to_h[1] == p_to_h[m] for m in const.natural_malefics])
        and any(
            [
                1 in q
                for q in [
                    house.aspected_planets_of_the_planet(h_to_p, b)
                    for b in const.natural_benefics
                ]
            ]
        )
    )
    ...
    bc = all(
        [
            mh in [2, 6, 8, 12]
            for mh in [_relative_house(p_to_h[m]) for m in charts.malefics(jd, place)]
        ]
    )
    bd_checks.append(bc)
    """ Weak moon in ascendant or 8th and Malefics in quadrants """
    bc = (
        const.house_strengths_of_planets[1][p_to_h[1]] < const._FRIEND
        and ((p_to_h[1] == asc_house) or (_relative_house(p_to_h[1]) == 8))
        and any(
            [
                m in house.quadrants_of_the_raasi(p_to_h[1])
                for m in const.natural_malefics
            ]
        )
    )
    ...
    bc = (
        (asc_house == p_to_h[1])
        and (_relative_house(p_to_h[0]) == 9)
        and (_relative_house(p_to_h[2]) == 8)
        and (_relative_house(p_to_h[6]) == 12)
    )
    bd_checks.append(bc)
    """ Moon in 8th, Mars in 7th, Rahu in 9th, and Jupiter in 3rd """
    bc = (
        (_relative_house(p_to_h[1]) == 8)
        and (_relative_house(p_to_h[2]) == 7)
        and (_relative_house(p_to_h[7]) == 9)
        and (_relative_house(p_to_h[4]) == 3)
    )
    bd_checks.append(bc)
    return bd_checks
...
def _alpayu_checks(jd, place, divisional_chart_factor=1):
    ...
    asc_house = p_to_h["L"]
    _relative_house = lambda sign: house.get_relative_house_of_planet(asc_house, sign)
    _house_owner = lambda sign: house.house_owner_from_planet_positions(
        planet_positions, sign
    )
    """ Saturn in 8th, Mars in 5th and Kethu with Ascendant/1st """
    ad_checks = []
    ac = (
        (_relative_house(p_to_h[6]) == 8)
        and (_relative_house(p_to_h[2]) == 5)
        and (asc_house == p_to_h[8])
    )
    ad_checks.append(ac)
    ac = all(
        [
    ...
    ac = (
        (p_to_h[1] == p_to_h[2])
        and (p_to_h[0] == p_to_h[1])(_relative_house(p_to_h[2]) == 7)
        and (asc_house == p_to_h[6])
    )
    ad_checks.append(ac)
    ...
    ac = any(
        [
            p_to_h[1] == p_to_h[m] and asc_house == p_to_h[m]
            for m in const.natural_malefics
        ]
    )
    ad_checks.append(ac)
    ...
    ac = (p_to_h[eighth_lord] == asc_house) and (
    ...
    ac = (
        _relative_house(p_to_h[7]) == 7
        and _relative_house(p_to_h[1]) == 8
        and (asc_house == p_to_h[4])
    )
    ad_checks.append(ac)
    """ Saturn/Ascendant owned by malefic and benefics in [3,6,9,12] """
    ac = (
        asc_house == p_to_h[6]
        and any([m == lagna_lord for m in charts.malefics(jd, place)])
        and all(
            [
                _relative_house(p_to_h[b]) in [3, 6, 9, 12]
                for b in charts.benefics(jd, place)
            ]
        )
    )
    ad_checks.append(ac)
    ...
    ac = (
        p_to_h[lagna_lord] == p_to_h[eighth_lord]
        and p_to_h[lagna_lord] == (asc_house + 7) % 12
        and any([p_to_h[m] == p_to_h[lagna_lord] for m in charts.malefics(jd, place)])
        and any(
            [
                lagna_lord in q
                for q in [
...
def _madhyayu_checks(jd, place, divisional_chart_factor=1):
    ...
    md_checks = []
    mc = p_to_h[
        _house_owner((asc_house + 2) % 12)
    ] in house.kendra_aspects_of_the_raasi(asc_house) or p_to_h[
        _house_owner((asc_house + 5) % 12)
    ] in house.kendra_aspects_of_the_raasi(
        asc_house
    )
    md_checks.append(mc)
    ...
    mc = (
        sum(
            [
                const.house_strengths_of_planets[p][p_to_h[p]] > const._FRIEND
                for p in [eighth_lord, tenth_lord, eleventh_lord]
            ]
        )
        >= 2
    )
...
if __name__ == "__main__":
    ...
    aa = life_span_range(jd, place)
    print(const.aayu_types[aa])
    exit()
