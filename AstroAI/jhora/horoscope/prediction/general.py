...
def get_prediction_resources(language="en"):
    ...
    """
    json_file = (
        _lang_path + const._DEFAULT_PREDICTION_JSON_FILE_PREFIX + language + ".json"
    )
...
def _get_general_lagna_rasi_prediction(
    jd, place, prediction_msgs, language=const._DEFAULT_LANGUAGE
):
    ...
    for s in range(source_count):
        ...
        results[ks] = (
            "<html><b>" + utils.resource_strings["general_prediction_str"] + "</b><br>"
        )
        # results[ks] += "<b>"+prediction_msgs['general_prediction_caution']+"</b><br>"
        results[ks] += (
            "<b>" + prediction_msgs["janma_raasi_" + str(s + 1)]["source"] + "</b><br>"
        )
...
def _get_planets_in_houses_prediction(planet_positions, prediction_msgs):
    ...
    for planet in [*range(9)]:
        ...
        key = (
            ...
            + "-"
            + utils.resource_strings["house_str"]
            + "#"
            + str(planet_house)
            + ":"
        )
...
def get_prediction_details(jd_at_dob, place, language=const._DEFAULT_LANGUAGE):
    ...
    results1 = _get_general_lagna_rasi_prediction(
        jd_at_dob, place, prediction_msgs, language=language
    )
