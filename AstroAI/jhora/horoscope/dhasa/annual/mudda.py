...
year_duration = (
    const.tropical_year
)  # const.sidereal_year  # some say 360 days, others 365.25 or 365.2563 etc
varsha_vimsottari_adhipati = lambda nak: const.varsha_vimsottari_adhipati_list[
    nak % (len(const.varsha_vimsottari_adhipati_list))
]
...
def varsha_vimsottari_dasha_start_date(
    jd, place, years, divisional_chart_factor=1, chart_method=1
):
    ...
    moon = (
        planet_positions[2][1][0] * 30 + planet_positions[2][1][1]
    )  # +(star_position_from_moon-1)*one_star
    nak = int(moon / one_star)
    rem = moon - nak * one_star
    lord = vimsottari.vimsottari_adhipati(
        nak
    )  # vimsottari_dasha_start_date(jd,place)[0]
    lord = (lord + years) % 9
    ...
    start_date = (
        jd + years * year_duration - period_elapsed
    )  # so many days before current day
    return [lord, start_date]


def varsha_vimsottari_mahadasa(
    jdut1, place, years, divisional_chart_factor=1, chart_method=1
):
    """List all mahadashas and their start dates"""
    lord, start_date = varsha_vimsottari_dasha_start_date(
        jdut1,
        place,
        years,
        divisional_chart_factor=divisional_chart_factor,
...
def varsha_vimsottari_bhukti(maha_lord, start_date):
    ...
    for i in range(9):
        factor = (
            const.varsha_vimsottari_days[lord]
            * const.varsha_vimsottari_days[maha_lord]
            / const.human_life_span_for_varsha_vimsottari_dhasa
        )
...
def varsha_vimsottari_antara(maha_lord, bhukti_lord, start_date):
    ...
    retval = []
    for i in range(9):
        factor = const.varsha_vimsottari_days[lord] * (
            const.varsha_vimsottari_days[maha_lord]
            / const.human_life_span_for_varsha_vimsottari_dhasa
        )
        duration = factor * (
            const.varsha_vimsottari_days[bhukti_lord]
            / const.human_life_span_for_varsha_vimsottari_dhasa
        )
        retval.append((lord, start_date, round(duration, 2)))
        start_date += duration
...
def varsha_vimsottari_dhasa_bhukthi(
    jd, place, years, include_antardhasa=True, divisional_chart_factor=1, chart_method=1
):
    """
    Calculates Varsha Vimshottari (also called Mudda dhasa) Dasha-bhukthi-antara-sukshma-prana
    ...
    for lord, dhasa_start, durn in dashas:
        ...
        if include_antardhasa:
            ...
            for bhukthi_lord, bhukthi_start, bhukthi_durn in bhuktis:
                ...
                dhasa_bukthi.append(
                    (dhasa_lord, bhukthi_lord, date_str, round(bhukthi_durn, 2))
                )
        else:
...
def mudda_dhasa_bhukthi(
    jd, place, years, include_antardhasa=True, divisional_chart_factor=1
):
    return varsha_vimsottari_dhasa_bhukthi(
        jd,
        place,
        years,
        include_antardhasa,
