...
def sudharshana_chakra_chart(
    jd_at_dob, place, dob, years_from_dob=0, divisional_chart_factor=1
):
    ...
    lagna_chart = [
        ((p + lagna_house) % 12, natal_chart[(p + lagna_house) % 12]) for p in range(12)
    ]
    # print('lagna_chart',lagna_chart)
    moon_chart = [
        ((p + moon_house) % 12, natal_chart[(p + moon_house) % 12]) for p in range(12)
    ]
    # print('moon_chart',moon_chart)
    sun_chart = [
        ((p + sun_house) % 12, natal_chart[(p + sun_house) % 12]) for p in range(12)
    ]
...
def _sudharsana_dhasa_calculation(jd_at_years, dhasa_seed_sign):
    ...
    for sign in dhasa_progression:
        ...
        antardhasa = [
            [
                (sign + h) % 12,
                drik.jd_to_gregorian(dhasa_start + h * antardhasa_duration),
                1.0,
            ]
            for h in range(12)
        ]
...
if __name__ == "__main__":
    ...
    jd_at_years = jd_at_dob + years_from_dob * const.sidereal_year
    lsd, msd, ssd, = sudharshana_chakra_chart(
        jd_at_dob, place, dob, years_from_dob, divisional_chart_factor
    )
    print(lsd, "\n", msd, "\n", ssd)
    exit()
    lsd, msd, ssd = sudharsana_chakra_dhasa_for_divisional_chart(
        jd_at_dob, place, dob, years_from_dob, divisional_chart_factor
    )
