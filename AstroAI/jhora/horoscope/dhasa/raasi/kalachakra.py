...
def _get_dhasa_progression(planet_longitude):
    ...
    dhasa_index_at_birth = next(
        x[0]
        for x in enumerate(dhasa_duration_cumulative)
        if x[1] > paramayush_completed
    )
    dhasa_remaining_at_birth = (
        dhasa_duration_cumulative[dhasa_index_at_birth] - paramayush_completed
    )
    ...
    dhasa_progression = (
        dhasa_progression[dhasa_index_at_birth:]
        + const.kalachakra_rasis[kalachakra_index_next][paadham_next][
            :dhasa_index_at_birth
        ]
    )
    dhasa_duration = [const.kalachakra_dhasa_duration[r] for r in dhasa_progression]
    ...
    for i, dp in enumerate(dhasa_progression):
        ad = antardhasa(
            dhasa_index_at_birth, i, dhasa_paramayush, kalachakra_index_next, paadham
        )
        """
        ...
        if len(ad) == 0:
            ad = [
                dhasa_progression,
                [const.kalachakra_dhasa_duration[r] for r in dhasa_progression],
            ]
...
def antardhasa(dhasa_index_at_birth, dp_index, paramayush, kc_index, paadham):
    ...
    antardhasa_duration = [
        const.kalachakra_dhasa_duration[r] for r in antardhasa_progression
    ]
