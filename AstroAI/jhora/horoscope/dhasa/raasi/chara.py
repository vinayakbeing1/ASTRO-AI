...
def _dhasa_duration_pvnrao_method(planet_positions, sign):
    ...
    if sign == 7:  # sign is scorpio Mars/Ketu are co-lords
        if p_to_h[2] == sign and p_to_h[8] == sign:  # Both in Scorpio
            ...
            lord_of_sign = house.house_owner_from_planet_positions(
...
def _dhasa_progression_pvnrao_method(planet_positions):
    ...
    sun_house_lord = house.house_owner_from_planet_positions(
        planet_positions, sun_house
    )
    asc_house = planet_positions[0][1][0]
    asc_house_lord = house.house_owner_from_planet_positions(
        planet_positions, asc_house
    )
    sh = house.stronger_planet_from_planet_positions(
        planet_positions, sun_house_lord, asc_house_lord
    )
    seed_house = asc_house if sh == asc_house_lord else sun_house
    moon_house = planet_positions[2][1][0]
    moon_house_lord = house.house_owner_from_planet_positions(
        planet_positions, moon_house
    )
    stronger_lord = house.stronger_planet_from_planet_positions(
        planet_positions, sh, moon_house_lord
    )
...
def _dhasa_progression_raghava_bhatta_method(planet_positions, gender=1):
    ...
    _dhasa_progression = (
        [(dhasa_seed + h) % 12 for h in range(12)]
        if asc_house in const.odd_signs
        else [(12 + dhasa_seed - h) % 12 for h in range(12)]
    )
...
def get_dhasa_antardhasa(
    ...
    for dc in range(_dhasa_cycles):
        for lord in dhasa_progression:
            dd = (
                _dhasa_duration_knrao_method(planet_positions, lord)
                if chara_method == 1
                else _dhasa_duration_pvnrao_method(planet_positions, lord)
            )
