...
def shoola_dhasa_bhukthi(
    dob, tob, place, divisional_chart_factor=1, include_antardhasa=True
):
    ...
    return shoola_dhasa(
        planet_positions, dob, tob, include_antardhasa=include_antardhasa
    )
...
def _shoola_dhasa(chart, dob):
    ...
    for sign in dhasa_progression:
        ...
        andtardhasa = _antardhasa(
            sign, p_to_h
        )  # )+' '+str(dhasa_duration)+' months each'
        dhasa_period_suffix = "-" + str(dob_month) + "-" + str(dob_day)
        dhasa_periods.append(
            [
                sign,
                str(dhasa_start) + dhasa_period_suffix,
                str(dhasa_end) + dhasa_period_suffix,
                andtardhasa,
                dhasa_duration,
            ]
        )
