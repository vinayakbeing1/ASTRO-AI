...
def get_dhasa_antardhasa(
    ...
    _dhasa_progression = [
        ((_dhasa_seed + h) % 12, _durn) for h, _durn in enumerate(_sandhya_duration)
    ]
    start_jd = jd_at_dob
    dhasa_info = []
    for dhasa_lord, dhasa_duration in _dhasa_progression:
        if include_antardhasa:
            ...
            for bhukthi_lord in [(dhasa_lord + h) % 12 for h in range(12)]:
                ...
                dhasa_info.append(
                    (dhasa_lord, bhukthi_lord, dhasa_start, round(bhukthi_duration, 2))
                )
                start_jd += bhukthi_duration * const.sidereal_year
        elif use_panchaka_variation:
            for b, bhukthi_lord in enumerate(
                [(dhasa_lord + h) % 12 for h in range(12)]
            ):
