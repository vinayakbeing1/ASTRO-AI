...
def _dhasa_duration(planet_positions, sign, varsha_narayana=False):
    ...
    dhasa_period = (
        utils.count_rasis(house_of_lord, sign)
    ...
    if dhasa_period <= 0:
        ...
        dhasa_period = 12
    if (
        const.house_strengths_of_planets[lord_of_sign][house_of_lord]
...
def _narayana_dhasa_calculation(
    ...
    if p_to_h[8] == dhasa_seed_sign:
        dhasa_progression = const.narayana_dhasa_ketu_exception_progression[
            dhasa_seed_sign
        ]
    elif p_to_h[6] == dhasa_seed_sign:
        dhasa_progression = const.narayana_dhasa_saturn_exception_progression[
            dhasa_seed_sign
        ]
    dhasa_periods = []
    ...
    for dhasa_lord in dhasa_progression:
        ...
        bhukthis = _narayana_antardhasa(
            planet_positions, dhasa_lord
        )  # _narayana_antardhasa(dhasa_lord,p_to_h)
        if include_antardhasa:
            ...
            for bhukthi_lord in bhukthis:
                ...
                dhasa_periods.append(
                    (dhasa_lord, bhukthi_lord, dhasa_start, dhasa_duration)
                )
...
def narayana_dhasa_for_divisional_chart(
    ...
    if divisional_chart_factor == 1:
        return narayana_dhasa_for_rasi_chart(
            dob, tob, place, years, months, sixty_hours, include_antardhasa
        )
    # Get Rasi Chart first
    jd_at_dob = utils.julian_day_number(dob, tob)
    planet_positions_rasi = charts.divisional_chart(jd_at_dob, place)
    h_to_p_rasi = utils.get_house_planet_list_from_planet_positions(
    ...
    seed_house = (
        p_to_h_rasi[const._ascendant_symbol] + divisional_chart_factor - 1
    ) % 12
    lord_of_seed_house = house.house_owner_from_planet_positions(
        planet_positions_rasi, seed_house, check_during_dhasa=True
    )
    """ 
        Important:
    ...
    p_to_h_varga = utils.get_planet_house_dictionary_from_planet_positions(
        varga_planet_positions
    )
    lord_sign = p_to_h_varga[lord_of_seed_house]
    h_to_p_varga = utils.get_house_planet_list_from_planet_positions(
        varga_planet_positions
    )
    h_to_p_varga = utils.get_house_planet_list_from_planet_positions(
        varga_planet_positions
    ...
    dhasa_seed_sign = house.stronger_rasi_from_planet_positions(
        varga_planet_positions, lord_sign, seventh_house
    )
...
def narayana_dhasa_for_rasi_chart(
    dob, tob, place, years=1, months=1, sixty_hours=1, include_antardhasa=True
):
    jd_at_dob = utils.julian_day_number(dob, tob)
    planet_positions = charts.rasi_chart(
        jd_at_dob, place, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    )
...
def _narayana_antardhasa(planet_positions, dhasa_rasi):
    _DEBUG_ = False
    if _DEBUG_:
        print("dhasa_rasi", utils.RAASI_LIST[dhasa_rasi])
    lord_of_dhasa_rasi = house.house_owner_from_planet_positions(
        planet_positions, dhasa_rasi, check_during_dhasa=True
    )
    if _DEBUG_:
        print("lord_of_dhasa_rasi", utils.PLANET_NAMES[lord_of_dhasa_rasi])
    house_of_dhasa_rasi_lord = planet_positions[lord_of_dhasa_rasi + 1][1][0]
    if _DEBUG_:
        print("house_of_dhasa_rasi_lord", utils.RAASI_LIST[house_of_dhasa_rasi_lord])
    lord_of_7thhouse_of_dhasa_rasi = house.house_owner_from_planet_positions(
        planet_positions, (dhasa_rasi + 7) % 12, check_during_dhasa=True
    )
    if _DEBUG_:
        print(
            "lord_of_7thhouse_of_dhasa_rasi",
            utils.PLANET_NAMES[lord_of_7thhouse_of_dhasa_rasi],
        )
    house_of_dhasa_rasi_lord_7thHouse = planet_positions[
        lord_of_7thhouse_of_dhasa_rasi + 1
    ][1][0]
    if _DEBUG_:
        print(
            "house_of_dhasa_rasi_lord_7thHouse",
            utils.RAASI_LIST[house_of_dhasa_rasi_lord_7thHouse],
        )
    antardhasa_seed_rasi = house.stronger_rasi_from_planet_positions(
        planet_positions, house_of_dhasa_rasi_lord, house_of_dhasa_rasi_lord_7thHouse
    )
    if _DEBUG_:
        print("stronger antardhasa_seed_rasi", utils.RAASI_LIST[antardhasa_seed_rasi])
    p_to_h = utils.get_planet_house_dictionary_from_planet_positions(planet_positions)
...
def _narayana_antardhasa_old(antardhasa_seed_rasi, p_to_h):
    direction = -1
    if (
        p_to_h[6] == antardhasa_seed_rasi or antardhasa_seed_rasi in const.odd_signs
    ):  # Forward
        direction = 1
...
def varsha_narayana_dhasa_bhukthi(
    ...
    p_to_h_rasi = utils.get_planet_house_dictionary_from_planet_positions(
    ...
    h_to_p_varga = utils.get_house_to_planet_dict_from_planet_to_house_dict(
        p_to_h_varga
    )
