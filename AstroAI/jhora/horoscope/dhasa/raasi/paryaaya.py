...
def _dhasa_lords(planet_positions, dhasa_seed):
    if dhasa_seed in const.dual_signs:  # Dual and Chara Paryaaya
        ...
        dhasa_lords = [
            (sr + h - 1) % 12 for h in [1, 5, 9, 2, 6, 10, 3, 7, 11, 4, 8, 12]
        ]
        if sr in const.even_footed_signs:
            dhasa_lords = [
                (sr - h + 13) % 12 for h in [1, 5, 9, 2, 6, 10, 3, 7, 11, 4, 8, 12]
            ]
        ...
        dhasa_lords = [
            (sr + h - 1) % 12 for h in [1, 4, 7, 10, 2, 5, 8, 11, 3, 6, 9, 12]
        ]
        if sr in const.even_footed_signs:  # Fixed and Sthira Paryaaya
            dhasa_lords = [
                (sr - h + 13) % 12 for h in [1, 4, 7, 10, 2, 5, 8, 11, 3, 6, 9, 12]
            ]
    else:  # Fixed = Sthira Paryaaya
        sr = house.stronger_rasi_from_planet_positions(
            planet_positions, dhasa_seed, (dhasa_seed + 6) % 12
        )
        dhasa_lords = [
            (sr + h - 1) % 12 for h in [1, 7, 2, 8, 3, 9, 4, 10, 5, 11, 6, 12]
        ]
        if sr in const.even_footed_signs:
            dhasa_lords = [
                (sr - h + 13) % 12 for h in [1, 7, 2, 8, 3, 9, 4, 10, 5, 11, 6, 12]
            ]
    return dhasa_lords
...
def get_dhasa_antardhasa(
    ...
    for dc in range(_dhasa_cycles):
        for dhasa_lord in dhasa_lords:
            duration = round(
                _dhasa_duration(planet_positions, dhasa_lord) * _tribhagi_factor, 2
            )
