...
def sudasa_dhasa_from_planet_positions(
    ...
    for s, dhasa_lord in enumerate(dhasa_progression):
        dhasa_duration = round(
            narayana._dhasa_duration(planet_positions, dhasa_lord), 2
        )
        if s == 0:
            dhasa_duration *= sl_frac_left
    ...
    for c, dhasa_lord in enumerate(dhasa_progression):
        dhasa_duration = (
            (12 - narayana._dhasa_duration(planet_positions, dhasa_lord))
            if c == 0
            else (12 - dhasa_info[c][-1])
        )
