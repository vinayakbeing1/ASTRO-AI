...
dhasa_adhipathi_list = lambda lord: {
    (lord - i) % 27: _dhasa_duration for i in range(_dhasa_count)
}
    ...
    for _ in range(_dhasa_cycles):
        for dhasa_lord in dhasa_progression:
            ...
            if include_antardhasa:
                bhukthis = _antardhasa(
                    dhasa_lord, antardhasa_option
                )  # [(dhasa_lord-i)%27 for i in range(_dhasa_count)]
