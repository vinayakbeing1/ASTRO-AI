...
def _astangata_harana(planet_positions):
    ...
    if _DEBUG:
        print("planets_in_combustion", planets_in_combustion)
    ...
    """
    if _DEBUG:
        print("planets_in_retrograde", planets_in_retrograde)
    _harana_factors = {p: 1.0 for p in [const._ascendant_symbol] + [*range(7)]}
    if _DEBUG:
        print("ignore_planets", ignore_planets)
...
def _shatru_kshetra_harana(
    planet_positions, treat_mars_as_strong_in_enemy_sign=True, method=2
):
    """
    ...
    """
    if method == 1:
        treat_mars_as_strong_in_enemy_sign = False
    planets_in_enemy_sign = [
        p
        for p, (h, _) in planet_positions[:8]
        if p != const._ascendant_symbol
        and const.house_strengths_of_planets[p][h] == const._ENEMY
    ]
    if treat_mars_as_strong_in_enemy_sign and 2 in planets_in_enemy_sign:
        planets_in_enemy_sign.remove(2)
    if _DEBUG:
        print("planets_in_enemy_sign", planets_in_enemy_sign)
    ...
    _harana_factors = {p: 1.0 for p in [const._ascendant_symbol] + [*range(7)]}
    temp_dict = {
        p: 2 / 3 for p in planets_in_enemy_sign if p not in planets_in_retrograde
    }
    _harana_factors.update(temp_dict)
    return _harana_factors


def _shatru_kshetra_harana_santhanam(
    planet_positions, treat_mars_as_strong_in_enemy_sign=False
):
    """
    ...
    """
    return _shatru_kshetra_harana(
        planet_positions, treat_mars_as_strong_in_enemy_sign=False, method=1
    )
...
def _chakrapata_harana(planet_positions, method=2):
    ...
    if _DEBUG:
        print(
            "subha_grahas",
            subha_grahas,
            const.natural_benefics,
            "asubha_grahas",
            asubha_grahas,
            const.natural_malefics,
        )
    subha_dict = {
        p: subha_asubha_factors[(house.get_relative_house_of_planet(asc_house, h))][0]
        for p, (h, _) in planet_positions
        if p in subha_grahas and (house.get_relative_house_of_planet(asc_house, h)) > 6
    }
    asubha_dict = {
        p: subha_asubha_factors[((house.get_relative_house_of_planet(asc_house, h)))][1]
        for p, (h, _) in planet_positions
        if p in asubha_grahas and (house.get_relative_house_of_planet(asc_house, h)) > 6
    }
    _harana_factors.update(subha_dict)
    _harana_factors.update(asubha_dict)
    return _harana_factors


def _chakrapata_harana_santhanam(
    planet_positions, bhava_starts_with_ascendant=False
):  # jd,place,bhava_starts_with_ascendant=False):
    """
    ...
    bhava_length = 30.0
    if _DEBUG:
        print("bhava houses", bhava_houses, bhava_length)
    bhava_start = (asc_long - 15) % 30
    bhava_end = (asc_long + 15) % 30
    if bhava_starts_with_ascendant:
        bhava_start = asc_long
        bhava_end = (asc_long + 30) % 30
    if _DEBUG:
        print("bhava start", bhava_start, "bhava end", bhava_end)
    vh = {}
    for p, (h, p_long) in planet_positions[:8]:
        bh = bhava_houses[p]
        if bh > 6:
            dp = (
                (p_long - bhava_start) % 30
                if p_long > bhava_start
                else (30 - bhava_start + p_long) % 30
            )
            if _DEBUG:
                print("dp", dp)
...
def _krurodaya_harana_santhanam(planet_positions):
    ...
    """
    _malefics = [0, 2, 6]
    _benefics = const.natural_benefics
    _harana_factors = {p: 1.0 for p in [const._ascendant_symbol] + [*range(7)]}
    p_to_h = utils.get_planet_house_dictionary_from_planet_positions(planet_positions)
    h_to_p = utils.get_house_planet_list_from_planet_positions(planet_positions)
    kh_fraction = (
        1.0 - (planet_positions[0][1][0] * 30 + planet_positions[0][1][1]) / 360.0
    )
    kh1 = {
        m: kh_fraction
        for m in _malefics
        if p_to_h[m] == p_to_h[const._ascendant_symbol]
    }
    mps = {
        p: planet_positions[p + 1][1][1]
        for p in asubha_grahas
        if p_to_h[p] == p_to_h[const._ascendant_symbol]
    }
    mps = sorted(
        mps.items(), key=lambda x: x[1] - planet_positions[0][1][1]
    )  # sort by ascending close to lagna
    if len(mps) == 0:
        return _harana_factors
    mp = mps[0][0]  # consider the closest one to lagna
    aps = [
        sp
        for sp in subha_grahas
        if p_to_h[sp] == p_to_h[mp]
        and planet_positions[sp][1][1] < planet_positions[mp][1][1]
    ]
    if len(aps) > 0:
        return _harana_factors
...
def _krurodaya_harana(planet_positions, method=2):
    ...
    if method == 1:
        return _krurodaya_harana_santhanam(planet_positions)
    p_to_h = utils.get_planet_house_dictionary_from_planet_positions(planet_positions)
    h_to_p = utils.get_house_planet_list_from_planet_positions(planet_positions)
    _harana_factors = {p: 1.0 for p in [const._ascendant_symbol] + [*range(7)]}
    kh_fraction = (
    ...
    """ If a Shubha graha viz., Guru, Shukra, Budha and Chandra occupy or aspect the Lagna, then the Harana is halved."""
    if any(
        [p in subha_grahas for p in house.aspected_planets_of_the_planet(h_to_p, mp)]
    ):
        kh1.update({mp: 0.5 * kh_fraction})
    _harana_factors.update(kh1)
...
def _bharana(planet_positions):
    ...
    pp_3 = charts.drekkana_chart(planet_positions)
    pp_9 = charts.navamsa_chart(planet_positions)
    chk1 = lambda p: p in retrograde_planets or const.house_strengths_of_planets[p][
        planet_positions[p + 1][1][0]
    ] in [const._EXALTED_UCCHAM, const._OWNER_RULER]
    ph = {p: 3.0 for p in range(7) if chk1(p)}
    _bharana_factors.update(ph)
    chk23 = lambda p: planet_positions[p + 1][1][0] == pp_9[p + 1][1][0]
    chk21 = (
        lambda p: const.house_strengths_of_planets[p][pp_9[p + 1][1][0]]
        == const._OWNER_RULER
    )
    chk22 = (
        lambda p: const.house_strengths_of_planets[p][pp_3[p + 1][1][0]]
        == const._OWNER_RULER
    )
    ph1 = {
        p: 2.0
        for p in range(7)
        if chk21(p) or chk22(p) or chk23(p) and _bharana_factors[p] != 2.0
    }
    _bharana_factors.update(ph1)
    if _DEBUG:
        print("bharana factors", _bharana_factors)
...
def _apply_harana(planet_positions, base_aayu, is_amsayu=False, method=2):
    ...
    if _DEBUG:
        print(base_aayu, "graha_ayu after harana", graha_aayu)
    return graha_aayu
...
def _pindayu_santhanam(planet_positions, apply_haranas=True, method=1):
    ...
    for planet in range(7):
        planet_long = (
            planet_positions[planet + 1][1][0] * 30 + planet_positions[planet + 1][1][1]
        )
        if _DEBUG:
            print("planet", planet, "planet_long", planet_long)
        arc_of_longevity = utils.norm360(
            360 + planet_long - const.planet_deep_exaltation_longitudes[planet]
        )
        if _DEBUG:
            print("planet", planet, "arc_of_longevity", arc_of_longevity)
        if arc_of_longevity > 180.0:
            planet_base_longevity[planet] = (
                const.pindayu_full_longevity_of_planets[planet]
                * arc_of_longevity
                / 360.0
            )
        else:
        ...
        if _DEBUG:
            print(
                "planet",
                planet,
                "planet_base_longevity santhanam",
                planet_base_longevity[planet],
            )
    if apply_haranas:
...
def _pindayu(planet_positions, apply_haranas=True, method=2):
    ...
    for planet in range(7):
        ...
        effective_arc = (
            arc_of_longevity - 180 if arc_of_longevity > 180 else arc_of_longevity
        )
        planet_base_longevity[planet] = (
            const.pindayu_full_longevity_of_planets[planet] * effective_arc / 360.0
        )
    if _DEBUG:
        print("planet_base_longevity", planet_base_longevity)
    if apply_haranas:
        return _apply_harana(planet_positions, planet_base_longevity, method=method)
    else:
        return planet_base_longevity
...
def _nisargayu_santhanam(planet_positions, apply_haranas=True, method=1):
    ...
    for planet in range(7):
        ...
        if _DEBUG:
            print("_nisargayu_santhanam", "planet_long", planet_long)
        arc_of_longevity = utils.norm360(
            360 + planet_long - const.planet_deep_exaltation_longitudes[planet]
        )
        if _DEBUG:
            print("_nisargayu_santhanam", "arc_of_longevity", arc_of_longevity)
        if arc_of_longevity > 180.0:
            planet_base_longevity[planet] = (
                const.nisargayu_full_longevity_of_planets[planet]
                * arc_of_longevity
                / 360.0
            )
        else:
        ...
        if _DEBUG:
            print(
                planet, "planet_base_longevity santhanam", planet_base_longevity[planet]
            )
    if _DEBUG:
        print("planet_base_longevity santhanam", planet_base_longevity)
    if apply_haranas:
        return _apply_harana(planet_positions, planet_base_longevity, method=method)
    else:
        return planet_base_longevity
...
def _nisargayu(planet_positions, apply_haranas=True, method=2):
    ...
    for planet in range(7):
        ...
        if _DEBUG:
            print("_nisargayu", "planet_long", planet_long)
        arc_of_longevity = utils.norm360(
            planet_long - const.planet_deep_exaltation_longitudes[planet]
        )
        if _DEBUG:
            print(
                "_nisargayu",
                "arc_of_longevity",
                arc_of_longevity,
                const.planet_deep_exaltation_longitudes[planet],
            )
        effective_arc = (
            arc_of_longevity - 180 if arc_of_longevity > 180 else arc_of_longevity
        )
        if _DEBUG:
            print("_nisargayu", "effective_arc", effective_arc)
        planet_base_longevity[planet] = (
            const.nisargayu_full_longevity_of_planets[planet] * effective_arc / 360.0
        )
        if _DEBUG:
            print(planet, "planet_base_longevity", planet_base_longevity[planet])
    if _DEBUG:
        print("planet_base_longevity", planet_base_longevity)
    if apply_haranas:
...
def _amsayu(planet_positions, apply_haranas=True, method=1):
    ...
    for planet, (h, p_long) in planet_positions[1:8]:  # range(7):
        ...
        if method == 2:
            planet_base_longevity[planet] = (planet_long * 60 / 200) % 12  # Varhamihira
    if _DEBUG:
        print("planet_base_longevity", planet_base_longevity)
    if apply_haranas:
...
def _stronger_of_lagna_sun_moon(planet_positions):
    ...
    sp_house = p_to_h[sp]
    sr = house.stronger_rasi_from_planet_positions(
        planet_positions, asc_house, sp_house
    )
    if sr == asc_house:
...
def _lagna_longevity_santhanam(jd, place):
    ...
    asc_rasi = pp_rasi[0][1][0]
    asc_rasi_lord = house.house_owner_from_planet_positions(pp_rasi, asc_rasi)
    asc_rasi_long = asc_rasi * 30 + pp_rasi[0][1][1]
    if _DEBUG:
        print(asc_rasi, "asc rasi long", asc_rasi_long)
    asc_nava = (asc_rasi + 8) % 12
    asc_navamsa_lord = house.house_owner_from_planet_positions(pp_rasi, asc_nava)
    asc_nava_long = asc_nava * 30 + (asc_rasi_long // 30)
    if _DEBUG:
        print(asc_nava, "asc nava long", asc_nava_long)
    sp = house.stronger_planet_from_planet_positions(
        pp_rasi, asc_rasi_lord, asc_navamsa_lord
    )
    if _DEBUG:
        print(asc_rasi_lord, asc_navamsa_lord, "stronger is", sp)
...
def _lagna_longevity(jd, place, divisional_chart_factor=9, chart_method=1):
    ...
    asc_chart = pp_chart[0][1][0]
    asc_chart_lord = house.house_owner_from_planet_positions(pp_chart, asc_chart)
    asc_chart_long = asc_chart * 30 + pp_chart[0][1][1]
    pp_navamsa = charts.divisional_chart(
        jd,
        place,
        divisional_chart_factor=divisional_chart_factor,
        chart_method=chart_method,
    )
    asc_nava = pp_navamsa[0][1][0]
    asc_navamsa_lord = house.house_owner_from_planet_positions(pp_navamsa, asc_nava)
    asc_nava_long = asc_nava * 30 + pp_navamsa[0][1][1]
    _lagna_aayu = asc_chart_long / 30.0
    if (
        const.house_strengths_of_planets[asc_navamsa_lord][asc_nava]
        > const.house_strengths_of_planets[asc_chart_lord][asc_chart]
    ):
        _lagna_aayu = asc_nava_long / 30.0
    return _lagna_aayu
...
def _get_global_constants(jd, place):
    ...
    subha_grahas, asubha_grahas = charts.benefics_and_malefics(
        jd, place, method=1
    )  # BV Raman's method
...
def get_dhasa_antardhasa(
    ...
    bm1 = drik.bhaava_madhya(jd, place, const.bhaava_madhya_method)
    bhava_madhya = bm1[:] + [bm1[0]]
    if _DEBUG:
        print(
            "bhava madhya",
            [utils.to_dms(bm, is_lat_long="plong") for bm in bhava_madhya],
        )
    bhava_lengths = [
        (bhava_madhya[i + 1] - bhava_madhya[i]) % 60
        for i in range(len(bhava_madhya) - 1)
    ]
    bhava_starts = [bhava_madhya[i] - 0.5 * bhava_lengths[i] for i in range(12)]
    if _DEBUG:
        print(
            "bhava length",
            [utils.to_dms(bm, is_lat_long="plong") for bm in bhava_lengths],
        )
    if _DEBUG:
        print(
            "bhava start",
            [utils.to_dms(bs % 30, is_lat_long="plong") for bs in bhava_starts],
        )
    _lagna_duration = _lagna_longevity(
        jd,
        place,
        divisional_chart_factor=divisional_chart_factor,
        chart_method=chart_method,
    ...
    if _DEBUG:
        print("stronger of lagna/Sun/Moon", sp)
    p_to_h = utils.get_planet_house_dictionary_from_planet_positions(planet_positions)
    _dhasa_seed = p_to_h[sp]
    if _DEBUG:
        print("dhasa seed", _dhasa_seed)
    dhasa_progression = charts.order_planets_from_kendras_of_raasi(
        planet_positions[:8], _dhasa_seed, include_lagna=True
    )
    if sp in [0, 1, const._ascendant_symbol]:
        dhasa_progression = [sp] + [p for p in dhasa_progression if p != sp]
    if _DEBUG:
        print("dhasa progression", dhasa_progression)
    start_jd = jd
    dhasas = []

    if sp == 0:
        if _DEBUG:
            print("Aayu type Pindayu")
        ...
        if _DEBUG:
            print("aayu type nisargayu")
        _dhasa_duration = _nisargayu(
            planet_positions, apply_haranas, method=dhasa_method
        )
        _total_duration = _TOTAL_NISARGAYU
        _dhasa_type = 1  # Nisargayu Moon is the lord
    else:  # sp == const._ascendant_symbol:
        if _DEBUG:
            print("aayu type amsayu")
    ...
    if _DEBUG:
        print("dhasa duration", _dhasa_duration)
    for lord in dhasa_progression:
        ...
        if include_antardhasa:
            ...
            for bhukthi in bhukthis:
                ...
                dhasa_start = (
                    "%04d-%02d-%02d" % (y, m, d) + " " + utils.to_dms(h, as_string=True)
                )
...
def pindayu_dhasa_bhukthi(
    ...
    return get_dhasa_antardhasa(
        jd,
        place,
        aayur_type=0,
        include_antardhasa=include_antardhasa,
        apply_haranas=apply_haranas,
...
def longevity(jd, place, aayu_type=None, dhasa_method=2):
    _at, _adb = get_dhasa_antardhasa(
        jd,
        place,
        aayur_type=aayu_type,
