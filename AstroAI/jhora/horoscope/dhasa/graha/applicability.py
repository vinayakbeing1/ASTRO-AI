...
def applicability_check(dob, tob, place, divisional_chart_factor=1):
    ...
    if ashtottari.applicability_check(planet_positions):
        applicable_dhasas.append("ashtottari")
    from jhora.horoscope.dhasa.graha import chathuraaseethi_sama

    if chathuraaseethi_sama.applicability_check(planet_positions):
        applicable_dhasas.append("chaturaaseeti_sama")
    from jhora.horoscope.dhasa.graha import dwadasottari

    navamsa_planet_positions = charts.divisional_chart(
        jd, place, divisional_chart_factor=9
    )
    if dwadasottari.applicability_check(navamsa_planet_positions):
        applicable_dhasas.append("dwadasottari")
    from jhora.horoscope.dhasa.graha import dwisatpathi

    if dwisatpathi.applicability_check(planet_positions):
        applicable_dhasas.append("dwisatpathi")
    dwadasamsa_planet_positions = charts.divisional_chart(
        jd, place, divisional_chart_factor=12
    )
    from jhora.horoscope.dhasa.graha import panchottari

    if panchottari.applicability_check(dwadasamsa_planet_positions):
        applicable_dhasas.append("panchottari")
    from jhora.horoscope.dhasa.graha import sataatbika

    if sataatbika.applicability_check(dob, tob, place):
        applicable_dhasas.append("satabdika")
    from jhora.horoscope.dhasa.graha import shastihayani

    if shastihayani.applicability_check(planet_positions):
        applicable_dhasas.append("shashtisama")
