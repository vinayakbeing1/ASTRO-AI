...
count_direction = (
    1  # 1> base star to birth star zodiac -1> base star to birth star antizodiac
)
...
def applicability_check(planet_positions):
    ...
    seventh_lord = house.house_owner_from_planet_positions(
        planet_positions, seventh_house
    )
    return (planet_positions[seventh_lord + 1][1][0] == lagna) or (
        planet_positions[lagna_lord + 1][1][0] == seventh_house
    )
...
def _next_adhipati(lord, dirn=1):
    ...
    next_lord = list(dhasa_adhipathi_list.keys())[
        ((current + dirn) % len(dhasa_adhipathi_list))
    ]
    return next_lord
...
def get_dhasa_bhukthi(
    ...
    for _ in range(_dhasa_cycles):
        for _ in range(len(dhasa_adhipathi_list)):
            _dhasa_duration = round(
                dhasa_adhipathi_list[dhasa_lord] * _tribhagi_factor, 2
            )
            if include_antardhasa:
                ...
                for bhukthi_lord in bhukthis:
                    ...
                    retval.append(
                        (dhasa_lord, bhukthi_lord, dhasa_start, _dhasa_duration)
                    )
