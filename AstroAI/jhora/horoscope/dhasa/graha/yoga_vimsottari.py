...
sidereal_year = (
    const.sidereal_year
)  # const.savana_year #  # some say 360 days, others 365.25 or 365.2563 etc
vimsottari_dict = {
    8: [(3, 12, 21), 7],
    5: [(4, 13, 22), 20],
    0: [(5, 14, 23), 6],
...
def vimsottari_mahadasa(jdut1, place):
    ...
    for i in range(9):
        ...
        lord_duration = vimsottari_dict[lord][1]
...
def _vimsottari_bhukti(maha_lord, start_date, antardhasa_option=1):
    ...
    for _ in range(len(vimsottari_dict)):
        retval[lord] = start_date
        bhukthi_duration = vimsottari_dict[lord][1]
        factor = (
            bhukthi_duration
            * dhasa_lord_duration
    ...
    for _ in range(_dhasa_cycles):
        for i in dashas:
            bhuktis = _vimsottari_bhukti(
                i, dashas[i], antardhasa_option=antardhasa_option
            )
            dhasa_lord = i
            for j in bhuktis:
                bhukthi_lord = j
