...
def get_dhasa_bhukthi(
    ...
    antardhasa_option1=False,
    antardhasa_option2=False,
):
    """
    ...
    if antardhasa_option1:
        bhukthi_house_list = [
            p for p in bhukthi_house_list if p not in _bhukthi_exempt_list_1
        ]
    if antardhasa_option2:
        bhukthi_house_list = [
            p for p in bhukthi_house_list if p not in _bhukthi_exempt_list_2
        ]
    ...
    for dhasa_lord in dhasa_lords:
        ...
        lord_house = (
            planet_positions[dhasa_lord + 1][1][0]
            if dhasa_lord != const._ascendant_symbol
            else planet_positions[0][1][0]
        )
        bhukthis = [
            h_to_p[(h + lord_house) % 12].split("/")
            for h in bhukthi_house_list
            if h_to_p[(h + lord_house) % 12] != ""
        ]
        bhukthis = utils.flatten_list(bhukthis)
        [bhukthis.remove(p) for p in ["L", "7", "8"] if p in bhukthis]
        if (
            mahadhasa_lord_has_no_antardhasa
            and dhasa_lord != const._ascendant_symbol
            and str(dhasa_lord) in bhukthis
        ):
