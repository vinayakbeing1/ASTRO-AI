...
def _dhasa_progression_and_periods(jd, place):
    ...
    if birth_time > dawn_start and birth_time < dawn_end:  # dawn
        ...
        kaala_frac = (birth_time - yday_night_start) / (
            yday_night_end - yday_night_start
        )
...
def get_dhasa_antardhasa(
    dob, tob, place, years=1, months=1, sixty_hours=1, include_antardhasa=False
):
    """
    ...
    (
        ...
        dhasas_first,
        dhasas_second,
    ) = _dhasa_progression_and_periods(jd_years, place)
    dhasa_info = []
    start_jd = jd_years
    for dhasa_lord in range(9):
        ...
        if include_antardhasa:
            ...
            for bhukthi_lord in range(9):
                ...
                dhasa_info.append(
                    (dhasa_lord, bhukthi_lord, dhasa_start, round(_bhukthi_duration, 2))
                )
