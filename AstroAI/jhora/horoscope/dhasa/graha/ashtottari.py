...
def applicability_check(planet_positions):
    ...
    chk1 = (
        rahu_house in house.trines_of_the_raasi(house_of_lagna_lord)
        and rahu_house != asc_house
    )
    chk2 = (
        rahu_house in house.quadrants_of_the_raasi(house_of_lagna_lord)
        and rahu_house != asc_house
    )
    return chk1 or chk2
...
def ashtottari_bhukthi(dhasa_lord, start_date, antardhasa_option=1):
    ...
    for _ in range(len(ashtottari_adhipathi_list)):
        ...
        lord_duration = ashtottari_adhipathi_dict[lord][1]
        factor = (
            lord_duration * dhasa_lord_duration / human_life_span_for_ashtottari_dhasa
        )
...
def get_ashtottari_dhasa_bhukthi(
    ...
    if use_tribhagi_variation:
        ...
        for k, (v1, v2) in ashtottari_adhipathi_dict.items():
            ashtottari_adhipathi_dict[k] = [
                v1,
                v2 * _tribhagi_factor,
            ]  # [v1,round(v2*_tribhagi_factor,2)]
