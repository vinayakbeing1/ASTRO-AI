...
class Horoscope:
    def __init__(
        ...
        if self.place_name == None:
            if (
                self.latitude == None
                or self.longitude == None
                or self.timezone_offset == None
            ):
                print(
                    "Please provide either place_with_country_code or combination of latitude and longitude ...\n Aborting script"
                )
                exit()
            else:
        ...
        self.Place = drik.Place(
            self.place_name, self.latitude, self.longitude, self.timezone_offset
        )
        ...
        if calculation_type not in const.available_horoscope_calculation_methods:
            print(
                "calculation type",
                calculation_type,
                " not in list of available calculation methods",
                const.available_horoscope_calculation_methods,
                "Default:drik is used",
            )
        ...
        if self.calculation_type == "ss":
            print(
                "Horoscope:main: Forcing ayanamsa to SURYASIDDHANTA for the SURYA SIDDHANTA calculation type"
            )
        ...
        self.julian_years = drik.next_solar_date(
            self.julian_day, place, years, months, sixty_hours
        )
        self.julian_years_utc = utils.julian_day_utc(self.julian_day, self.Place)
        self.calendar_info = self.get_calendar_information()
        self.bhava_chart, self.bhava_chart_info = self.get_bhava_chart_information(
            self.julian_years, place, self._bhava_madhya_method
        )
        return
    ...
    def _get_calendar_resource_strings(self):  # , language='en'):
        ...
        cal_key_list = utils._read_resource_messages_from_file(
            msg_file
        )  # utils.get_resource_messages(msg_file)
        _ = utils._read_resource_lists_from_file(
            list_file
        )  # utils.get_resource_lists(list_file)
        return cal_key_list
    ...
    def get_calendar_information(self):  # , language='en'):
        ...
        calendar_info = {
            ...
            cal_key_list["latitude_str"]: utils.to_dms(
                self.latitude, is_lat_long="lat", as_string=as_string
            ),  # "{0:.2f}".format(self.latitude),
            cal_key_list["longitude_str"]: utils.to_dms(
                self.longitude, is_lat_long="long", as_string=as_string
            ),  # "{0:.2f}".format(self.longitude),
            cal_key_list["timezone_offset_str"]: "{0:.2f}".format(self.timezone_offset),
            cal_key_list["report_date_str"]: "{0:d}-{1:d}-{2:d}".format(
                self.Date.year, self.Date.month, self.Date.day
            ),  # self.Date.isoformat(),
        }
        vaaram = drik.vaara(jd)
        calendar_info[cal_key_list["vaaram_str"]] = utils.DAYS_LIST[vaaram]
        calendar_info[cal_key_list["calculation_type_str"]] = cal_key_list[
            "drik_panchang_str"
        ]
        ...
        calendar_info[cal_key_list["lunar_year_month_str"]] = (
            utils.YEAR_LIST[_samvatsara]
            + " / "
            + utils.MONTH_LIST[maasam_no - 1]
            + " "
            + adhik_maasa_str
            + nija_month_str
        )
        calendar_info[cal_key_list["tamil_month_str"]] = (
            utils.MONTH_LIST[tm] + " " + cal_key_list["date_str"] + " " + str(td)
        )
        ...
        calendar_info[cal_key_list["tithi_str"]] = (
            ...
            + " "
            + utils.TITHI_LIST[_tithi[0] - 1]
            + " ("
            + utils.TITHI_DEITIES[_tithi[0] - 1]
            + ") "
            + utils.to_dms(_tithi[1])
            + " "
            + cal_key_list["starts_at_str"]
            + " "
            + utils.to_dms(_tithi[2])
            + " "
            + cal_key_list["ends_at_str"]
            + " ("
            + "{0:.2f}".format(frac_left)
            + "% "
        ...
        calendar_info[cal_key_list["raasi_str"]] = (
            utils.RAASI_LIST[rasi[0] - 1]
            + " "
            + utils.to_dms(rasi[1])
            + " "
            + cal_key_list["ends_at_str"]
        ...
        calendar_info[cal_key_list["nakshatra_str"]] = (
            ...
            + " "
            + " ("
            + utils.PLANET_SHORT_NAMES[utils.nakshathra_lord(nak[0])]
            + ") "
            + cal_key_list["paadham_str"]
            + str(nak[1])
            + " "
            + utils.to_dms(nak[2])
            + " "
        ...
        calendar_info[cal_key_list["raahu_kaalam_str"]] = (
            _raahu_kaalam[0]
            + " "
            + cal_key_list["starts_at_str"]
            + " "
        ...
        calendar_info[cal_key_list["kuligai_str"]] = (
            kuligai[0]
            + " "
            + cal_key_list["starts_at_str"]
            + " "
        ...
        calendar_info[cal_key_list["yamagandam_str"]] = (
            yamagandam[0]
            + " "
            + cal_key_list["starts_at_str"]
            + " "
        ...
        calendar_info[cal_key_list["abhijit_str"]] = (
            abhijit[0]
            + " "
            + cal_key_list["starts_at_str"]
            + " "
        ...
        calendar_info[cal_key_list["dhurmuhurtham_str"]] = (
            _dhurmuhurtham[0]
            + " "
            + cal_key_list["starts_at_str"]
            + " "
    ...
    def get_horoscope_chart_counter(self, chart_key):
        ...
        counter = [
            index for index, value in enumerate(value_list) if chart_key in value
        ][0]
        return counter

    def get_bhava_chart_information(
        self, jd, place, bhaava_madhya_method=const.bhaava_madhya_method
    ):
        _bhava_chart_info = []
        cal_key_list = self.cal_key_list
        _bhava_chart = ["" for _ in range(len(utils.RAASI_LIST))]
        # jd = self.julian_day; place = self.Place
        _bhava_info = charts.bhava_chart(
            jd, place, bhava_madhya_method=bhaava_madhya_method
        )
        ...
        for br, (bs, bm, be), pls in _bhava_info:
            ...
            bs1 = drik.dasavarga_from_long(bs)
            bm1 = drik.dasavarga_from_long(bm)
            be1 = drik.dasavarga_from_long(be)
            bss = (
                utils.RAASI_SHORT_LIST[bs1[0]]
                + " "
                + utils.to_dms(bs1[1], is_lat_long="plong")
            )
            bms = (
                utils.RAASI_SHORT_LIST[bm1[0]]
                + " "
                + utils.to_dms(bm1[1], is_lat_long="plong")
            )
            bes = (
                utils.RAASI_SHORT_LIST[be1[0]]
                + " "
                + utils.to_dms(be1[1], is_lat_long="plong")
            )
            ps = ""
            retStr = ""
            for p in pls:
    ...
    def get_horoscope_information_for_chart(
        ...
        horoscope_info = {}
        self._vimsottari_balance = ()
        self._yoga_vimsottari_balance = ()
        self._arudha_lagna_data_kundali = {}
        self._sphuta_data_kundali = {}
        self._graha_lagna_data_kundali = {}
        self._hora_lagna_data_kundali = {}
        self._ghati_lagna_data_kundali = {}
        self._vighati_lagna_data_kundali = {}
        self._pranapada_lagna_data_kundali = {}
        self._indu_lagna_data_kundali = {}
        self._bhrigu_bindhu_lagna_data_kundali = {}
        self._bhava_lagna_data_kundali = {}
        self._sree_lagna_data_kundali = {}
        self._kunda_lagna_data_kundali = {}
        self._varnada_lagna_data_kundali = {}
        self._maandhi_data_kundali = (
            {}
        )  # ; self._aayu_dhasa_type = -1; self._kaala_dhasa_type = -1
        horoscope_charts = ["" for _ in range(len(utils.RAASI_LIST))]
        horoscope_ascendant_house = -1
        cal_key_list = (
            self.cal_key_list
        )  # self._get_calendar_resource_strings(language)
        jd = (
            self.julian_day
        )  # V3.1.9 If Julian_Years to be used then years/months arguments should not be used
        place = drik.Place(
            self.place_name, self.latitude, self.longitude, self.timezone_offset
        )
        ...
        jd = (
            self.julian_years
        )  # V3.1.9 Special Lagna do not take years arguments - so use julian years
        key_dhasa_factor = (
            dhasavarga_dict[dhasavarga_factor]
            if divisional_chart_factor in const.division_chart_factors
            else cal_key_list["custom_varga_kundali_str"]
        )
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["bhava_lagna_str"]
            + " ("
            + cal_key_list["bhava_lagna_short_str"]
            + ")"
        )
        value = drik.bhava_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        horoscope_info[key] = (
            utils.RAASI_LIST[value[0]]
            + " "
            + utils.to_dms(value[1], is_lat_long="plong")
        )
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["hora_lagna_str"]
            + " ("
            + cal_key_list["hora_lagna_short_str"]
            + ")"
        )
        value = drik.hora_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["ghati_lagna_str"]
            + " ("
            + cal_key_list["ghati_lagna_short_str"]
            + ")"
        )
        value = drik.ghati_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["vighati_lagna_str"]
            + " ("
            + cal_key_list["vighati_lagna_short_str"]
            + ")"
        )
        value = drik.vighati_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["pranapada_lagna_str"]
            + " ("
            + cal_key_list["pranapada_lagna_short_str"]
            + ")"
        )
        value = drik.pranapada_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["indu_lagna_str"]
            + " ("
            + cal_key_list["indu_lagna_short_str"]
            + ")"
        )
        value = drik.indu_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["bhrigu_bindhu_lagna_str"]
            + " ("
            + cal_key_list["bhrigu_bindhu_lagna_short_str"]
            + ")"
        )
        value = drik.bhrigu_bindhu_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["kunda_lagna_str"]
            + " ("
            + cal_key_list["kunda_lagna_short_str"]
            + ")"
        )
        value = drik.kunda_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["sree_lagna_str"]
            + " ("
            + cal_key_list["sree_lagna_short_str"]
            + ")"
        )
        jd = self.julian_day  # V3.1.9 revert to julian after special lagna calculations
        value = drik.sree_lagna(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            divisional_chart_factor=dhasavarga_factor,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["varnada_lagna_str"]
            + " ("
            + cal_key_list["varnada_lagna_short_str"]
            + ")"
        )
        value = charts.varnada_lagna(
            dob,
            tob,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
        ...
        key = (
            key_dhasa_factor
            + "-"
            + cal_key_list["maandi_str"]
            + " ("
            + cal_key_list["maandi_short_str"]
            + ")"
        )
        value = drik.maandi_longitude(
            dob,
            tob,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
        ...
        horoscope_info[key_dhasa_factor + "-" + cal_key_list["ascendant_str"]] = (
            utils.RAASI_LIST[ascendant_navamsa[0]]
            + " "
            + utils.to_dms(ascendant_navamsa[1], True, "plong")
        )
        """ Chara karakas are based only on Rasi Chart???. Fixed V3.7.2 """
        chara_karaka_names = [x + "_str" for x in house.chara_karaka_names]
        chara_karaka_dict = get_chara_karakas(
            jd,
            place,
            ayanamsa_mode=self.ayanamsa_mode,
            years=self.years,
        ...
        for p, (h, long) in planet_positions[1:]:
            ...
            if p != "L" and p < 8:
                ...
                ck_str = (
                    " (" + cal_key_list[chara_karaka_names[ck_index]] + ")"
                )  # Fixed V3.7.2
            v = (
                utils.RAASI_LIST[h]
                + " "
            ...
            horoscope_info[k] = v
        sub_planet_list_1 = {
            "kaala_str": "kaala_longitude",
            "mrityu_str": "mrityu_longitude",
            "artha_str": "artha_praharaka_longitude",
            "yama_str": "yama_ghantaka_longitude",
            "gulika_str": "gulika_longitude",
            "maandi_str": "maandi_longitude",
        }
        sub_planet_list_2 = [
            "dhuma",
            "vyatipaata",
        ...
        for sp, sp_func in sub_planet_list_1.items():
            k = (
                ...
                + cal_key_list[sp]
                + " ("
                + cal_key_list[sp.replace("_str", "_short_str")]
                + ")"
            )
            v = eval(
                "drik."
                + sp_func
                + "(dob,tob,place,ayanamsa_mode=self.ayanamsa_mode,divisional_chart_factor=dhasavarga_factor)"
            )
            horoscope_info[k] = (
                utils.RAASI_LIST[v[0]] + " " + utils.to_dms(v[1], is_lat_long="plong")
            )
        for sp in sub_planet_list_2:
            k = (
                key_dhasa_factor
                + "-"
                + cal_key_list[sp + "_str"]
                + " ("
                + cal_key_list[sp + "_short_str"]
                + ")"
            )
            v = eval(
                "charts."
                + "solar_upagraha_longitudes(planet_positions,sp,divisional_chart_factor=dhasavarga_factor)"
            )  # Fixed in V3.6.9
            horoscope_info[k] = (
                utils.RAASI_LIST[v[0]] + " " + utils.to_dms(v[1], is_lat_long="plong")
            )
        ...
        for spl in spl_list:
            ...
            vl = eval(
                "sphuta."
                + spl
                + "_sphuta(dob,tob,place,divisional_chart_factor=dhasavarga_factor,chart_method=chart_method,base_rasi=base_rasi,count_from_end_of_sign=count_from_end_of_sign)"
            )
            k = (
                key_dhasa_factor
                + "-"
                + cal_key_list[spl + "_sphuta_str"]
                + " "
                + cal_key_list["sphuta_str"]
            )
            horoscope_info[k] = (
                utils.RAASI_LIST[vl[0]] + " " + utils.to_dms(vl[1], is_lat_long="plong")
            )
    ...
    def get_special_planets_for_chart(
        self,
        jd,
        place,
        ...
        spl = charts.special_planet_longitudes(
            dob,
            tob,
            place,
        ...
        for sp, (h, _) in spl:
            _special_planet_chart[h] += (
                cal_key_list[sub_planet_list[sp].replace("_str", "_short_str")] + "\n"
            )
        return {cal_key_list["upagraha_str"]: _special_planet_chart}

    def get_special_planets_for_mixed_chart(
        self,
        jd,
        place,
        varga_factor_1=1,
        ...
        spl = charts.special_planet_longitudes_mixed_chart(
            dob,
            tob,
            place,
    ...
    def get_horoscope_information_for_mixed_chart(
        ...
        planet_positions = charts.mixed_chart(
            jd,
            place,
            varga_factor_1=dhasavarga_factor_1,
            chart_method_1=chart_method_1,
            varga_factor_2=dhasavarga_factor_2,
            chart_method_2=chart_method_2,
        )
        ascendant_navamsa = planet_positions[0][1]
        asc_house = ascendant_navamsa[0]
        ...
        abl = self._get_arudha_padhas_mixed_chart(
            dob,
            tob,
            place,
            dhasavarga_factor_1,
        ...
        key_dhasa_factor = (
            "D" + str(dhasavarga_factor_1) + "x" + "D" + str(dhasavarga_factor_2)
        )
        key = (
            key_dhasa_factor
            + "-"
        ...
        value = drik.bhava_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = drik.hora_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = drik.ghati_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = drik.vighati_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = drik.pranapada_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = drik.indu_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = drik.bhrigu_bindhu_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = drik.kunda_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        jd = self.julian_day  # V3.1.9 revert to julian after special lagna calculations
        value = drik.sree_lagna_mixed_chart(
            jd,
            place,
            dhasavarga_factor_1,
            chart_method_1,
        ...
        value = charts.varnada_lagna_mixed_chart(
            dob,
            tob,
            place,
            self.ayanamsa_mode,
        ...
        for sp, (h, long) in spl:
            k = (
                key_dhasa_factor
                + "-"
                + cal_key_list[sub_planet_list[sp]]
                + " ("
                + cal_key_list[sub_planet_list[sp].replace("_str", "_short_str")]
                + ")"
            )
            horoscope_info[k] = (
                utils.RAASI_LIST[h] + " " + utils.to_dms(long, is_lat_long="plong")
            )
            self._special_planet_chart[h] += (
                cal_key_list[sub_planet_list[sp].replace("_str", "_short_str")] + "\n"
            )
        for h in range(12):
        ...
        for spl in spl_list:
            ...
            vl = vl = eval(
                "sphuta."
                + spl
                + "_sphuta_mixed_chart(dob,tob,place, varga_factor_1=dhasavarga_factor_1, chart_method_1=chart_method_1, varga_factor_2=dhasavarga_factor_2, chart_method_2=chart_method_2)"
            )
            k = (
                key_dhasa_factor
                + "-"
    ...
    def get_horoscope_information(self):  # ,language='en'):
        ...
        self._vighati_lagna_data = {}
        self._pranapada_lagna_data = {}
        self._indu_lagna_data = {}
        self._bhrigu_bindhu_lagna_data = {}
        self._bhava_lagna_data = {}
        self._sree_lagna_data = {}
        self._kunda_lagna_data = {}
        self._varnada_lagna_data = {}
        self._maandhi_data = {}
        self._aayu_dhasa_type = -1
        self._kaala_dhasa_type = -1
        cal_key_list = (
            self.cal_key_list
        )  # self._get_calendar_resource_strings(language)
        global dhasavarga_dict
        ...
        horoscope_charts = [
            ["" for _ in range(len(utils.RAASI_LIST))]
            for _ in range(len(dhasavarga_dict) + 1)
        ]
        horoscope_ascendant_houses = [
            -1 for _ in range(len(const.division_chart_factors))
        ]
        chart_counter = 0
        divisional_chart_factor = 1
        ...
        horoscope_charts[chart_counter][asc_house] += (
            cal_key_list["ascendant_str"] + "\n"
        )
        horoscope_info[
            cal_key_list["raasi_str"] + "-" + cal_key_list["ascendant_str"]
        ] = (
            utils.RAASI_LIST[asc_house]
            + " "
            + utils.to_dms(_ascendant[1], is_lat_long="plong")
        )
        chara_karaka_names = [x + "_str" for x in house.chara_karaka_names]
        chara_karaka_dict = house.chara_karakas(planet_positions)
        for p, (h, long) in planet_positions[1:]:
            ...
            relative_planet_house = house.get_relative_house_of_planet(
                asc_house, planet_house
            )
        ...
        for sp, sp_func in sub_planet_list_1.items():
            ...
            v = eval(
                "drik."
                + sp_func
                + "(dob,tob,place,ayanamsa_mode=self.ayanamsa_mode,divisional_chart_factor=divisional_chart_factor)"
            )
            horoscope_info[k] = (
                utils.RAASI_LIST[v[0]] + " " + utils.to_dms(v[1], is_lat_long="plong")
            )
        for sp in sub_planet_list_2:
            ...
            v = eval(
                "drik."
                + "solar_upagraha_longitudes(sun_long,sp,divisional_chart_factor=divisional_chart_factor)"
            )
            horoscope_info[k] = (
                utils.RAASI_LIST[v[0]] + " " + utils.to_dms(v[1], is_lat_long="plong")
            )
        ...
        for dhasavarga_factor in dhasavarga_dict.keys():
            ...
            jd = (
                self.julian_day
            )  # V3.1.9 revert to julian after special lagna calculations
            value = drik.sree_lagna(
                jd,
                place,
                ayanamsa_mode=self.ayanamsa_mode,
            ...
            horoscope_info[
                dhasavarga_dict[dhasavarga_factor] + "-" + cal_key_list["ascendant_str"]
            ] = (
                utils.RAASI_LIST[ascendant_navamsa[0]]
                + " "
                + utils.to_dms(ascendant_navamsa[1], True, "plong")
            ...
            for p, (h, long) in planet_positions[1:]:
                ...
                if p != "L" and p < 8:
                    ...
                    ck_str = (
                        " ("
                        + cal_key_list[chara_karaka_names[chara_karaka_dict[p]]]
                        + ")"
                    )
                v = (
                    utils.RAASI_LIST[h]
            ...
            for sp, sp_func in sub_planet_list_1.items():
                ...
                v = eval(
                    "drik."
                    + sp_func
                    + "(dob,tob,place,divisional_chart_factor=dhasavarga_factor)"
                )
                horoscope_info[k] = (
                    utils.RAASI_LIST[v[0]]
                    + " "
                    + utils.to_dms(v[1], is_lat_long="plong")
                )
            for sp in sub_planet_list_2:
                k = (
                    dhasavarga_dict[dhasavarga_factor]
        ...
        return (
            ...
            horoscope_charts,
            horoscope_ascendant_houses,
        )  # , vimsottari_dhasa_bhukti_info,ashtottari_dhasa_bhukti_info,narayana_dhasa_info

    def get_varnada_lagna_for_chart(
        self,
        dob,
        tob,
        place,
    ...
    def get_varnada_lagna_for_mixed_chart(
        self,
        dob,
        tob,
        place,
    ...
    def _get_vimsopaka_bala(self, dob, tob, place_as_tuple):
        ...
        sv1 = {}
        for p in range(9):
            sv1[utils.PLANET_NAMES[p]] = (
                utils.SHADVARGAMSA_NAMES[sv[p][0]]
                + "\n("
                + sv[p][1]
                + ")\n"
                + str(round(sv[p][2], 1))
        ...
        sv2 = {}
        for p in range(9):
            sv2[utils.PLANET_NAMES[p]] = (
                utils.SAPTAVARGAMSA_NAMES[sv[p][0]]
                + "\n("
                + sv[p][1]
                + ")\n"
                + str(round(sv[p][2], 1))
        ...
        dv = {}
        for p in range(9):
            dv[utils.PLANET_NAMES[p]] = (
                utils.DHASAVARGAMSA_NAMES[sv[p][0]]
                + "\n("
                + sv[p][1]
                + ")\n"
                + str(round(sv[p][2], 1))
        ...
        sv3 = {}
        for p in range(9):
            sv3[utils.PLANET_NAMES[p]] = (
                utils.SHODASAVARGAMSA_NAMES[sv[p][0]]
                + "\n("
                + sv[p][1]
                + ")\n"
                + str(round(sv[p][2], 1))
    ...
    def _get_sphuta_mixed_chart(
        ...
        for s in const.sphuta_list:
            key = (
                self.cal_key_list[s + "_sphuta_str"]
                + " "
                + self.cal_key_list["sphuta_str"]
                + " ("
                + self.cal_key_list[s + "_sphuta_short_str"]
                + ")"
            )
            fn = (
                "sphuta."
                + s
                + "_sphuta_mixed_chart(dob,tob,place,varga_factor_1=varga_factor_1,chart_method_1=chart_method_1,varga_factor_2=varga_factor_2,chart_method_2=chart_method_2)"
            )
            value = eval(fn)
            _sphuta_dict[key] = (
                utils.RAASI_LIST[value[0]]
    ...
    def _get_sphuta(
        ...
        for s in const.sphuta_list:
            ...
            fn = (
                "sphuta."
                + s
                + "_sphuta(dob,tob,place,divisional_chart_factor=divisional_chart_factor,chart_method=chart_method,base_rasi=base_rasi,count_from_end_of_sign=count_from_end_of_sign)"
            )
            value = eval(fn)
            _sphuta_dict[key] = (
                utils.RAASI_LIST[value[0]]
    ...
    def _get_arudha_padhas_mixed_chart(
        self,
        dob,
        tob,
        place,
        ...
        for i, h in enumerate(houses):
            bhava_arudhas[
                ...
                + " ("
                + self.cal_key_list["bhava_arudha_a" + str(i + 1) + "_short_str"]
                + ")"
            ] = utils.RAASI_LIST[ba[i]]
    ...
    def _get_arudha_padhas_menu_dict(self, planet_positions):
        ...
        arudha_base_list = [
            self.cal_key_list["ascendant_short_str"]
        ] + utils.PLANET_SHORT_NAMES[: const._planets_upto_ketu]
        ...
        for ab, (key, _) in enumerate(arudha_menu_dict.items()):
            ba = arudhas.bhava_arudhas_from_planet_positions(
                planet_positions, arudha_base=ab
            )
            ba_chart = ["" for _ in range(12)]
            astr = arudha_base_list[ab]
            for p, r in enumerate(ba):
                ba_chart[r] += (
                    self.cal_key_list["bhava_arudha_a" + str(p + 1) + "_short_str"]
                    + "\n"
                    if ab == 0
                    else astr + str(p + 1) + "\n"
                )
    ...
    def _get_graha_dhasa_bhukthi(self, dob, tob, place):
        (
            self._vimsottari_balance,
            _vimsottari_dhasa_bhkthi_info,
        ) = self._get_vimsottari_dhasa_bhukthi(dob, tob, place)
        # print('horo _vimsottari_balance',self._vimsottari_balance)
        _ashtottari_dhasa_bhkthi_info = self._get_ashtottari_dhasa_bhukthi(
            dob, tob, place
        )
        _yogini_dhasa_bhkthi_info = self._get_yogini_dhasa_bhukthi(dob, tob, place)
        return (
            _vimsottari_dhasa_bhkthi_info,
            _ashtottari_dhasa_bhkthi_info,
            _yogini_dhasa_bhkthi_info,
        )

    def _get_rasi_dhasa_bhukthi(self, dob, tob, place):
        _narayana_dhasa_bhukthi_info = self._get_narayana_dhasa(dob, tob, place)
        _kendraadhi_rasi_dhasa_bhukthi_info = self._get_kendraadhi_rasi_dhasa(
            dob, tob, place
        )
        ...
        _kendraadhi_karaka_dhasa_bhukthi_info = self._get_kendraadhi_karaka_dhasa(
            dob, tob, place
        )
        ...
        _padhanadhamsa_dhasa_bhukthi_info = self._get_padhanadhamsa_dhasa(
            dob, tob, place
        )
        ...
        return [
            _narayana_dhasa_bhukthi_info,
            _kendraadhi_rasi_dhasa_bhukthi_info,
            _sudasa_dhasa_bhukthi_info,
            _drig_dhasa_bhukthi_info,
            _nirayana_dhasa_bhukthi_info,
            _shoola_dhasa_bhukthi_info,
            _kendraadhi_karaka_dhasa_bhukthi_info,
            _chara_dhasa_bhukthi_info,
            _lagnamsaka_dhasa_bhukthi_info,
    ...
    def _get_annual_dhasa_bhukthi(self, divisional_chart_factor=1):
        ...
        _mudda_dhasa_bhukthi_info = self._get_varsha_vimsottari_dhasa(
            self.julian_day,
            self.Place,
            self.years - 1,
            divisional_chart_factor=divisional_chart_factor,
        )
        _varsha_narayana_dhasa_bhukthi_info = self._get_varsha_narayana_dhasa(
            self.Date,
            self.birth_time,
            self.Place,
            self.years,
        ...
        return [
            _patyayini_dhasa_bhukthi_info,
            _mudda_dhasa_bhukthi_info,
            _varsha_narayana_dhasa_bhukthi_info,
        ]

    def _get_varsha_narayana_dhasa(
        self, dob, tob, place, years, divisional_chart_factor=1
    ):
        from jhora.horoscope.dhasa.raasi import narayana

        db = narayana.varsha_narayana_dhasa_bhukthi(
            dob,
            tob,
            place,
            years,
        ...
        for i in range(len(db)):
            ...
            dhasa_bhukti_info.append(
                (
                    utils.RAASI_LIST[dhasa_lord] + "-" + utils.RAASI_LIST[bukthi_lord],
                    bukthi_start,
                )
            )
    ...
    def _get_varsha_vimsottari_dhasa(self, jd, place, years, divisional_chart_factor=1):
        ...
        md = mudda.varsha_vimsottari_dhasa_bhukthi(
            jd, place, years, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []
    ...
    def _get_patyatini_dhasa_bhukthi(self, divisional_chart_factor=1):
        ...
        self.julian_years = drik.next_solar_date(
            self.julian_day, self.Place, self.years, self.months, self.sixty_hours
        )
        p_d_b = patyayini.patyayini_dhasa(
            self.julian_years,
            self.Place,
            self.ayanamsa_mode,
            divisional_chart_factor=divisional_chart_factor,
    ...
    def _get_karaka_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = karaka.get_dhasa_antardhasa(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        chara_karaka_names = [x + "_str" for x in house.chara_karaka_names]
        chara_karaka_dict = house.chara_karakas(planet_positions)
        dhasa_bhukti_info = []
        for i in range(len(db)):
            ...
            dhasa_bhukti_info.append(
                (
                    ...
                    + "\n"
                    + self.cal_key_list[
                        chara_karaka_names[chara_karaka_dict[dhasa_lord]]
                    ]
                    + "\n"
                    + "-"
                    + utils.PLANET_NAMES[bukthi_lord]
                    + "\n"
                    + self.cal_key_list[
                        chara_karaka_names[chara_karaka_dict[bukthi_lord]]
                    ]
                    + "\n",
                    bukthi_start,
                )
    ...
    def _get_naisargika_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = naisargika.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
            [dhasa_lord, bukthi_lord, bukthi_start, _] = db[i]
            dhasa_str = (
                self.cal_key_list["ascendant_str"]
                if dhasa_lord == const._ascendant_symbol
                else utils.PLANET_NAMES[dhasa_lord]
            )
            bukthi_str = (
                self.cal_key_list["ascendant_str"]
                if bukthi_lord == const._ascendant_symbol
                else utils.PLANET_NAMES[bukthi_lord]
            )
    ...
    def _get_aayu_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        self._aayu_dhasa_type, db = aayu.get_dhasa_antardhasa(
            jd, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
            [dhasa_lord, bukthi_lord, bukthi_start, _] = db[i]
    ...
    def _get_vimsottari_dhasa_bhukthi(
        self, dob, tob, place, **kwargs
    ):  # ,divisional_chart_factor=1):
        jd = utils.julian_day_number(dob, tob)
        from jhora.horoscope.dhasa.graha import vimsottari

        self._vimsottari_balance, db = vimsottari.get_vimsottari_dhasa_bhukthi(
            jd, place, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_kaala_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        self._kaala_dhasa_type, db = kaala.get_dhasa_antardhasa(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_chakra_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = chakra.get_dhasa_antardhasa(
            dob,
            tob,
            place,
            include_antardhasa=True,
    ...
    def _get_sandhya_panchaka_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = sandhya.get_dhasa_antardhasa(
            dob,
            tob,
            place,
            use_panchaka_variation=True,
            divisional_chart_factor=divisional_chart_factor,
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_yoga_vimsottari_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        self._yoga_vimsottari_balance, db = yoga_vimsottari.get_dhasa_bhukthi(
            jd, place, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_buddhi_gathi_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        jd = utils.julian_day_number(dob, tob)
        db = buddhi_gathi.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_tithi_yogini_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = tithi_yogini.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_shodasottari_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = shodasottari.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_dwadasottari_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = dwadasottari.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_dwisatpathi_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = dwisatpathi.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_panchottari_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = panchottari.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_satabdika_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = sataatbika.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_chaturaaseeti_sama_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = chathuraaseethi_sama.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_karana_chaturaaseeti_sama_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = karana_chathuraaseethi_sama.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_shashtisama_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = shastihayani.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_shattrimsa_sama_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = shattrimsa_sama.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
    ...
    def _get_saptharishi_nakshathra_dhasa_bhukthi(self, dob, tob, place, **kwargs):
        ...
        db = saptharishi_nakshathra.get_dhasa_bhukthi(
            dob, tob, place, include_antardhasa=True, **kwargs
        )
        dhasa_bhukti_info = []
        for i in range(len(db)):
            ...
            dhasa_bhukti_info.append(
                (
                    utils.NAKSHATRA_LIST[dhasa_lord]
                    + "-"
                    + utils.NAKSHATRA_LIST[bukthi_lord],
                    bukthi_start,
                )
            )
        return dhasa_bhukti_info
    ...
    def _get_narayana_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = narayana.narayana_dhasa_for_rasi_chart(
    ...
    def _get_kendraadhi_rasi_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = kendradhi_rasi.kendradhi_rasi_dhasa(
    ...
    def _get_nirayana_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = nirayana.nirayana_shoola_dhasa_bhukthi(
    ...
    def _get_kendraadhi_karaka_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = kendradhi_rasi.karaka_kendradhi_rasi_dhasa(
            dob,
            tob,
            place,
            divisional_chart_factor=divisional_chart_factor,
    ...
    def _get_lagnamsaka_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = lagnamsaka.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_padhanadhamsa_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = padhanadhamsa.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_mandooka_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = mandooka.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_sthira_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = sthira.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_tara_lagna_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = tara_lagna.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_brahma_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = brahma.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_varnada_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = varnada.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_yogardha_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = yogardha.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_navamsa_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = navamsa.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
        dhasa_bhukti_info = []  # {}
        for i in range(len(db)):
    ...
    def _get_paryaaya_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = paryaaya.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
    ...
    def _get_trikona_dhasa(self, dob, tob, place, divisional_chart_factor=1):
        ...
        db = trikona.get_dhasa_antardhasa(
            dob, tob, place, divisional_chart_factor=divisional_chart_factor
        )
    ...
    def get_chara_karakas_for_chart(
        self,
        jd_at_dob,
        place,
        divisional_chart_factor=1,
        ...
        for ki, k in enumerate(_karakas):
            ...
            _vl_chart[h] = (
                self.cal_key_list[const.chara_karaka_names[ki] + "_short_str"]
                + "\n"
                + _vl_chart[h]
            )
    ...
    def get_chara_karakas_for_mixed_chart(
        self,
        jd_at_dob,
        place,
        varga_factor_1=None,
    ...
    def get_special_lagnas_for_chart(
        self,
        jd_at_dob,
        place,
        divisional_chart_factor=1,
        ...
        for spl in spl_list:
            if spl == "varnada_lagna":
                vl = eval(
                    "charts."
                    + spl
                    + "(dob,tob,place,divisional_chart_factor=divisional_chart_factor,chart_method=chart_method,base_rasi=base_rasi,count_from_end_of_sign=count_from_end_of_sign)"
                )
            else:
                vl = eval(
                    "drik."
                    + spl
                    + "(jd_at_dob,place,divisional_chart_factor=divisional_chart_factor,chart_method=chart_method,base_rasi=base_rasi,count_from_end_of_sign=count_from_end_of_sign)"
                )
    ...
    def get_special_lagnas_for_mixed_chart(
        self,
        jd_at_dob,
        place,
        varga_factor_1=None,
        ...
        for spl in spl_list:
            if spl == "varnada_lagna":
                vl = eval(
                    "charts."
                    + spl
                    + "_mixed_chart(dob,tob, place, varga_factor_1=varga_factor_1, chart_method_1=chart_method_1, varga_factor_2=varga_factor_2, chart_method_2=chart_method_2)"
                )
            else:
                vl = eval(
                    "drik."
                    + spl
                    + "_mixed_chart(jd_at_dob, place, varga_factor_1=varga_factor_1, chart_method_1=chart_method_1, varga_factor_2=varga_factor_2, chart_method_2=chart_method_2)"
                )
            _vl_chart[vl[0]] += self.cal_key_list[spl + "_short_str"] + "\n"
        _special_lagna_dict = {self.cal_key_list["special_lagnas_str"]: _vl_chart}
        return _special_lagna_dict

    def get_sphutas_for_chart(
        self,
        jd_at_dob,
        place,
        divisional_chart_factor=1,
    ...
    def get_sphutas_for_mixed_chart(
        self,
        jd_at_dob,
        place,
        varga_factor_1=None,
    ...
    def get_ava_saha_yoga_info_for_chart(
        self,
        jd_at_dob,
        place,
        divisional_chart_factor=1,
        ...
        ystr = (
            self.cal_key_list["yogi_sphuta_str"]
            + " "
            + self.cal_key_list["raasi_str"]
            + ":"
            + utils.RAASI_LIST[yh]
            + " "
            + self.cal_key_list["longitude_str"]
            + ":"
            + utils.to_dms(yl, is_lat_long="plong")
        )
        ...
        ystr += (
            ...
            + self.cal_key_list["yogi_sphuta_str"]
            + " "
            + self.cal_key_list["nakshatra_str"]
            + ":"
            + utils.NAKSHATRA_LIST[ynak[0] - 1]
        )
        yogi_planet = [
            l for l, naks in const.nakshathra_lords.items() if ynak[0] - 1 in naks
        ][0]
        ystr += (
            line_sep
            + self.cal_key_list["yogi_sphuta_str"]
            + " "
            + self.cal_key_list["planet_str"]
            + ":"
        ...
        ystr += (
            line_sep
            + self.cal_key_list["sahayogi_str"]
            + " "
            + self.cal_key_list["planet_str"]
            + ":"
            + utils.PLANET_NAMES[sahayogi_planet]
        )
        yh, yl = sphuta.avayogi_sphuta(
            dob,
            tob,
            place,
            divisional_chart_factor=divisional_chart_factor,
        ...
        ystr += (
            line_sep
            + self.cal_key_list["avayogi_sphuta_str"]
            + " "
            + self.cal_key_list["raasi_str"]
            + ":"
            + utils.RAASI_LIST[yh]
    ...
    def get_ava_saha_yoga_for_mixed_chart(
        self,
        jd_at_dob,
        place,
        varga_factor_1=None,
        ...
        yh, yl = sphuta.yogi_sphuta_mixed_chart(
            dob,
            tob,
            place,
            varga_factor_1=varga_factor_1,
        ...
        yh, yl = sphuta.avayogi_sphuta_mixed_chart(
            dob,
            tob,
            place,
            varga_factor_1=varga_factor_1,
    ...
    def get_sahams(self, planet_positions):
        ...
        for sah in const._saham_list:
            ...
            key = (
                self.cal_key_list[sah + "_saham_str"]
                + " "
                + self.cal_key_list["saham_str"]
            )
            value = (
                utils.RAASI_LIST[vl[0]] + " " + utils.to_dms(vl[1], is_lat_long="plong")
            )
...
def get_chara_karakas(
    ...
    rasi_planet_positions = charts.rasi_chart(
        jd,
        place,
        ayanamsa_mode,
        years,
...
if __name__ == "__main__":
    ...
    jd_at_dob = utils.julian_day_number(dob, tob)
    a = Horoscope(
        place_with_country_code=place.Place,
        latitude=place.latitude,
        longitude=place.longitude,
        timezone_offset=place.timezone,
        date_in=drik.Date(dob[0], dob[1], dob[2]),
        birth_time="10:34:00",
    )

    horo_info, chart_info, asc_info = a.get_horoscope_information_for_chart(
    ...
    print(
        "yogi info chart",
        a.get_ava_saha_yoga_info_for_chart(
            jd_at_dob, place, dcf, chart_method, base_rasi, count_from_end_of_sign
        ),
    )
    exit()
    chart_index_1 = 8
    ...
    chart_index_2 = 12
    chart_method_2 = 1
    horo_info, chart_info, asc_info = a.get_horoscope_information_for_mixed_chart(
    ...
    print(
        "aspects of ",
        utils.PLANET_NAMES[planet],
        house.aspected_planets_of_the_raasi(h_to_p, p_to_h[planet]),
    )
    # exit()
    as_string = True
    a = Horoscope(
