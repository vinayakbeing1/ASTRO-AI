...
divisional_chart_functions = {
    2: "hora_chart",
    3: "drekkana_chart",
    4: "chaturthamsa_chart",
    5: "panchamsa_chart",
    6: "shashthamsa_chart",
    7: "saptamsa_chart",
    8: "ashtamsa_chart",
    ...
    16: "shodasamsa_chart",
    20: "vimsamsa_chart",
    24: "chaturvimsamsa_chart",
    27: "nakshatramsa_chart",
    30: "trimsamsa_chart",
    40: "khavedamsa_chart",
    45: "akshavedamsa_chart",
    60: "shashtyamsa_chart",
    81: "nava_navamsa_chart",
    108: "ashtotharamsa_chart",
    144: "dwadas_dwadasamsa_chart",
    # 150:'nadiamsa_chart'
}
...
def rasi_chart(
    ...
    jd_years = (
        ...
        else drik.next_solar_date(jd_at_dob, place_as_tuple, years, months, sixty_hours)
    )
    if pravesha_type == 2:
    ...
    ascendant_constellation, ascendant_longitude, _, _ = drik.ascendant(
        jd_years, place_as_tuple
    )
    ...
    planet_positions = drik.dhasavarga(
        jd_years, place_as_tuple, divisional_chart_factor=1
    )
    # print('planet_positions\n',planet_positions)
    planet_positions = [
        [ascendant_index, (ascendant_constellation, ascendant_longitude)]
    ] + planet_positions
...
def bhava_houses(
    ...
    place,
    ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE,
    bhava_starts_with_ascendant=False,
):
    bp = bhava_chart_houses(
        jd,
        place,
        ayanamsa_mode,
        bhava_starts_with_ascendant=bhava_starts_with_ascendant,
    )
    bp = {
        p: house.get_relative_house_of_planet(bp[const._ascendant_symbol][0], h)
        for p, (h, _) in bp.items()
    }
    return bp
...
def _bhaava_madhya_new(
    jd, place, planet_positions, bhava_madhya_method=const.bhaava_madhya_method
):
    """
    ...
    ascendant_constellation, ascendant_longitude = (
        planet_positions[0][1][0],
        planet_positions[0][1][1],
    )
    ...
    if bhava_madhya_method == 1:  # Equal Housing - Lagna in the middle
        ...
        return drik._assign_planets_to_houses(
        ...
        bm = drik.bhaava_madhya_sripathi(jd, place)
        bm = bm[:] + [bm[0]]
        for h in range(12):
            _bhava_start = bm[h]
        ...
        bm = (
            drik.bhaava_madhya_kp(jd, place)
            if bhava_madhya_method == 4
            else drik.bhaava_madhya_swe(jd, place, house_code=bhava_madhya_method)
        )
        bm = bm[:] + [bm[0]]
        for h in range(12):
            bmh = bm[h]
...
def _hora_chart_kashinath(planet_positions_in_rasi):
    ...
    for planet, [rasi_sign, long] in planet_positions_in_rasi:
        d_long = (long * dvf) % 30
        hora = int(long // 15.0)
        lord_of_rasi = house.house_owner_from_planet_positions(
            planet_positions_in_rasi, rasi_sign
        )
        if rasi_sign in const.odd_signs:
            hora_sign = (
                planet_hora[lord_of_rasi][0]
                if hora == 0
                else planet_hora[lord_of_rasi][1]
            )
        else:
            hora_sign = (
                planet_hora[lord_of_rasi][1]
                if hora == 0
                else planet_hora[lord_of_rasi][0]
            )
        dp.append([planet, [hora_sign, d_long]])
    return dp
...
def _hora_traditional_parasara_chart(planet_positions_in_rasi):
    ...
    for planet, [sign, long] in planet_positions_in_rasi:
        ...
        l = int(long // 15.0)
        r = 3  # Moon's hora
        if (sign in const.odd_signs and l == 0) or (
            sign in const.even_signs and l == 1
        ):
            r = 4  # Sun's Hora
...
def __parivritti_alternate(planet_positions_in_rasi, dvf, dirn=1):
    f1 = 30.0 / dvf
    _hora_list = utils.parivritti_alternate(dvf, dirn)
    dp = []
    for planet, [rasi_sign, long] in planet_positions_in_rasi:
        d_long = (long * dvf) % 30
        hora = int(long // f1)
...
def navamsa_chart(planet_positions_in_rasi, chart_method=1):
    ...
    # Traditional Parasara Method
    navamsa_dict = {
        0: (1, const.fire_signs),
        3: (1, const.water_signs),
        6: (1, const.air_signs),
        9: (1, const.earth_signs),
    }
    if chart_method == 4:
        navamsa_dict = {
            0: (1, const.fire_signs),
...
def trimsamsa_chart(planet_positions_in_rasi, chart_method=1):
    ...
    if chart_method == 2:
        ...
        return [
            [planet, [(int(long // f1) + sign) % 12, (long * dvf) % 30]]
            for planet, [sign, long] in planet_positions_in_rasi
        ]
    # Traditional Parasara Method
    ...
    for planet, [sign, long] in planet_positions_in_rasi:
        ...
        if sign in const.odd_signs:
            r = [
                rasi % 12
                for (l_min, l_max, rasi) in odd
                if (long >= l_min and long <= l_max)
            ]
        else:
            r = [
                rasi % 12
                for (l_min, l_max, rasi) in even
                if (long >= l_min and long <= l_max)
            ]
...
def ashtotharamsa_chart(planet_positions_in_rasi, chart_method=1):
    ...
    dvf_2 = 12
    chart_method_2 = 1
    pp = mixed_chart_from_rasi_positions(
        planet_positions_in_rasi,
        varga_factor_1=dvf_1,
        chart_method_1=chart_method_1,
        varga_factor_2=dvf_2,
...
def nadiamsa_chart(planet_positions_in_rasi, chart_method=1):
    ...
    return divisional_positions_from_rasi_positions(
        planet_positions_in_rasi, divisional_chart_factor=150, chart_method=chart_method
    )


def custom_divisional_chart(
    planet_positions_in_rasi,
    ...
    @return: planet_positions list in the format [[planet,(raasi,planet_longitude)],...]]
        ...
        Example: [ ['L',(0,13.4)],[0,(11,12.7)],...]] Lagnam in Aries 13.4 degrees, Sun in Taurus 12.7 degrees
    """
    dvf = divisional_chart_factor
    f1 = 30.0 / dvf
    if base_rasi == None:
        return __parivritti_cyclic(planet_positions_in_rasi, dvf, dirn=1)
    _hora_list = utils.__varga_non_cyclic(
        dvf,
        base_rasi=base_rasi,
        start_sign_variation=chart_method,
...
def mixed_chart(
    ...
    if varga_factor_1 == 1 and varga_factor_2 == 1:
        return planet_positions_in_rasi
    pp1 = (
        planet_positions_in_rasi
        if varga_factor_1 == 1
        else eval(
            divisional_chart_functions[varga_factor_1]
            + "(planet_positions_in_rasi,chart_method=chart_method_1)"
        )
    )
    pp2 = (
        pp1
        if varga_factor_2 == 2
            ...
            divisional_chart_functions[varga_factor_2]
            + "(pp1,chart_method=chart_method_2)"
        )
    )
    return pp2
...
def mixed_chart_from_rasi_positions(
    ...
    pp2 = eval(
        divisional_chart_functions[varga_factor_2] + "(pp1,chart_method=chart_method_2)"
    )
    return pp2


def divisional_positions_from_rasi_positions(
    planet_positions_in_rasi,
    ...
    if divisional_chart_factor == 1:
        ...
        if (not const.TREAT_STANDARD_CHART_AS_CUSTOM) and (
            divisional_chart_factor in divisional_chart_functions.keys()
            and (base_rasi == None and (chart_method != None and chart_method > 0))
        ):
            return eval(
                divisional_chart_functions[divisional_chart_factor]
                + "(planet_positions_in_rasi,chart_method)"
            )
        elif divisional_chart_factor in range(1, const.MAX_DHASAVARGA_FACTOR + 1):
            return custom_divisional_chart(
                planet_positions_in_rasi,
...
def _planets_in_retrograde_old(planet_positions):
    ...
    for p, (h, p_long) in planet_positions[
        3:8
    ]:  # Exclude Lagna, Sun,Moon,, Rahu and Ketu
        ...
        if p == 2:
            ...
            if house.get_relative_house_of_planet(sun_house, planet_house) in [
                *range(6, 9)
            ]:
...
def planets_in_retrograde(planet_positions):
    ...
    for p, (h, p_long) in planet_positions[
        ...
        planet_long = h * 30 + p_long
        p_long_from_sun_1 = (
            sun_long + 360 + const.planets_retrograde_limits_from_sun[p][0]
        ) % 360
        p_long_from_sun_2 = (
            sun_long + 360 + const.planets_retrograde_limits_from_sun[p][1]
        ) % 360
        if p_long_from_sun_2 < p_long_from_sun_1:
            p_long_from_sun_2 += 360.0
...
def planets_in_combustion(planet_positions, use_absolute_longitude=True):
    ...
    sun_long = (
        planet_positions[1][1][0] * 30 + planet_positions[1][1][1]
        if use_absolute_longitude
        else planet_positions[1][1][1]
    )
    combustion_planets = []
    for p, (h, h_long) in planet_positions[2:8]:  # Exclude Lagna, Sun, Rahu and Ketu
        ...
        if p in retrograde_planets:
            combustion_range = (
                const.combustion_range_of_planets_from_sun_while_in_retrogade
            )
        if (
            p_long >= sun_long - combustion_range[p - 2]
            and p_long <= sun_long + combustion_range[p - 2]
        ):
            combustion_planets.append(p)
    return combustion_planets


def vaiseshikamsa_dhasavarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    ...
    @return count for each planet - list - Example [3,4,5,6..] Sun in its own house in 3 charts, moon in 4 charts and so on.
        ...
        Sreedhaamaamsa – 10.
    """
    return _vaiseshikamsa_bala_of_planets(
        jd_at_dob, place_as_tuple, ayanamsa_mode, const.dhasavarga_amsa_vaiseshikamsa
    )


def vaiseshikamsa_shadvarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    ...
    """
    return _vaiseshikamsa_bala_of_planets(
        jd_at_dob, place_as_tuple, ayanamsa_mode, const.shadvarga_amsa_vaiseshikamsa
    )


def vaiseshikamsa_sapthavarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    ...
    """
    return _vaiseshikamsa_bala_of_planets(
        jd_at_dob, place_as_tuple, ayanamsa_mode, const.sapthavarga_amsa_vaiseshikamsa
    )


def vaiseshikamsa_shodhasavarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    ...
    return _vaiseshikamsa_bala_of_planets(
        ...
        place_as_tuple,
        ayanamsa_mode,
        const.shodhasa_varga_amsa_vaiseshikamsa,
    )


def _vaiseshikamsa_bala_of_planets(
    jd_at_dob,
    place_as_tuple,
    ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE,
    ...
    for dcf in amsa_vaiseshikamsa.keys():
        planet_positions = divisional_chart(
            jd_at_dob, place_as_tuple, ayanamsa_mode, divisional_chart_factor=dcf
        )[: const._pp_count_upto_ketu]
        for p, (h, _) in planet_positions:
            if p == const._ascendant_symbol:
                continue
            elif (
                h == const.moola_trikona_of_planets[p]
                or const.house_strengths_of_planets[p][h] > const._FRIEND
            ):
                p_d[p] += 1
                p_d_c[p] += "D" + str(dcf) + "/"
...
def _vimsopaka_bala_of_planets(
    jd_at_dob,
    place_as_tuple,
    ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE,
    amsa_vimsopaka=None,
...
def vimsopaka_dhasavarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    Get the count - in how many dhasa varga charts the planets are in their own raasi or exalted
    ...
    @return count for each planet - list - Example [3,4,5,6..] Sun in its own house in 3 charts, moon in 4 charts and so on.
        ...
        Sreedhaamaamsa – 10.
    """
    return _vimsopaka_bala_of_planets(
        jd_at_dob, place_as_tuple, ayanamsa_mode, const.dhasavarga_amsa_vimsopaka
    )


def vimsopaka_shadvarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    Get the count - in how many shad varga charts the planets are in their own raasi or exalted
    ...
    """
    return _vimsopaka_bala_of_planets(
        jd_at_dob, place_as_tuple, ayanamsa_mode, const.shadvarga_amsa_vimsopaka
    )


def vimsopaka_sapthavarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    Get the count - in how many saptha varga charts the planets are in their own raasi or exalted
    ...
    """
    return _vimsopaka_bala_of_planets(
        jd_at_dob, place_as_tuple, ayanamsa_mode, const.sapthavarga_amsa_vimsopaka
    )


def vimsopaka_shodhasavarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
    Get the count - in how many shodhasa varga charts the planets are in their own raasi or exalted
    ...
    """
    return _vimsopaka_bala_of_planets(
        jd_at_dob, place_as_tuple, ayanamsa_mode, const.shodhasa_varga_amsa_vimsopaka
    )


def vimsamsavarga_of_planets(
    jd_at_dob, place_as_tuple, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
):
    """
...
def _varnada_lagna_sanjay_rath_mixed_chart(
    dob,
    tob,
    place,
    house_index=1,
    ...
    planet_positions = mixed_chart(
        jd_at_dob, place, varga_factor_1, chart_method_1, varga_factor_2, chart_method_2
    )
    asc_sign = planet_positions[0][1][0]
    asc_long = planet_positions[0][1][1]
    asc_sign = (asc_sign + house_index - 1) % 12
    asc_long = asc_sign * 30 + asc_long
    hora_sign, hora_long = drik.hora_lagna_mixed_chart(
        jd_at_dob, place, varga_factor_1, chart_method_1, varga_factor_2, chart_method_2
    )
    ...
    if not asc_is_odd:
        asc_long = 360.0 - asc_long
    if _debug_:
        print(asc_sign, utils.RAASI_LIST[asc_sign], asc_is_odd, asc_long)
    hora_is_odd = hora_sign in const.odd_signs
    if not hora_is_odd:
        hora_long = 360.0 - hora_long
    if _debug_:
        print(hora_sign, utils.RAASI_LIST[hora_sign], hora_is_odd, hora_long)
    if hora_is_odd == asc_is_odd:
        vl = (asc_long + hora_long) % 360
        if _debug_:
            print("adding", asc_long, hora_long, vl)
    else:
        vl = (max(asc_long, hora_long) - min(asc_long, hora_long)) % 360
        if _debug_:
            print("subtracting", asc_long, hora_long, vl)
    if _debug_:
        print(
            "asc_sign",
            asc_sign,
    ...
    if _debug_:
        print("return drik dasavarg", dl)
    return dl


def _varnada_lagna_sanjay_rath(
    dob,
    tob,
    place,
    house_index=1,
    ...
    asc_sign = (asc_sign + house_index - 1) % 12
    asc_long = asc_sign * 30 + asc_long
    hora_sign, hora_long = drik.hora_lagna(
        jd_at_dob,
        place,
        ayanamsa_mode=ayanamsa_mode,
        divisional_chart_factor=divisional_chart_factor,
...
def _varnada_lagna_jha_pandey_mixed_chart(
    dob,
    tob,
    place,
    house_index=1,
    ...
    if not lagna_is_odd:
        asc_long = 360.0 - asc_long
    count1 = (
        utils.count_rasis(0, lagna, dir=1)
        if lagna_is_odd
        else utils.count_rasis(11, lagna, dir=-1)
    )
    hora_sign, hora_long = drik.hora_lagna_mixed_chart(
        jd_at_dob, place, varga_factor_1, chart_method_1, varga_factor_2, chart_method_2
    )
    ...
    if not lagna_is_odd:
        hora_long = 360.0 - hora_long
    count2 = (
        utils.count_rasis(0, hora_lagna, dir=1)
        if hora_lagna_is_odd
        else utils.count_rasis(11, hora_lagna, dir=-1)
    )
    count = (
        (count1 + count2) % 12
    ...
    vl = (
        (asc_long + hora_long) % 360
        if count_is_odd
        else (max(asc_long, hora_long) - min(asc_long, hora_long)) % 360
    )
...
def varnada_lagna_mixed_chart(
    ...
    """
    if varnada_method == 1:
        return _varnada_lagna_bv_raman_mixed_chart(
            dob,
            tob,
            place,
            house_index=house_index,
            varga_factor_1=varga_factor_1,
            chart_method_1=chart_method_2,
            varga_factor_2=varga_factor_2,
            chart_method_2=chart_method_2,
        )
    elif varnada_method == 2:
        return _varnada_lagna_sharma_mixed_chart(
            dob,
            tob,
            place,
            house_index=house_index,
        ...
        return _varnada_lagna_sanjay_rath_mixed_chart(
            dob,
            tob,
            place,
            house_index=house_index,
        ...
        return _varnada_lagna_jha_pandey_mixed_chart(
            dob,
            tob,
            place,
            house_index=house_index,
...
def varnada_lagna(
    ...
    """
    if varnada_method == 1:
        return _varnada_lagna_bv_raman(
            dob,
            tob,
            place,
            house_index,
        ...
        return _varnada_lagna_sanjay_rath(
            dob,
            tob,
            place,
            house_index,
        ...
        return _varnada_lagna_jha_pandey(
            dob,
            tob,
            place,
            house_index,
...
def _varnada_lagna_bv_raman_mixed_chart(
    dob,
    tob,
    place,
    house_index=1,
    ...
    lagna = (planet_positions[0][1][0] + house_index - 1) % 12
    asc_long = planet_positions[0][1][1]
    lagna_is_odd = lagna in const.odd_signs
    count1 = (
        utils.count_rasis(0, lagna, dir=1)
    ...
    hora_lagna, _ = drik.hora_lagna_mixed_chart(
        jd_at_dob, place, varga_factor_1, chart_method_1, varga_factor_2, chart_method_2
    )
    ...
    count = (
        (count1 + count2) % 12
        if hora_lagna_is_odd == lagna_is_odd
        else (max(count1, count2) - min(count1, count2)) % 12
    )
    _varnada_lagna = (
        utils.count_rasis(1, count, dir=1)
        if lagna_is_odd
        else utils.count_rasis(12, count, dir=-1)
    )
    _varnada_lagna -= 1  ## Keep in 0..11 range instead of 1..12
    return _varnada_lagna, asc_long  # hl


def _varnada_lagna_bv_raman(
    dob,
    tob,
    place,
    house_index=1,
...
def _varnada_lagna_santhanam_mixed_chart(
    dob,
    tob,
    place,
    house_index=1,
...
def _varnada_lagna_sharma_mixed_chart(
    dob,
    tob,
    place,
    house_index=1,
...
def benefics_and_malefics(
    ...
    malefics = (
        const.natural_malefics[:-2] if exclude_rahu_ketu else const.natural_malefics[:]
    )
    ...
    if method == 2:
        ...
        if _tithi >= 8 and _tithi <= 15:
            benefics.append(1)
        if _tithi >= 23 and _tithi <= 30:
            malefics.append(1)
    planet_positions = divisional_chart(
        jd,
        place,
    ...
    mars_malefics = [
        p
        for p in malefics
        if planet_positions[p + 1][1][0] == planet_positions[4][1][0]
    ]
    ...
    if (
        mars_benefics_count == 0
        and mars_malefics_count == 0
        or mars_benefics_count > mars_malefics_count
    ):
        ...
        mercury_house, mercury_long = next(
            (h, l) for p, (h, l) in planet_positions if p == 3
        )
        planet_closest_to_mars = min(
            [p for p, (h, l) in planet_positions if h == mercury_house and p != 3],
            key=lambda p: abs(
                next(l for x, (h, l) in planet_positions if x == p) - mercury_long
            ),
            default=None,
        )
        ...
        if planet_closest_to_mars in benefics:
            ...
            malefics += [3]
    benefics = sorted(set(benefics))
    malefics = sorted(set(malefics))
...
def benefics(
    ...
    In addition If Mars is associated with both malefics and benefics
        ...
        (ii) if count is same one nearer to Mars in longitude decides
    """
    return benefics_and_malefics(
        jd,
        place,
        method=method,
        ayanamsa_mode=ayanamsa_mode,
        exclude_rahu_ketu=exclude_rahu_ketu,
    )[0]
...
def order_planets_from_kendras_of_raasi(
    planet_positions, raasi=None, include_lagna=False
):
    base_house = raasi
    if raasi == None:
        base_house = planet_positions[0][1][0]
    ks = sum(house.kendras()[:3], [])
    h_to_p = utils.get_house_planet_list_from_planet_positions(planet_positions)
    hp1 = [h_to_p[(base_house + h - 1) % 12] for h in ks]
    kps = []
    for pl in hp1:
        if not include_lagna and pl == "L":
            continue
        if pl == "":
            continue
        pl2 = sorted(
            [(p, long) for p, (_, long) in planet_positions if str(p) in pl],
            key=lambda x: x[1],
            reverse=True,
        )
        # print(pl2)
...
def _stronger_planet_from_the_list(planet_positions, planet_list):
    def _compare(planet1, planet2):
        return (
            1
            if house.stronger_planet_from_planet_positions(
                planet_positions, planet1, planet2
            )
            == planet1
            else -1
        )
...
def _order_stronger_planets(planet_positions, reverse=False):
    ...
    planet_list.sort(key=cmp_to_key(_compare))
    if reverse:
        planet_list = list(reversed(planet_list))
    return planet_list


def special_planet_longitudes_mixed_chart(
    dob,
    tob,
    place,
    varga_factor_1=1,
    ...
    if varga_factor_1 == 1 and varga_factor_2 == 1:
        return spl_planet_positions_in_rasi
    pp1 = (
        spl_planet_positions_in_rasi
        if varga_factor_1 == 1
        else eval(
            divisional_chart_functions[varga_factor_1]
            + "(spl_planet_positions_in_rasi,chart_method=chart_method_1)"
        )
    )
    pp2 = (
        pp1
...
def special_planet_longitudes(
    ...
    sub_planet_list_1 = {
        "Kl": "kaala_longitude",
        "Mr": "mrityu_longitude",
        "Ap": "artha_praharaka_longitude",
        "Yg": "yama_ghantaka_longitude",
        "Gk": "gulika_longitude",
        "Md": "maandi_longitude",
    }
    spl_rasi_positions = []
    ...
    for sp, sp_func in sub_planet_list_2.items():
        eval_str = (
            "drik.solar_upagraha_longitudes(sun_long,upagraha='" + str(sp_func) + "')"
        )
        v = eval(eval_str)
        spl_rasi_positions.append([sp, [v[0], v[1]]])
    # """
    ...
    if (not const.TREAT_STANDARD_CHART_AS_CUSTOM) and (
        ...
        return eval(
            divisional_chart_functions[divisional_chart_factor]
            + "(spl_rasi_positions,chart_method)"
        )
    elif divisional_chart_factor in range(1, const.MAX_DHASAVARGA_FACTOR + 1):
        return custom_divisional_chart(
            spl_rasi_positions,
...
def solar_upagraha_longitudes(planet_positions, upagraha, divisional_chart_factor=1):
    ...
    return drik.solar_upagraha_longitudes(
        solar_longitude, upagraha, divisional_chart_factor=divisional_chart_factor
    )
...
def _amsa(
    ...
    dob = drik.Date(y, m, d)
    tob = (fh, 0, 0)
    div_planet_positions = divisional_chart(
        jd,
        place,
        ayanamsa_mode=ayanamsa_mode,
        divisional_chart_factor=divisional_chart_factor,
    ...
    if include_special_lagnas:
        special_lagna_dict = {
            "bhava_lagna_str": 0.25,
            "hora_lagna_str": 0.5,
            "ghati_lagna_str": 1.25,
            "pranapada_lagna_str": 5.0,
            "vighati_lagna_str": 15.0,
        }  # Bhava,hora,ghati,pranapada,vighati
        for sl, lf in special_lagna_dict.items():
            sf = drik.special_ascendant(
                jd,
                place,
                ayanamsa_mode=ayanamsa_mode,
                divisional_chart_factor=divisional_chart_factor,
        ...
        __amsa_special["bhrigu_bindhu_lagna_str"] = _get_amsa_index_from_longitude(
            bl[1]
        )
        kl = drik.kunda_lagna(
            jd,
    ...
    if include_upagrahas:
        ...
        for sp, sp_func in sub_planet_list_1.items():
            v = eval(
                "drik."
                + sp_func
                + "(dob,tob,place,ayanamsa_mode=ayanamsa_mode,divisional_chart_factor=divisional_chart_factor)"
            )
            __amsa_upagraha[sp] = _get_amsa_index_from_longitude(v[1])
        for sp in sub_planet_list_2:
            v = eval(
                "solar_upagraha_longitudes(div_planet_positions,sp,divisional_chart_factor=divisional_chart_factor)"
            )
    ...
    if include_sphutas:
        ...
        for s in const.sphuta_list:
            fn = (
                "sphuta."
                + s
                + "_sphuta(dob,tob,place,divisional_chart_factor=divisional_chart_factor)"
            )
            sp = eval(fn)
...
def get_pachakadi_sambhandha(planet_positions):
    prd = {
        planet: [
            (
                planet_positions[_pre[0] + 1][1][0]
                == (planet_positions[planet + 1][1][0] + _pre[1] - 1) % 12,
                _pre[2],
            )
            for _pre in _pr
        ]
        for planet, _pr in const.paachakaadi_sambhandha.items()
    }
    ...
    pachakadi_relation_dict = {
        ...
        for index, (flag, char) in enumerate(value)
        if flag
    }
    # """
...
def planets_in_pushkara_navamsa_bhaga(planet_positions):
    pna = [
        planet
        for planet, (sign, long) in planet_positions[1 : const._pp_count_upto_ketu]
        if (
            long >= const.pushkara_navamsa[sign]
            and long < const.pushkara_navamsa[sign] + (30 / 9)
        )
        or (
            long >= const.pushkara_navamsa[sign] + 60 / 9
            and long < const.pushkara_navamsa[sign] + 10
        )
    ]
    ...
    pb = [
        planet
        for planet, (sign, long) in planet_positions[1 : const._pp_count_upto_ketu]
        if long >= const.pushkara_bhagas[sign] - 1
        and long < const.pushkara_bhagas[sign]
    ]
    # for planet, (sign,long) in planet_positions:
...
def planets_in_mrityu_bhaga(dob, tob, place, planet_positions):
    ...
    planet_positions = planet_positions[: const._pp_count_upto_ketu] + [
        ["Md", drik.maandi_longitude(dob, tob, place)]
    ]
    ...
    def compare_planet_positions(planet_positions):
        ...
        for planet, (rasi, longitude) in planet_positions:
            ...
            tolerance = (
                const.mrityu_bhaga_tolerances[planet]
                if planet in ["Md", "L"]
                else const.mrityu_bhaga_tolerances[planet_index]
            )
...
def get_planets_in_marana_karaka_sthana(planet_positions, consider_ketu_4th_house=True):
    ...
    p_end = (
        const._pp_count_upto_ketu
        if consider_ketu_4th_house
        else const._pp_count_upto_rahu
    )
...
def previous_planet_entry_date_divisional_chart(
    jd,
    place,
    planet,
    divisional_chart_factor=1,
    ...
    return next_planet_entry_date_divisional_chart(
        jd,
        place,
        planet,
        divisional_chart_factor=divisional_chart_factor,
...
def next_planet_entry_date_divisional_chart(
    jd,
    place,
    planet,
    divisional_chart_factor=1,
    ...
    if planet == 8:
        ...
        ret = next_planet_entry_date_divisional_chart(
            jd,
            place,
            7,
            divisional_chart_factor=divisional_chart_factor,
    ...
    increment_days = (
        ...
        if planet in ["L", 1]
        else 0.1 / divisional_chart_factor
    )
    planet_index = 0 if planet == "L" else planet + 1
    sla = divisional_chart(
        jd,
        place,
        divisional_chart_factor=divisional_chart_factor,
        chart_method=chart_method,
    ...
    offsets = (
        [t * 0.25 for t in range(-5, 5)]
        if planet != "L"
        else [t * increment_days for t in range(-5, 5)]
    )
    planet_longs = []
    for t in offsets:
        sla = divisional_chart(
...
def previous_planet_entry_date_mixed_chart(
    jd,
    place,
    planet,
    varga_factor_1=None,
    ...
    return next_planet_entry_date_mixed_chart(
        jd,
        place,
        planet,
        varga_factor_1=varga_factor_1,
...
def next_planet_entry_date_mixed_chart(
    jd,
    place,
    planet,
    varga_factor_1=None,
...
def next_conjunction_of_planet_pair_divisional_chart(
    jd,
    place: drik.Place,
    p1,
    p2,
    ...
    _planet_speeds = [361] + [
        abs(psi[3]) for p, psi in drik.planets_speed_info(jd, place).items()
    ]
    ...
    if (p1 == 7 and p2 == 8) or (p1 == 8 and p2 == 7):
        ...
        return None
    pi1 = 0 if p1 == "L" else p1 + 1
    pi2 = 0 if p2 == "L" else p2 + 1
    ...
    while search_counter < max_days_to_search:
        ...
        if long_diff < long_diff_check:
            ...
            try:
                ...
                conj_jd = drik.__next_conjunction_of_planet_pair(
                    cur_jd, place, p1, p2, direction, separation_angle
                )
                if conj_jd != None:
                    sla = divisional_chart(
                        conj_jd,
...
def lattha_stars_planets(planet_positions, include_abhijith=True):
    ...
    for p, (h, long) in planet_positions[1 : const._pp_count_upto_ketu]:
        ...
        _latta_star = utils.cyclic_count_of_stars_with_abhijit(
            p_star,
            const.latta_stars_of_planets[p][0],
            const.latta_stars_of_planets[p][1],
            star_count,
        )
...
if __name__ == "__main__":
    ...
    print(lp)
    _star_list = (
        [utils.NAKSHATRA_LIST[s] for s in const.abhijit_order_of_stars]
        if include_abhijith
        else utils.NAKSHATRA_LIST
    )
    ...
    dt = (
        (360 + p2_long - p1_long) / (p1_speed - p2_speed)
        if p1_speed > p2_speed
        else (360 + p1_long - p2_long) / (p2_speed - p1_speed)
    )
    ...
    for planet in ["L"] + [*range(9)]:
        start_time = time.time()
        nae = next_planet_entry_date_divisional_chart(
            jd, place, planet, dcf, 1, chart_method, base_rasi, count_from_end_of_sign
        )
        ...
        nae = previous_planet_entry_date_divisional_chart(
            jd, place, planet, dcf, chart_method, base_rasi, count_from_end_of_sign
        )
        _, _, _, fhn = utils.jd_to_gregorian(nae[0])
        print(
            utils.jd_to_gregorian(nae[0]),
