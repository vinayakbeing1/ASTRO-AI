...
get_relative_house_of_planet = (
    lambda from_house, planet_house: (planet_house + 12 - from_house) % 12 + 1
)
strong_signs_of_planet = lambda planet, strength=const._FRIEND: [
    h for h in range(12) if const.house_strengths_of_planets[planet][h] == strength
]
""" Get All trikona aspects of the given raasi"""
trikona_aspects_of_the_raasi = lambda raasi: [
    (raasi) % 12,
    (raasi + 4) % 12,
    (raasi + 8) % 12,
]
...
functional_malefic_lord_houses = lambda asc_house: [
    (asc_house + 2) % 12,
    (asc_house + 5) % 12,
    (asc_house + 10) % 12,
]
functional_neutral_lord_houses = lambda asc_house: [
    (asc_house + 1) % 12,
    (asc_house + 7) % 12,
    (asc_house + 11) % 12,
]
...
def is_yoga_kaaraka(asc_house, planet, planet_house):
    ...
    """
    return (
        planet_house in quadrants_of_the_raasi(asc_house)
        and planet_house in trines_of_the_raasi(asc_house)
        and const.house_strengths_of_planets[planet][planet_house] == 5
    )
...
def trikonas():
    ...
    for house in range(12):
        trik = [
            house,
            trikona_aspects_of_the_raasi(house),
        ]  # [house,[(house)%12, (house+4)%12, (house+8)%12]]
...
dushthana_aspects_of_the_raasi = lambda raasi: [
    (raasi) % 12,
    (raasi + 2) % 12,
    (raasi + 6) % 12,
]
...
def dushthanas():
    ...
    for house in range(12):
        dust = [
            house,
            dushthana_aspects_of_the_raasi(house),
        ]  # [house,[(house)%12, (house+2)%12, (house+6)%12]]
...
def chathusras():
    ...
    for house in range(12):
        chat = [
            house,
            chathusra_aspects_of_the_raasi(house),
        ]  # [house,[(house)%12, (house+4)%12]]
...
kendra_aspects_of_the_raasi = lambda raasi: [
    (raasi) % 12,
    (raasi + 3) % 12,
    (raasi + 6) % 12,
    (raasi + 9) % 12,
...
def kendras():
    ...
    for house in range(12):
        ken = [
            house,
            kendra_aspects_of_the_raasi(house),
        ]  # [house,[(house)%12, (house+3)%12, (house+6)%12,(house+9)%12]]
...
""" Get All kendra aspects of the given raasi"""
upachaya_aspects_of_the_raasi = lambda raasi: [
    (raasi) % 12,
    (raasi + 3) % 12,
    (raasi + 7) % 12,
    (raasi + 8) % 12,
...
def upachayas():
    ...
    for house in range(12):
        upa = [
            house,
            [(house) % 12, (house + 3) % 12, (house + 7) % 12, (house + 8) % 12],
        ]
...
def graha_drishti_from_chart(house_to_planet_dict, separator="/"):
    ...
    for p, _ in enumerate(planet_list):  # [:7]):
        ...
        app[p] = sum(
            [
                h_to_p[ar].replace(const._ascendant_symbol, "").split(separator)
                for ar in arp[p]
                if h_to_p[ar] != ""
            ],
            [],
...
def _get_raasi_drishti():
    ...
    _raasi_drishti = {
        **_get_raasi_drishti_movable(),
        **_get_raasi_drishti_fixed(),
        **_get_raasi_drishti_dual(),
    }
...
def raasi_drishti_from_chart(house_to_planet_dict, separator="/"):
    ...
    for p, _ in enumerate(planet_list[:9]):
        ...
        app[p] = sum(
            [h_to_p[ar].split(separator) for ar in arp[p] if h_to_p[ar] != ""], []
        )
        app[p] = [
            int(pp) for pp in app[p] if pp != "" and pp != const._ascendant_symbol
        ]
    return arp, ahp, app
...
def aspected_planets_of_the_planet(house_to_planet_dict, planet, separator="/"):
    ...
    _, _, app = graha_drishti_from_chart(house_to_planet_dict, separator)
    # print('app',app)
    aspected_planets = utils.flatten_list(
        [map(int, value) for key, value in app.items() if planet == key]
    )
...
def aspected_rasis_of_the_planet(house_to_planet_dict, planet, separator="/"):
    ...
    (
        arp,
        _,
    ) = graha_drishti_from_chart(house_to_planet_dict, separator)
    aspected_rasis = utils.flatten_list(
        [map(int, value) for key, value in arp.items() if planet == key]
    )
...
def aspected_houses_of_the_planet(house_to_planet_dict, planet, separator="/"):
    ...
    aspected_houses = utils.flatten_list(
        [map(int, value) for key, value in ahp.items() if planet == key]
    )
...
def get_argala(house_to_planet_dict, separator="\n"):
    ...
    argala = [
        [
            h_to_p[(r + asc_house + a - 1) % 12]
            .replace(const._ascendant_symbol, "")
            .replace(separator, "/")
            .replace("//", "/")
            for a in const.argala_houses
        ]
        for r in range(12)
    ]
    virodhargala = [
        [
            ...
            .replace(separator, "/")
            .replace("//", "/")
            for a in const.virodhargala_houses
        ]
        for r in range(12)
    ]
    return argala, virodhargala
...
def stronger_planet_from_planet_positions(
    planet_positions, planet1=const._SATURN, planet2=7, check_during_dhasa=False
):
    """
    ...
    if planet1 == const._ascendant_symbol:
        planet1_rasi = planet_positions[0][1][0]
        planet2_rasi = planet_positions[planet2 + 1][1][0]
        sr = stronger_rasi_from_planet_positions(
            planet_positions, planet1_rasi, planet2_rasi
        )
        if sr == planet1_rasi:
            return planet1
        else:
    ...
    if planet2 == const._ascendant_symbol:
        planet2_rasi = planet_positions[0][1][0]
        planet1_rasi = planet_positions[planet1 + 1][1][0]
        sr = stronger_rasi_from_planet_positions(
            planet_positions, planet1_rasi, planet2_rasi
        )
        if sr == planet1_rasi:
            ...
            return planet2
    house_to_planet_dict = utils.get_house_planet_list_from_planet_positions(
        planet_positions
    )
    ...
    if _debug_print:
        print(
            "Rule-4: Both planets are in same type of rasi Dual/fixed/movable - and are equally stronger"
        )
    planet1_house = p_to_h[planet1]
    planet1_longitude = planet_positions[planet1 + 1][1][
        1
    ]  # +1 to inlcude first element 'L'
    ...
    if check_during_dhasa:
        ...
        planet1_narayana_dhasa_duration = narayana._dhasa_duration(
            planet_positions, planet1_house
        )
        if _debug_print:
            print("planet1_narayana_dhasa_duration", planet1_narayana_dhasa_duration)
        planet2_narayana_dhasa_duration = narayana._dhasa_duration(
            planet_positions, planet2_house
        )
        if _debug_print:
...
def _stronger_planet_new(house_to_planet_dict, planet1=const._SATURN, planet2=7):
    ...
    if _debug_print:
        print(
            "stronger_planet_new: finding stronger co lords ",
            planet_list[planet1],
            planet_list[planet2],
        )
    if planet1 == planet2:
    ...
    if _debug_print:
        print(
            ...
            "in house",
            planet2_house,
            rasi_names_en[planet2_house],
        )
    if planet1 in [7, 8]:
        lord_house_of_planets = const.houses_of_rahu_kethu[
            planet1
        ]  # const.house_lords_dict#
    elif planet2 in [7, 8]:
        lord_house_of_planets = const.houses_of_rahu_kethu[
            planet2
        ]  # const.house_lords_dict#
    if _debug_print:
        print("lord_house_of_planets", lord_house_of_planets)
    """ Basic Rule - If Planet1/Saturn/Mars in Aq/Sc and Planet2/Rahu/Ketu elsewhere then Planet2/Rahu/Ketu is stronger """
    if (
        (planet2 in [7, 8] or planet1 in [7, 8])
        and planet1_house == lord_house_of_planets
        and planet2_house != lord_house_of_planets
    ):
        if _debug_print:
    ...
    if _debug_print:
        print(
            ...
            planet_list[planet2],
            "in",
            lord_house_of_planets,
        )
    """ Rule-1: If one planet is joined by more planets than the other, it is stronger. """
    planet1_co_planet_count = (
        sum(value == planet1_house for value in p_to_h.values()) - 1
    )  # Exclude planet itsef
    if _debug_print:
        print("Rule-1", "planet1_co_planet_count", planet1_co_planet_count)
    planet2_co_planet_count = (
        sum(value == planet2_house for value in p_to_h.values()) - 1
    )  # Exclude planet itself
    if _debug_print:
    ...
    if _debug_print:
        print(
            "Rule-1: Both planets have same co planet count",
            planet1_co_planet_count,
            planet2_co_planet_count,
        )
    ...
    if _debug_print:
        print(
            "Aspects of",
            planet_list[planet1],
            [planet_list[p] for p in planet1_aspects],
        )
    planet1_co_planet_count += sum(
        p1 in planet1_aspects for p1 in [3, 4, dispositor_of_planet1_house]
    )
    if _debug_print:
    ...
    if _debug_print:
        print(
            "Aspects of",
            planet_list[planet2],
            [planet_list[p] for p in planet2_aspects],
        )
    planet2_co_planet_count += sum(
        p2 in planet2_aspects for p2 in [3, 4, dispositor_of_planet2_house]
    )
    if _debug_print:
    ...
    if _debug_print:
        print(
            "Rule-2 Both planets have same Cojoin/Aspects count",
            planet1_co_planet_count,
            planet2_co_planet_count,
        )
    ...
    if _debug_print:
        print(
            "house_strengths_of_planet1",
            const.house_strengths_of_planets[planet1][planet1_house],
            "house_strengths_of_planet2",
            const.house_strengths_of_planets[planet2][planet2_house],
        )
    if const.house_strengths_of_planets[planet1][
        planet1_house
    ] == const._EXALTED_UCCHAM and (
        const.house_strengths_of_planets[planet1][planet1_house]
        > const.house_strengths_of_planets[planet2][planet2_house]
    ):
        if _debug_print:
        ...
        return planet1
    if const.house_strengths_of_planets[planet2][
        planet2_house
    ] == const._EXALTED_UCCHAM and (
        const.house_strengths_of_planets[planet2][planet2_house]
        > const.house_strengths_of_planets[planet2][planet2_house]
...
def stronger_planet(
    ...
    planet1=const._SATURN,
    planet2=7,
    check_during_dhasa=False,
    planet1_longitude=None,
    planet2_longitude=None,
):
    """
    To find stronger planet between Rahu/Saturn/Aquarius or Ketu/Mars/Scorpio
    @param house_to_planet_dict: list of raasi with planet ids in them
    ...
    lord_house_of_planets = (
        const.house_lords_dict
    )  # const.houses_of_rahu_kethu[planet2]
    # print('lord_house_of_planets',lord_house_of_planets,[rasi_names_en[p] for p in lord_house_of_planets])
    """ Basic Rule - If Planet1/Saturn/Mars in Aq/Sc and Planet2/Rahu/Ketu elsewhere then Planet2/Rahu/Ketu is stronger """
    if (
        planet1_house == lord_house_of_planets
        and planet2_house != lord_house_of_planets
    ):
        # print('Basic Rule','Planet2/Rahu/Ketu is stronger')
        return planet2
    if (
        planet2_house == lord_house_of_planets
        and planet1_house != lord_house_of_planets
    ):
        # print('Basic Rule','Planet1/Saturn/Mars is stronger')
        return planet1
    ...
    planet1_co_planet_count += sum(
        planet1_aspects.count(p1) for p1 in [3, 4, dispositor_of_planet1_house]
    )
    # print('Planet1',planet_list[planet1],' aspect count',planet1_co_planet_count)
    ...
    planet2_co_planet_count += sum(
        planet2_aspects.count(p2) for p2 in [3, 4, dispositor_of_planet2_house]
    )
    # print('Planet2',planet_list[planet2],' aspect count',planet2_co_planet_count)
    ...
    if check_during_dhasa:
        ...
        planet1_narayana_dhasa_duration = narayana._dhasa_duration(
            p_to_h, planet1_house
        )
        # print('planet1_narayana_dhasa_duration',planet1_narayana_dhasa_duration)
        planet2_narayana_dhasa_duration = narayana._dhasa_duration(
            p_to_h, planet2_house
        )
        # print('planet2_narayana_dhasa_duration',planet2_narayana_dhasa_duration)
        if planet1_narayana_dhasa_duration > planet2_narayana_dhasa_duration:
            # print('Rule-5(a): Planet1/Saturn/Mars is stronger')
...
def stronger_rasi_from_planet_positions(planet_positions, rasi1, rasi2):
    ...
    lord_of_rasi1 = house_owner_from_planet_positions(
        planet_positions, rasi1
    )  # const.house_owners[rasi1]
    lord_of_rasi2 = house_owner_from_planet_positions(
        planet_positions, rasi2
    )  # const.house_owners[rasi2]
    ...
    #    exit()
    if (
        planet_positions[lord_of_rasi1 + 1][1][1]
        > planet_positions[lord_of_rasi2 + 1][1][1]
    ):  # sp == lord_of_rasi1:
...
def stronger_rasi(house_to_planet_dict, rasi1, rasi2):
    ...
    if _DEBUG_:
        print(
            "Aspects of", rasi_names_en[rasi1], [planet_list[p] for p in rasi1_aspects]
        )
    rasi1_co_planet_count += sum(
        rasi1_aspects.count(p1) for p1 in [3, 4, lord_of_rasi1]
    )
    ...
    if _DEBUG_:
        print(
            "Aspects of", rasi_names_en[rasi2], [planet_list[p] for p in rasi2_aspects]
        )
    rasi2_co_planet_count += sum(
        rasi2_aspects.count(p1) for p1 in [3, 4, lord_of_rasi2]
    )
    ...
    rasi1_exhalted_planet_count = sum(
        [
            const.house_strengths_of_planets[int(p)][rasi1] == const._EXALTED_UCCHAM
            for p in p_to_h
            if p != const._ascendant_symbol and p_to_h[p] == rasi1
        ]
    )
    if _DEBUG_:
        print("Rule-3", "rasi1_exhalted_planet_count", rasi1_exhalted_planet_count)
    rasi2_exhalted_planet_count = sum(
        [
            const.house_strengths_of_planets[int(p)][rasi2] == const._EXALTED_UCCHAM
            for p in p_to_h
            if p != const._ascendant_symbol and p_to_h[p] == rasi2
        ]
    )
    if _DEBUG_:
    ...
    if rasi1_exhalted_planet_count > 0 and rasi2_exhalted_planet_count == 0:
        if _DEBUG_:
            print(
                "rasi1", rasi1, "contains exhalted planet and rasi2", rasi2, "does not"
            )
        return rasi1
    if rasi2_exhalted_planet_count > 0 and rasi1_exhalted_planet_count == 0:
        if _DEBUG_:
            print(
                "rasi2", rasi2, "contains exhalted planet and rasi1", rasi1, "does not"
            )
        return rasi2
    ...
    rasi1_has_oddity = (
        rasi1 in const.odd_signs and p_to_h[lord_of_rasi1] in const.even_signs
    ) or (rasi1 in const.even_signs and p_to_h[lord_of_rasi1] in const.odd_signs)
    if _DEBUG_:
        print(rasi2, lord_of_rasi2, p_to_h[lord_of_rasi2])
    rasi2_has_oddity = (
        rasi2 in const.odd_signs and p_to_h[lord_of_rasi2] in const.even_signs
    ) or (rasi2 in const.even_signs and p_to_h[lord_of_rasi2] in const.odd_signs)
    if _DEBUG_:
        print(
            "Rule-4",
            "rasi1_has_oddity",
...
def natural_friends_of_planets(h_to_p=None):
    ...
    for p in range(9):
        ...
        if p < 7:
            er = [
                house_owner(h_to_p, r)
                for r in range(12)
                if const.house_strengths_of_planets[p][r] == const._EXALTED_UCCHAM
            ]
        ...
        fr = [
            house_owner(h_to_p, r)
            for r in range(12)
            if const.house_strengths_of_planets[p][r] == const._FRIEND
        ]
        nf[p].append(fr)
...
def _get_temporary_friends_of_planets(h_to_p):
    ...
    for p in range(9):
        p_raasi = p_to_h[p]
        _temp_friends = utils.flatten_list(
            [
                h_to_p[(p_raasi + h) % 12].split("/")
                for h in const.temporary_friend_raasi_positions
                if h_to_p[(p_raasi + h) % 12] != ""
            ]
        )
...
def _get_temporary_enemies_of_planets(h_to_p):
    ...
    for p in range(9):
        p_raasi = p_to_h[p]
        _temp_enemies = utils.flatten_list(
            [
                h_to_p[(p_raasi + h) % 12].split("/")
                for h in const.temporary_enemy_raasi_positions
                if h_to_p[(p_raasi + h) % 12] != ""
            ]
        )
...
def house_owner_from_planet_positions(planet_positions, sign, check_during_dhasa=False):
    ...
    if sign == 7:
        lord_of_sign = stronger_planet_from_planet_positions(
            planet_positions, 2, 8, check_during_dhasa=check_during_dhasa
        )
    elif sign == 10:
        lord_of_sign = stronger_planet_from_planet_positions(
            planet_positions, 6, 7, check_during_dhasa=check_during_dhasa
        )
    return lord_of_sign
...
def marakas_from_planet_positions(planet_positions):
    ...
    maraka_planets = [
        house_owner_from_planet_positions(planet_positions, sign)
        for sign in maraka_sthanas
    ]  ## 2 and 7th houses are maraka sthanas
    print(maraka_sthanas, "maraka_sthana_owners", maraka_planets)
    mpls = [
        mp
        for mp in [*range(9)]
        if p_to_h[mp] in maraka_sthanas
        or p_to_h[mp] in [p_to_h[p] for p in maraka_planets]
    ]
    print("mpls", mpls)
    if mpls:
        maraka_planets += mpls
...
def marakas(h_to_p):
    ...
    maraka_sthanas = [(h + p_to_h["L"] - 1) % 12 for h in [2, 7]]
    maraka_planets = [
        house_owner(h_to_p, sign) for sign in maraka_sthanas
    ]  ## 2 and 7th houses are maraka sthanas
    # print(maraka_sthanas,'maraka_sthana_owners',maraka_planets)
    mpls = [
        mp
...
def rudra_based_on_planet_positions(dob, tob, place, divisional_chart_factor=1):
    ...
    eighth_house_lord = house_owner(
        h_to_p, const.rudra_eighth_house[(lagna_house + 7) % 12]
    )
    ...
    _rudra = stronger_planet_from_planet_positions(
        planet_positions,
        eighth_house_lord,
        seventh_house_lord,
        check_during_dhasa=False,
...
def brahma(planet_positions):
    ...
    lords = [
        house_owner_from_planet_positions(planet_positions, (sp + h - 1) % 12)
        for h in [6, 8, 12]
    ]
    # print('lords',lords)
    ...
    # print(lords_scores)
    lords_scores = dict(
        sorted(lords_scores.items(), key=lambda x: x[1], reverse=True)[:2]
    )
    # print(lords_scores)
    if len(lords_scores) == 1:
        brahma = list(lords_scores.keys())[0]
    elif len(lords_scores) == 2:
        brahma = stronger_planet_from_planet_positions(
            planet_positions, list(lords_scores.keys())[0], list(lords_scores.keys())[1]
        )
    else:
        b1 = stronger_planet_from_planet_positions(
            planet_positions, list(lords_scores.keys())[0], list(lords_scores.keys())[1]
        )
        brahma = stronger_planet_from_planet_positions(
            planet_positions, brahma, list(lords_scores.keys())[2]
        )
    return brahma
...
def rudra(planet_positions):
    ...
    seventh_house_lord = house_owner(
        h_to_p, const.rudra_eighth_house[(lagna_house + 6) % 12]
    )  # 8th house of 7th house from Lagna
    # print('7th house',rasi_names_en[const.rudra_eighth_house[(lagna_house+6)%12]],'7th house lord',planet_list[seventh_house_lord])
    _rudra = stronger_planet_from_planet_positions(
        planet_positions, eighth_house_lord, seventh_house_lord
    )
...
def maheshwara_from_planet_positions(planet_positions):
    ...
    _maheshwara = house_owner_from_planet_positions(
        planet_positions, (atma_karaka_house + 7) % 12
    )  # V4.6.0
    ...
    if p_to_h[_maheshwara] == const.house_owners[_maheshwara]:
        ...
        atma_8th_lord = house_owner_from_planet_positions(
            planet_positions, (atma_karaka_house + 7) % 12
        )  # V4.6.0
        atma_12th_lord = house_owner_from_planet_positions(
            planet_positions, (atma_karaka_house + 11) % 12
        )  # V4.6.0
        _maheshwara = stronger_planet_from_planet_positions(
            planet_positions, atma_8th_lord, atma_12th_lord
        )  # V4.6.0
    ...
    if p_to_h[_maheshwara] == p_to_h[7] or p_to_h[_maheshwara] == p_to_h[8]:
        _maheshwara = house_owner_from_planet_positions(
            planet_positions, (atma_karaka_house + 5) % 12
        )  # V4.6.0
    elif (
        p_to_h[7] == (p_to_h[_maheshwara] + 7) % 12
        or p_to_h[8] == (p_to_h[_maheshwara] + 7) % 12
    ):
        _maheshwara = house_owner_from_planet_positions(
            planet_positions, (atma_karaka_house + 5) % 12
        )  # V4.6.0
...
longevity_of_pair = lambda rasi1, rasi2: [
    key for key, value in const.longevity.items() if (rasi1, rasi2) in value
][0]
...
def longevity(dob, tob, place, divisional_chart_factor=1):
    ...
    rasi_type = lambda rasi: [
        index
        for index, r_type in enumerate(
            [const.fixed_signs, const.movable_signs, const.dual_signs]
        )
        if rasi in r_type
    ][0]
    p_to_h = utils.get_planet_house_dictionary_from_planet_positions(planet_positions)
    ...
    eighth_lord_house = p_to_h[
        house_owner(h_to_p, const.rudra_eighth_house[lagna_house])
    ]
    ...
    pair1_longevity = longevity_of_pair(
        rasi_type(lagna_lord_house), rasi_type(eighth_lord_house)
    )
    ...
    hora_lagna_house = drik.hora_lagna(
        jd, place, divisional_chart_factor=divisional_chart_factor
    )[
        0
    ]  # V3.1.9
    # print('hora_lagna_house',hora_lagna_house)
    pair3_longevity = longevity_of_pair(
        rasi_type(lagna_house), rasi_type(hora_lagna_house)
    )
    ...
    def _longevity_pair_check(pair1_longevity, pair2_longevity, pair3_longevity):
        if pair1_longevity == pair2_longevity and pair2_longevity == pair3_longevity:
            ...
            pair1_longevity != pair2_longevity
            and pair1_longevity != pair3_longevity
            and pair2_longevity != pair3_longevity
        ):
...
def associations_of_the_planet(planet_positions, planet):
    ...
    planet_to_house_dict = utils.get_planet_house_dictionary_from_planet_positions(
        planet_positions
    )
    ap = []
    """ (1) The two planets are conjoined,"""
    pl = [
        int(p)
        for p in [*range(9)]
        if planet_to_house_dict[p] == planet_to_house_dict[planet] and p != planet
    ]
    # print('1',planet,pl)
    ap += pl
    ...
    pl = [
        ...
        for p in [*range(9)]
        if int(planet) != int(p)
        and house_owner_from_planet_positions(planet_positions, planet_to_house_dict[p])
        == planet
        and house_owner_from_planet_positions(
            planet_positions, planet_to_house_dict[planet]
        )
        == p
    ]
    # print('3',planet,pl)
...
if __name__ == "__main__":
    ...
    planet_positions = charts.divisional_chart(
        jd_at_dob, place_as_tuple, divisional_chart_factor=dcf, chart_method=dcf
    )
    print(
        "rudra",
        rudra(planet_positions),
        "brahma",
        brahma(planet_positions),
        "maheshwara",
