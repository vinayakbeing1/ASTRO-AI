...
def get_yoga_details_for_all_charts(
    jd, place, language="en", divisional_chart_factor=None
):
    """
    ...
    asc_house_navamsa, asc_long = drik.dasavarga_from_long(
        ascendant_longitude, divisional_chart_factor=9
    )
    planet_positions_navamsa += [[ascendant_index, (asc_house_navamsa, asc_long)]]
    p_to_h_navamsa = utils.get_planet_house_dictionary_from_planet_positions(
        planet_positions_navamsa
    )
    h_to_p_navamsa = utils.get_house_planet_list_from_planet_positions(
        planet_positions_navamsa
    )
    if divisional_chart_factor == None:
        for dv in division_chart_factors:
            yoga_results, _, _ = get_yoga_details(
                jd, place, divisional_chart_factor=dv, language=language
            )
    ...
    return (
        yoga_results_combined,
        len(yoga_results_combined),
        len(msgs) * len(division_chart_factors),
    )
...
def get_yoga_details(jd, place, divisional_chart_factor=1, language="en"):
    ...
    for yoga_function, details in msgs.items():
        ...
        yoga_exists = eval(yoga_function + "_from_planet_positions")(
            planet_positions
        )  # (h_to_p)#
        if yoga_exists:
...
def vesi_yoga_from_planet_positions(planet_positions):
    ...
    yp = str(1) not in yoga_house_planets and (
        (
            len(yoga_house_planets) == 1
            and const._ascendant_symbol not in yoga_house_planets
        )
        or len(yoga_house_planets) > 1
    )
    return yp
...
def ubhayachara_yoga_from_planet_positions(planet_positions):
    ...
    yp = vesi_yoga_from_planet_positions(
        planet_positions
    ) and vosi_yoga_from_planet_positions(planet_positions)
    return yp
...
budha_aaditya_yoga_from_planet_positions = (
    lambda planet_positions: nipuna_yoga_from_planet_positions(planet_positions)
)
...
def sunaphaa_yoga_from_planet_positions(planet_positions):
    ...
    yoga_house_planets = h_to_p[yoga_house].split("/")
    yp = str(other_than_planet) not in yoga_house_planets and (
        (
            len(yoga_house_planets) == 1
            and const._ascendant_symbol not in yoga_house_planets
        )
...
def duradhara_yoga_from_planet_positions(planet_positions):
    ...
    return sunaphaa_yoga_from_planet_positions(
        planet_positions
    ) and anaphaa_yoga_from_planet_positions(planet_positions)
...
def kemadruma_yoga_from_planet_positions(planet_positions):
    ...
    planets_from_moon = [
        p for h in houses_from_moon for p, h1 in p_to_h.items() if h == h1
    ]
    ...
    planets_from_quadrants_of_lagna = [
        p for h in quadrants_from_lagna for p, h1 in p_to_h.items() if h == h1
    ]
...
def adhi_yoga_from_planet_positions(planet_positions):
    ...
    adhi_yoga_1 = any(
        "4" in x or "5" in x
        for x in [h_to_p[(p_to_h[1] + mh - 1) % 12] for mh in [6, 7, 8]]
    )
    # Mercury (3) benefic if alone.
    adhi_yoga_2 = any(
        x == "3" for x in [h_to_p[(p_to_h[1] + mh - 1) % 12] for mh in [6, 7, 8]]
    )
...
def ruchaka_yoga_from_planet_positions(planet_positions):
    ...
    ruchaka_yoga_2 = any(
        "2" in x
        for x in [
            h_to_p[(p_to_h[const._ascendant_symbol] + mh - 1) % 12]
            for mh in [1, 4, 7, 10]
        ]
    )
...
def maalaa_yoga_from_planet_positions(planet_positions):
    ...
    planets_in_lagnam_quadrant = [
        h_to_p[a].split("/")
        for a in quadrants_of_the_house(p_to_h[const._ascendant_symbol])
    ]
    chk = [
        [str(nb) in str(pl) for pl in planets_in_lagnam_quadrant]
        for nb in const.natural_benefics
    ]
...
def sarpa_yoga_from_planet_positions(planet_positions):
    ...
    chk = [
        [str(nb) in str(pl) for pl in planets_in_lagnam_quadrant]
        for nb in const.natural_malefics
    ]
    # print('chk',chk,)
...
def gadaa_yoga_from_planet_positions(planet_positions):
    ...
    slq = [
        tuple(sorted(((asc_house) % 12, (asc_house + 3) % 12))),
        tuple(sorted(((asc_house + 3) % 12, (asc_house + 6) % 12))),
        tuple(sorted(((asc_house + 6) % 12, (asc_house + 9) % 12))),
        tuple(sorted(((asc_house + 9) % 12, asc_house % 12))),
    ]
...
def sringaataka_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    slq = [
        tuple(sorted(((asc_house) % 12, (asc_house + 4) % 12, (asc_house + 8) % 12)))
    ]
    sph = tuple(sorted(set(list(p_to_h.values())[:-1])))
...
def hala_yoga_from_planet_positions(planet_positions):
    ...
    slq = [
        tuple(
            sorted(((asc_house + 1) % 12, (asc_house + 5) % 12, (asc_house + 9) % 12))
        ),
        tuple(
            sorted(((asc_house + 2) % 12, (asc_house + 6) % 12, (asc_house + 10) % 12))
        ),
        tuple(
            sorted(((asc_house + 3) % 12, (asc_house + 7) % 12, (asc_house + 11) % 12))
        ),
    ]
    sph = tuple(sorted(set(list(p_to_h.values())[:-1])))
...
def vajra_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    chk1 = [
        [str(nb) in h_to_p[a] for a in [asc_house % 12, (asc_house + 6) % 12]]
        for nb in const.natural_benefics
    ]
    vajra_yoga_1 = sum([sum(x) for x in chk1]) > 1  # all(any(row) for row in chk1)
    chk2 = [
        [str(nb) in h_to_p[a] for a in [(asc_house + 3) % 12, (asc_house + 9) % 12]]
        for nb in const.natural_malefics
    ]
...
def kamala_yoga_from_planet_positions(planet_positions):
    ...
    chk = [
        [str(nb) in str(pl) for pl in planets_in_lagnam_quadrant] for nb in all_planets
    ]
...
def subha_yoga_from_planet_positions(planet_positions):
    ...
    chk = [
        [
            str(nb) in h_to_p[a]
            for a in [(asc_house) % 12, (asc_house + 1) % 12, (asc_house + 11) % 12]
        ]
        for nb in const.natural_benefics
    ]
...
def gaja_kesari_yoga_from_planet_positions(planet_positions):
    ...
    chk2 = any(
        int(p) in const.natural_benefics or p_to_h[int(p)] == jupiter_house
        for p in bpj
    ...
    #    return False
    chk3 = (
        const.house_strengths_of_planets[4][jupiter_house] > const._ENEMY
    )  # not debiliated not in enemy's house
...
def guru_mangala_yoga_from_planet_positions(planet_positions):
    ...
    gm_yoga = (
        p_to_h[2] == p_to_h[4]
        or p_to_h[2] == (p_to_h[4] + 6) % 12
        or p_to_h[4] == (p_to_h[2] + 6) % 12
    )
    return gm_yoga
...
def amala_yoga_from_planet_positions(planet_positions):
    ...
    ay = any(
        [
            str(p1) in str(h_to_p[h])
            for p1 in const.natural_benefics
            for h in [lagna_tenth_house, moon_tenth_house]
        ]
    )
    return ay
...
def parvata_yoga_from_planet_positions(planet_positions):
    ...
    chk2 = [
        [
            h_to_p[a] == "" or (str(nb) in h_to_p[a])
            for a in [(asc_house + 6) % 12, (asc_house + 7) % 12]
        ]
        for nb in const.natural_benefics
    ]
...
def kaahala_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    fourth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 3) % 12
    )
...
def chaamara_yoga_from_planet_positions(planet_positions):
    ...
    cy = [
        [
            (str(nb) in h_to_p[a])
            for a in [(asc_house + 6) % 12, (asc_house + 8) % 12, (asc_house + 9) % 12]
        ]
        for nb in const.natural_benefics
    ]
    cy1 = sum(sum(cy, [])) > 1
    if cy1:
        return True
    cy2 = const.house_strengths_of_planets[lagna_lord][
        lagna_house
    ] >= const._EXALTED_UCCHAM and str(lagna_lord) in house.graha_drishti_of_the_planet(
        h_to_p, 4
    )
...
def sankha_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    fifth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 4) % 12
    )
    sixth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 5) % 12
    )
    lagna_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    ky1 = str(fifth_lord) in [
        h_to_p[a] for a in quadrants_of_the_house((asc_house + 5) % 12)
    ]
    ky2 = str(sixth_lord) in [
        h_to_p[a] for a in quadrants_of_the_house((asc_house + 4) % 12)
    ]
    ky3 = const.house_strengths_of_planets[lagna_lord][asc_house] > const._NEUTRAL_SAMAM
    ninth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 8) % 12
    )
    ky4 = (
        const.house_strengths_of_planets[ninth_lord][p_to_h[ninth_lord]]
        > const._NEUTRAL_SAMAM
    )
    ky5 = (
        p_to_h[lagna_lord] == p_to_h[ninth_lord]
        and p_to_h[lagna_lord] in const.movable_signs
    )
...
def bheri_yoga_from_planet_positions(planet_positions):
    ...
    by3 = [
        h in quadrants_of_the_house(asc_house)
        for h in [p_to_h[4], p_to_h[5], asc_house]
    ]
...
def mridanga_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    quadrants_and_trines = house.quadrants_of_the_raasi(
        asc_house
    ) + house.trines_of_the_raasi(asc_house)
    my1 = any(
        [
            any(
                [
                    p_to_h[p] == h
                    and const.house_strengths_of_planets[p][h] > const._FRIEND
                    for h in quadrants_and_trines
                ]
            )
            for p in range(9)
        ]
    ...
    lagna_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    my2 = (
        const.house_strengths_of_planets[lagna_lord][p_to_h[lagna_lord]] > const._FRIEND
    )
    return my1 and my2
...
def sreenaatha_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    seventh_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 6) % 12
    )
    ninth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 8) % 12
    )
    tenth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 8) % 12
    )
    sy1 = (
        const.house_strengths_of_planets[seventh_lord][(asc_house + 9) % 12]
        > const._FRIEND
    )
...
def matsya_yoga_from_planet_positions(planet_positions):
    ...
    if not my1:
        return False
    ph = sum(
        [h_to_p[h].split("/") for h in [(asc_house + 3) % 12, (asc_house + 7) % 12]], []
    )
    ph = [p for p in ph if p != ""]
...
def koorma_yoga_from_planet_positions(planet_positions):
    ...
    ph = sum(
        [
            h_to_p[h].split("/")
            for h in [(asc_house + 4) % 12, (asc_house + 5) % 12, (asc_house + 6) % 12]
        ],
        [],
    )
    ph = [p for p in ph if p != ""]
    ...
    my1 = all(
        [
            const.house_strengths_of_planets[int(p)][p_to_h[int(p)]]
            > const._NEUTRAL_SAMAM
            for p in ph
            if p != const._ascendant_symbol
        ]
    ...
    ph = sum(
        [
            h_to_p[h].split("/")
            for h in [asc_house, (asc_house + 2) % 12, (asc_house + 10) % 12]
        ],
        [],
    )
    ph = [p for p in ph if p != ""]
...
def khadga_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    second_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 1) % 12
    )
    ninth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 8) % 12
    )
    lagna_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    ky1 = (p_to_h[second_lord] == (asc_house + 8) % 12) and (
        p_to_h[ninth_lord] == (asc_house + 1) % 12
    )
    quadrants_and_trines = house.quadrants_of_the_raasi(
        asc_house
    ) + house.trines_of_the_raasi(asc_house)
...
def kusuma_yoga_from_planet_positions(planet_positions):
    ...
    ky3 = any(
        [
            p_to_h[1] in house.trines_of_the_raasi(p_to_h[nb])
            for nb in const.natural_benefics
        ]
    )
    ...
    ky3 = all(
        [
            any(
                [
                    str(p1) in str(p2)
                    for p2 in house.aspected_planets_of_the_raasi(h_to_p, p_to_h[4])
                ]
            )
            for p1 in [3, 5]
        ]
...
def kalpadruma_yoga_from_planet_positions(planet_positions):
    ...
    # Comment this for actual """
    lagna_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    depositor_1 = house.house_owner_from_planet_positions(
        planet_positions, p_to_h[lagna_lord]
    )
    depositor_2 = house.house_owner_from_planet_positions(
        planet_positions, p_to_h[depositor_1]
    )
    depositor_3 = house.house_owner_from_planet_positions(
        planet_positions, p_to_h_navamsa[depositor_1]
    )
    all_four_planets = [lagna_lord, depositor_1, depositor_2, depositor_3]
    ky1 = []
    quadrants_and_trines_of_rasi = house.quadrants_of_the_raasi(
        asc_house
    ) + house.trines_of_the_raasi(asc_house)
    quadrants_and_trines_of_navamsa = house.quadrants_of_the_raasi(
        asc_house_navamsa
    ) + house.trines_of_the_raasi(asc_house_navamsa)
...
def lagnaadhi_yoga_from_planet_positions(planet_positions):
    ...
    ly1 = all(
        [
            any([p_to_h[nb] == h for h in [(asc_house + 6) % 12, (asc_house + 7) % 12]])
            for nb in const.natural_benefics
        ]
    )
    if not ly1:
...
def hari_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    lord_house = p_to_h[
        house.house_owner_from_planet_positions(planet_positions, (asc_house + 1) % 12)
    ]
    benefic_houses = [
        (lord_house + 1) % 12,
        (lord_house + 7) % 12,
    ...
    ly1 = all(
        [
            any([p_to_h[nb] == h for h in benefic_houses])
            for nb in const.natural_benefics
        ]
    )
    return ly1
...
def hara_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    lord_house = p_to_h[
        house.house_owner_from_planet_positions(planet_positions, (asc_house + 6) % 12)
    ]
    benefic_houses = [
        (lord_house + 3) % 12,
        (lord_house + 8) % 12,
...
def brahma_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    lord_house = p_to_h[
        house.house_owner_from_planet_positions(planet_positions, asc_house)
    ]
    benefic_houses = [
        (lord_house + 3) % 12,
        (lord_house + 9) % 12,
    ...
    ly2 = p_to_h[4] in quadrants_of_the_house(
        p_to_h[
            house.house_owner_from_planet_positions(
                planet_positions, (asc_house + 8) % 12
            )
        ]
    )
    ly3 = p_to_h[5] in quadrants_of_the_house(
        p_to_h[
            house.house_owner_from_planet_positions(
                planet_positions, (asc_house + 10) % 12
            )
        ]
    )
    ly4_1 = p_to_h[3] in quadrants_of_the_house(
        p_to_h[
            house.house_owner_from_planet_positions(planet_positions, (asc_house) % 12)
        ]
    )
    ly4_2 = p_to_h[3] in quadrants_of_the_house(
        p_to_h[
            house.house_owner_from_planet_positions(
                planet_positions, (asc_house + 9) % 12
            )
        ]
    )
...
def vishnu_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    ninth_lord_in_rasi = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 8) % 12
    )
    vy1 = (
        p_to_h[ninth_lord_in_rasi] == (asc_house + 1) % 12
        and p_to_h[
            house.house_owner_from_planet_positions(
                planet_positions, (asc_house + 9) % 12
            )
    ...
    if not vy1:
        return False
    lord_of_ninth_in_navamsa = house.house_owner_from_planet_positions(
        planet_positions, p_to_h_navamsa[ninth_lord_in_rasi]
    )
...
def siva_yoga_from_planet_positions(planet_positions):
    ...
    sy = (
        p_to_h[fifth_lord] == (asc_house + 8) % 12
        and p_to_h[ninth_lord] == (asc_house + 9) % 12
        and p_to_h[tenth_lord] == (asc_house + 4) % 12
    )
    return sy
...
def trilochana_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    ty = p_to_h[1] in house.trines_of_the_raasi(p_to_h[0]) and p_to_h[
        2
    ] in house.trines_of_the_raasi(p_to_h[0])
    return ty
...
def gouri_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    navamsa_lord = house.house_owner_from_planet_positions(
        planet_positions, asc_house_navamsa
    )
    rasi_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    gy = (
        p_to_h[rasi_lord] == (asc_house + 9) % 12
        and p_to_h[navamsa_lord] == (asc_house + 9) % 12
        and const.house_strengths_of_planets[navamsa_lord][(asc_house + 9) % 12]
        > const._FRIEND
    )
    return gy
...
def chandikaa_yoga_from_planet_positions(planet_positions):
    ...
    cy1 = asc_house in const.fixed_signs and str(
        lagna_lord
    ) in house.graha_drishti_of_the_planet(h_to_p, sixth_lord)
    # print('cy1',asc_house,const.fixed_signs,str(lagna_lord),house.graha_drishti_of_the_planet(h_to_p, sixth_lord))
    sixth_lord_owner_in_navamsa = house.house_owner_from_planet_positions(
        planet_positions, p_to_h_navamsa[sixth_lord]
    )
    ninth_lord_owner_in_navamsa = house.house_owner_from_planet_positions(
        planet_positions, p_to_h_navamsa[ninth_lord]
    )
    cy2 = (
        p_to_h[0] == p_to_h[sixth_lord_owner_in_navamsa]
        and p_to_h[0] == p_to_h[ninth_lord_owner_in_navamsa]
    )
    return cy1 and cy2
...
def lakshmi_yoga_from_planet_positions(planet_positions):
    ...
    lagna_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    ly1 = const.house_strengths_of_planets[ninth_lord][
        p_to_h[ninth_lord]
    ] > const._FRIEND and p_to_h[ninth_lord] in house.quadrants_of_the_raasi(asc_house)
    ly2 = (
        const.house_strengths_of_planets[lagna_lord][p_to_h[lagna_lord]] > const._FRIEND
    )
    return ly1 and ly2
...
def saarada_yoga_from_planet_positions(planet_positions):
    ...
    if not sy3:
        return False
    sy4 = [
        any([str(h) in str(t) for t in house.trines_of_the_raasi(p_to_h[1])])
        for h in [p_to_h[3], p_to_h[4]]
    ]
    if not sy4:
        return False
...
def bhaarathi_yoga_from_planet_positions(planet_positions):
    ...
    eleventh_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 10) % 12
    )
    ninth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 8) % 12
    ...
    navamsa_lords = [
        house.house_owner_from_planet_positions(
            planet_positions, p_to_h_navamsa[second_lord]
        ),
        house.house_owner_from_planet_positions(
            planet_positions, p_to_h_navamsa[fifth_lord]
        ),
        house.house_owner_from_planet_positions(
            planet_positions, p_to_h_navamsa[eleventh_lord]
        ),
    ]
    by1 = [
        p_to_h[ninth_lord] == p_to_h[nl]
        and const.house_strengths_of_planets[nl][p_to_h[ninth_lord]] > const._FRIEND
...
def saraswathi_yoga_from_planet_positions(planet_positions):
    ...
    quadrants_and_trines = house.quadrants_of_the_raasi(
        asc_house
    ) + house.trines_of_the_raasi(p_to_h[3])
    sy1 = all(
        [
            any(
                [
                    str(p) in str(qt)
                    for qt in [(asc_house + 1) % 12] + quadrants_and_trines
                ]
            )
            for p in [3, 4, 5]
        ]
...
def amsaavatara_yoga_from_planet_positions(planet_positions):
    ...
    ay1 = all(
        [
            any([str(p) in str(q) for q in house.quadrants_of_the_raasi(p_to_h[4])])
            for p in [4, 5, 6]
        ]
    )
...
def devendra_yoga_from_planet_positions(planet_positions):
    ...
    if not dy2:
        return False
    dy3 = (
        p_to_h[house.house_owner_from_planet_positions(planet_positions, asc_house)]
        == (asc_house + 10) % 12
        and p_to_h[
            house.house_owner_from_planet_positions(
                planet_positions, (asc_house + 10) % 12
...
def kulavardhana_yoga_from_planet_positions(planet_positions):
    ...
    ky = all(
        [
            p_to_h[p] == (p_to_h[const._ascendant_symbol] + 4) % 12
            or p_to_h[p] == (p_to_h[0] + 4) % 12
            or p_to_h[p] == (p_to_h[1] + 4) % 12
            for p in [*range(2, 9)]
        ]
    )
    return ky
...
def vasumati_yoga_from_planet_positions(planet_positions):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    vy = str(p_to_h[6]) in house.upachayas_of_the_raasi(p_to_h[5]) or p_to_h[
        5
    ] in house.upachayas_of_the_raasi(p_to_h[6])
    return vy
...
def gandharva_yoga_from_planet_positions(planet_positions):
    ...
    lagna_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    gy1 = p_to_h[
        house.house_owner_from_planet_positions(planet_positions, (asc_house + 9) % 12)
    ] in house.trines_of_the_raasi((asc_house + 6) % 12)
    if not gy1:
        return False
    # gy2 = p_to_h[lagna_lord] == p_to_h[4] or p_to_h[lagna_lord] in house.aspected_rasis_of_the_planet(h_to_p, 4)
    gy2 = p_to_h[lagna_lord] == p_to_h[4] or str(
        lagna_lord
    ) in house.graha_drishti_of_the_planet(h_to_p, 4)
...
def go_yoga_from_planet_positions(planet_positions):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    gy1 = (
        const.house_strengths_of_planets[4][const.moola_trikona_of_planets[4]]
        > const._FRIEND
    )
    if not gy1:
        return False
...
def vidyut_yoga_from_planet_positions(planet_positions):
    ...
    lagna_lord = house.house_owner_from_planet_positions(planet_positions, asc_house)
    vy1 = (
        const.house_strengths_of_planets[eleventh_lord][p_to_h[eleventh_lord]]
        > const._FRIEND
    )
    if not vy1:
        return False
...
def pushkala_yoga_from_planet_positions(planet_positions):
    ...
    dispositor_of_moon = house.house_owner_from_planet_positions(
        planet_positions, moon_house
    )
    ...
    py3 = (
        len([h for h in h_to_p[p_to_h[const._ascendant_symbol]].split("/") if h != ""])
        > 1
    )
...
def makuta_yoga_from_planet_positions(planet_positions):
    ...
    my2 = any(
        [
            np in ["5", "6"]
            for np in h_to_p[(asc_house + 8) % 12].split("/")
            if np != const._ascendant_symbol and np != ""
        ]
    )
...
def jaya_yoga_from_planet_positions(planet_positions):
    ...
    jy1 = (
        const.house_strengths_of_planets[tenth_lord][p_to_h[tenth_lord]]
        == const._EXALTED_UCCHAM
    )
    sixth_lord = house.house_owner_from_planet_positions(
        planet_positions, (asc_house + 5) % 12
    )
    jy2 = (
        const.house_strengths_of_planets[sixth_lord][p_to_h[sixth_lord]]
        == const._DEFIBILATED_NEECHAM
    )
    return jy1 and jy2
...
def sarala_yoga_from_planet_positions(planet_positions):
    ...
    return (
        p_to_h[
            house.house_owner_from_planet_positions(
                planet_positions, (asc_house + 7) % 12
            )
        ]
        == (asc_house + 7) % 12
    )
...
def vimala_yoga_from_planet_positions(planet_positions):
    ...
    return (
        p_to_h[
            house.house_owner_from_planet_positions(
                planet_positions, (asc_house + 11) % 12
            )
        ]
        == (asc_house + 11) % 12
    )
