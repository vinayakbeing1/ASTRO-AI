...
raasi_index = lambda planet, planet_positions_in_chart: [
    i
    for i, raasi in enumerate(planet_positions_in_chart)
    if planet != const._ascendant_symbol and planet.lower() in raasi.lower()
][0]
...
def get_ashtaka_varga(house_to_planet_list):
    ...
    prastara_ashtaka_varga = [
        [[0 for r in range(12)] for p1 in range(10)] for p2 in range(8)
    ]
    ...
    samudhaya_ashtaka_varga = (
        np.asarray(binna_ashtaka_varga[:-1]).sum(axis=0).tolist()
    )  # [0:-1] to exlcude Lagnam
...
def _sodhya_pindas(binna_ashtaka_varga_after_ekadhipatya, chart_1d):
    ...
    planet_houses = list(utils.get_planet_to_house_dict_from_chart(chart_1d).values())[
        :7
    ]  # Exclude Rahu, Ketu and Lagnam
    for p in range(7):
        graha_pindas[p] = sum(
            [
                grahamana_multipliers[i] * bav[p][pr]
                for i, pr in enumerate(planet_houses)
            ]
        )
...
def sodhaya_pindas(binna_ashtaka_varga, house_to_planet_chart):
    ...
    binna_ashtaka_varga_after_ekadhipatya = _ekadhipatya_sodhana(
        binna_ashtaka_varga_after_trikona, house_to_planet_chart
    )
    ...
    raasi_pindas, graha_pindas, sodhya_pindas = _sodhya_pindas(
        binna_ashtaka_varga_after_ekadhipatya, house_to_planet_chart
    )
    return raasi_pindas, graha_pindas, sodhya_pindas
