...
def kala_sarpa(house_to_planet_list):
    ...
    kpdc1 = all(
        [
            any([p_to_h[ph] == (rahu_house + rkh) % 12 for rkh in [*range(7)]])
            for ph in [*range(7)]
        ]
    )
    ketu_house = p_to_h[8]
    kpdc2 = all(
        [
            any([p_to_h[ph] == (ketu_house + rkh) % 12 for rkh in [*range(7)]])
            for ph in [*range(7)]
        ]
    )
...
def manglik(
    ...
    from_house = p_to_h[manglik_reference_planet]
    mars_house = p_to_h[2]
...
def _manglik_exceptions(planet_positions):
    ...
    mars_long = planet_positions[3][1][1]
    mars_from_lagna = house.get_relative_house_of_planet(lagna_house, mars_house)
    _me = []
    _me_details = []
    c1 = mars_house in [4, 10]
    _me.append(c1)
    c2 = mars_from_lagna == 2 and mars_house in [2, 5]
    _me.append(c2)
    c3 = mars_from_lagna == 4 and mars_house in [0, 7]
    _me.append(c3)
    c4 = mars_from_lagna == 7 and mars_house in [3, 9]
    _me.append(c4)
    c5 = mars_from_lagna == 8 and mars_house in [8, 11]
    _me.append(c5)
    c6 = mars_from_lagna == 12 and mars_house in [1, 6]
    _me.append(c6)
    c7 = len(house.associations_of_the_planet(planet_positions, 2)) > 0
    _me.append(c7)
    c8 = 2 in charts.planets_in_retrograde(planet_positions)
    _me.append(c8)
    c9_1 = 2 in charts.planets_in_combustion(planet_positions)
    c9_2 = mars_long < const.rasi_sandhi_duration or mars_long > (
        30.0 - const.rasi_sandhi_duration
    )
    c9 = c9_1 or c9_2
    _me.append(c9)
    c10 = house.house_owner_from_planet_positions(planet_positions, lagna_house) == 2
    _me.append(c10)
    c11 = False
    _me.append(c11)
    c12 = const.house_strengths_of_planets[2][mars_house] >= const._FRIEND
    _me.append(c12)
    c13 = mars_house in const.movable_signs
    _me.append(c13)
    c14 = False
    _me.append(c14)
    c15 = lagna_house in [3, 4]
    _me.append(c15)
    c16 = mars_house in [p_to_h[4], p_to_h[5]]
    _me.append(c16)
    c17 = lagna_house in [p_to_h[4], p_to_h[5]]
    _me.append(c17)
    _me_i = []
...
def pitru_dosha(planet_positions):
    ...
    """
    house_planet_list = utils.get_house_planet_list_from_planet_positions(
        planet_positions
    )
    planet_house_dict = utils.get_planet_house_dictionary_from_planet_positions(
        planet_positions
    )
    ninth_house = (planet_house_dict["L"] + 8) % 12
    pd = []
    pd1 = (
        planet_house_dict[0] == ninth_house
        or planet_house_dict[1] == ninth_house
        or planet_house_dict[7] == ninth_house
    )
    pd.append(pd1)
    ...
    pd3 = any(
        [
            2 in house.associations_of_the_planet(planet_positions, p1)
            or 6 in house.associations_of_the_planet(planet_positions, p1)
            for p1 in [0, 1, 7, 8]
        ]
    )
    pd.append(pd3)
    pd4 = any(
        [
            sum(
                [
                    planet_house_dict[p1] == (planet_house_dict["L"] + h - 1) % 12
                    for p1 in [3, 5, 7]
                ]
            )
            > 1
    ...
    pd5 = any(
        [
            any([planet_house_dict[p1] == planet_house_dict[p2] for p2 in [7, 8]])
            for p1 in [0, 1]
        ]
    )
    pd.append(pd5)
...
def guru_chandala_dosha(planet_positions):
    ...
    if jupiter_house == rahu_house:
        jupiter_is_strong = (
            house.stronger_planet_from_planet_positions(planet_positions, 4, 7) == 4
        )
        return True, jupiter_is_strong
    elif jupiter_house == ketu_house:
        jupiter_is_strong = (
            house.stronger_planet_from_planet_positions(planet_positions, 4, 8) == 4
        )
        return True, jupiter_is_strong
    else:
        return False, False
...
def kalathra(planet_positions, reference_planet="L"):
    ...
    kc = all(
        [
            any(
                [
                    planet_positions[p + 1][1][0] == (reference_house + h - 1) % 12
                    for h in [1, 2, 4, 7, 8, 12]
                ]
            )
            for p in const.natural_malefics
        ]
    )
    # print(kc)
    return kc
...
def _get_guru_chandala_results(planet_positions, dosha_msgs, key_str):
    ...
    if _gc[0]:
        ...
        if _gc[1]:
            if jupiter_house == rahu_house:
                m_results[m_key] += (
                    utils.resource_strings["guru_stronger_than_rahu"] + next_line
                )
            elif jupiter_house == ketu_house:
                m_results[m_key] += (
                    utils.resource_strings["guru_stronger_than_ketu"] + next_line
                )
...
def _get_kala_sarpa_results(planet_positions, dosha_msgs, key_str):
    ...
    if kpd:
        ks_results[ks_key] = "<html>" + ks_msgs[-1] + next_line
        rahu_house = house.get_relative_house_of_planet(
            planet_positions[0][1][0], planet_positions[8][1][0]
        )
...
def _get_ghata_results(planet_positions, dosha_msgs, key_str):
    ...
    if kpd:
        ks_results[ks_key] = "<html>" + ks_msgs[-1] + next_line
        mars_house = house.get_relative_house_of_planet(
            planet_positions[0][1][0], planet_positions[3][1][0]
        )
...
def _get_shrapit_results(planet_positions, dosha_msgs, key_str):
    ...
    if kpd:
        ks_results[ks_key] = "<html>" + ks_msgs[-1] + next_line
        saturn_house = house.get_relative_house_of_planet(
            planet_positions[0][1][0], planet_positions[7][1][0]
        )
...
def get_dosha_details(jd_at_dob, place_as_tuple, language=const._DEFAULT_LANGUAGE):
    ...
    m_results = _get_kalathra_results(
        planet_positions, dosha_msgs, key_str, reference_planet="L"
    )
    dosha_results.update(m_results)
    """ get ghata """
