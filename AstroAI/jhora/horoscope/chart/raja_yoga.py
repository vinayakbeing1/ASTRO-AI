...
lords_of_quadrants = lambda h_to_p, raasi: [
    house.house_owner(h_to_p, h) for h in house.quadrants_of_the_raasi(raasi)
]  # V2.3.1
lords_of_trines = lambda h_to_p, raasi: [
    house.house_owner(h_to_p, h) for h in house.trines_of_the_raasi(raasi)
]  # V2.3.1
lords_of_quadrants_from_planet_positions = lambda planet_positions, raasi: [
    house.house_owner_from_planet_positions(planet_positions, int(h))
    for h in house.quadrants_of_the_raasi(raasi)
]  # V2.3.1
lords_of_trines_from_planet_positions = lambda planet_positions, raasi: [
    house.house_owner_from_planet_positions(planet_positions, int(h))
    for h in house.trines_of_the_raasi(raasi)
]  # V2.3.1
...
def get_raja_yoga_resources(language="en"):
    ...
    """
    json_file = (
        _lang_path + const._DEFAULT_RAJA_YOGA_JSON_FILE_PREFIX + language + ".json"
    )
...
def get_raja_yoga_details_for_all_charts(
    ...
    if divisional_chart_factor == None:
        for dv in division_chart_factors:
            raja_yoga_results, _, _ = get_raja_yoga_details(
    ...
    return (
        raja_yoga_results_combined,
        len(raja_yoga_results_combined),
    ...
    for raja_yoga_function, details in msgs.items():
        ...
        if raja_yoga_pairs:
            ...
            for rp1, rp2 in raja_yoga_pairs:
                raja_yoga_exists = eval(raja_yoga_function + "_from_planet_positions")(
                    planet_positions, rp1, rp2
                )
                if raja_yoga_exists:
                    rp_str += (
                        " "
                        + "["
                        + utils.PLANET_NAMES[rp1]
                        + "-"
                        + utils.PLANET_NAMES[rp2]
                        + "] "
                    )
                    # details_str += rp_str
            if rp_str != "":
...
def _check_association(h_to_p, lord1, lord2):
    ...
    chk2 = (
        chk2_1
        and str(lord1) in house.graha_drishti_of_the_planet(h_to_p, lord2)
        and str(lord2) in house.graha_drishti_of_the_planet(h_to_p, lord1)
    )
    if chk2:
    ...
    chk3 = (lord1 == house.house_owner(h_to_p, p_to_h[lord2])) and (
        lord2 == house.house_owner(h_to_p, p_to_h[lord1])
    )
...
def _check_association_from_planet_positions(planet_positions, lord1, lord2):
    ...
    chk3 = (
        lord1
        == house.house_owner_from_planet_positions(planet_positions, p_to_h[lord2])
    ) and (
        lord2
        == house.house_owner_from_planet_positions(planet_positions, p_to_h[lord1])
    )
    return chk1 or chk2 or chk3
...
def __check_association(h_to_p, planet1, planet2):
    ...
    chk0 = (
        planet1_house in house.quadrants_of_the_raasi(asc_house)
        and planet2_house in house.trines_of_the_raasi(asc_house)
    ) or (
        planet2_house in house.quadrants_of_the_raasi(asc_house)
        and planet1_house in house.trines_of_the_raasi(asc_house)
    )
    ...
    chk2 = str(planet2) in house.graha_drishti_of_the_planet(h_to_p, planet1) or str(
        planet1
    ) in house.graha_drishti_of_the_planet(h_to_p, planet2)
    # print('chk2',planet2,house.graha_drishti_of_the_planet(h_to_p, planet1),'aspect check',chk2)
    chk3 = (
        planet1_house == p_to_h[planet2_lord] and planet2_house == p_to_h[planet1_lord]
    )
...
def __check_association_from_planet_positions(planet_positions, planet1, planet2):
    ...
    p_to_h = utils.get_planet_to_house_dict_from_chart(h_to_p)
    asc_house = p_to_h[const._ascendant_symbol]
    planet1_lord = house.house_owner_from_planet_positions(
        planet_positions, planet1
    )  # V2.3.1
    planet1_house = p_to_h[planet1]
    # print('planet1,lord,house',planet1,planet1_lord,planet1_house)
    planet2_lord = house.house_owner_from_planet_positions(
...
def dharma_karmadhipati_raja_yoga(p_to_h, raja_yoga_planet1, raja_yoga_planet2):
    ...
    house_lords = [
        house.house_owner(h_to_p, h)
        for h in [(asc_house + 8) % 12, (asc_house + 9) % 12]
    ]
    dkchk = all(
        [
            any([hl == rp for hl in house_lords])
...
def dharma_karmadhipati_raja_yoga_from_planet_positions(
    planet_positions, raja_yoga_planet1, raja_yoga_planet2
):
    """
    Dharma-Karmadhipati Yoga: This is a special case of the above yoga. If the lords
    of dharma sthana (9th) and karma sthana (10th) form a raja yoga
    ...
    h_to_p = utils.get_house_to_planet_dict_from_planet_to_house_dict(p_to_h)
    house_lords = [
        house.house_owner_from_planet_positions(planet_positions, h)
        for h in [(asc_house + 8) % 12, (asc_house + 9) % 12]
    ]
    dkchk = all(
        [
...
def vipareetha_raja_yoga(p_to_h, raja_yoga_planet1, raja_yoga_planet2):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    vrchk1 = [
        ([p_to_h[rp] == dh for dh in house.dushthanas_of_the_raasi(asc_house)])
        for rp in [raja_yoga_planet1, raja_yoga_planet2]
    ]
...
def vipareetha_raja_yoga_from_planet_positions(
    planet_positions, raja_yoga_planet1, raja_yoga_planet2
):
    """
    Checks if given two raja yoga planets also for vipareetha raja yoga/
...
def neecha_bhanga_raja_yoga(p_to_h, raja_yoga_planet1, raja_yoga_planet2):
    ...
    house_to_planet_list = utils.get_house_to_planet_dict_from_planet_to_house_dict(
        p_to_h
    )
    # p_to_h = utils.get_planet_to_house_dict_from_chart(house_to_planet_list)
    ...
    " Rule-1"
    chk1_1 = const.house_strengths_of_planets[raja_yoga_planet1][
        rp1_rasi
    ] <= const._DEFIBILATED_NEECHAM and (
        const.house_strengths_of_planets[rp1_lord][rp1_rasi] >= const._EXALTED_UCCHAM
        or rp1_rasi in kendra_from_moon
    )
    chk1_2 = const.house_strengths_of_planets[raja_yoga_planet2][
        rp2_rasi
    ] <= const._DEFIBILATED_NEECHAM and (
        const.house_strengths_of_planets[rp2_lord][rp2_rasi] >= const._EXALTED_UCCHAM
        or rp2_rasi in kendra_from_moon
    )
    ...
    chk2_2 = (
        const.house_strengths_of_planets[raja_yoga_planet1][rp1_rasi]
        >= const._EXALTED_UCCHAM
    ) and (
        const.house_strengths_of_planets[raja_yoga_planet2][rp2_rasi]
    ...
    " Rule 3"
    chk3_1 = (
        const.house_strengths_of_planets[raja_yoga_planet1][rp2_rasi]
        <= const._DEFIBILATED_NEECHAM
    ) and (
        str(raja_yoga_planet1)
        in house.graha_drishti_of_the_planet(house_to_planet_list, rp1_lord)
    )
    chk3_2 = (
        const.house_strengths_of_planets[raja_yoga_planet2][rp2_rasi]
        <= const._DEFIBILATED_NEECHAM
        ...
        str(raja_yoga_planet1)
        in house.graha_drishti_of_the_planet(house_to_planet_list, rp2_lord)
    )
    chk3 = chk3_1 or chk3_2
    return chk3


def neecha_bhanga_raja_yoga_from_planet_positions(
    planet_positions, raja_yoga_planet1, raja_yoga_planet2
):
    """
    Checks if given raja yoga pairs form neecha bhanga raja yoga
...
def check_other_raja_yoga_2(jd, place, divisional_chart_factor=1):
    ...
    chk3_1 = (
        const.house_strengths_of_planets[ak][p_to_h[ak]] > const._FRIEND
        and const.house_strengths_of_planets[pk][p_to_h[pk]] > const._FRIEND
    )
    chk3_2 = (
        const.house_strengths_of_planets[lagna_lord][fifth_house] > const._FRIEND
        and const.house_strengths_of_planets[fifth_lord][asc_house] > const._FRIEND
    )
...
def check_other_raja_yoga_3(jd, place, divisional_chart_factor=1):
    ...
    chk = any(
        [
            h1 == h2
            for h1 in [p_to_h[ninth_lord], p_to_h[ak]]
            for h2 in [(asc_house + h) % 12 for h in [0, 4, 6]]
        ]
    )
    pass
    ...
    def raja_yoga_tests():
        ...
        for p1, p2 in ry_pairs:
            print(
                ...
                p1,
                p2,
                neecha_bhanga_raja_yoga_from_planet_positions(planet_positions, p1, p2),
            )
        chart_14_rajiv_gandhi = [
            "",
            "",
            "6",
            "7",
        ...
        for p1, p2 in ry_pairs:
            ...
            print(
                ...
                p1,
                p2,
                dharma_karmadhipati_raja_yoga_from_planet_positions(
        ...
        for p1, p2 in ry_pairs:
            ...
            print(
                ...
                p1,
                p2,
                vipareetha_raja_yoga_from_planet_positions(planet_positions, p1, p2),
            )
