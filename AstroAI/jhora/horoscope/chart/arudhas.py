...
def bhava_arudhas_from_planet_positions(planet_positions, arudha_base=0):
    ...
    for h in houses:
        lord_of_the_house = house.house_owner_from_planet_positions(
            planet_positions, h, check_during_dhasa=False
        )
        house_of_the_lord = p_to_h[lord_of_the_house]
        signs_between_house_and_lord = utils.count_rasis(h, house_of_the_lord)
        bhava_arudha_of_house = (
            house_of_the_lord + signs_between_house_and_lord - 1
        ) % 12
        signs_from_the_house = utils.count_rasis(
            h, bhava_arudha_of_house
        )  # ((bhava_arudha_of_house+1+12-h)%12)
...
def graha_arudhas_from_planet_positions(planet_positions):
    ...
    for p in range(const._planets_upto_ketu):
        ...
        if len(sign_owned_by_planet) > 1:
            sign_owned_by_planet = house.stronger_rasi_from_planet_positions(
                planet_positions, sign_owned_by_planet[0], sign_owned_by_planet[1]
            )
        else:
...
def graha_arudhas(chart):
    ...
    for p in range(const._planets_upto_ketu):
        ...
        sign_owned_by_planet = const.house_lords_dict[p]
        if len(sign_owned_by_planet) > 1:
            sign_owned_by_planet = house.stronger_rasi(
                h_to_p, sign_owned_by_planet[0], sign_owned_by_planet[1]
            )
        else:
            sign_owned_by_planet = sign_owned_by_planet[0]
        count_to_strong = (sign_owned_by_planet + 1 + 12 - house_of_the_planet) % 12
