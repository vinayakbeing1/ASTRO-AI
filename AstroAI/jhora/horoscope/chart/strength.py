...
_DOB_EPOCH = (1900, 1, 1)
_TOB_EPOCH = (0, 0, 0)
_PLACE_EPOCH = drik.Place("Ujjain,India", 23.18, 76, 5.5)
_EPOCH_YEAR = 1900
_JD_EPOCH = utils.julian_day_number(_DOB_EPOCH, _TOB_EPOCH)
_DAYS_FROM_EPOCH = (
    lambda jd, place: jd
    - _JD_EPOCH
    + (_PLACE_EPOCH.longitude - place.longitude) / 15 / 24
)
is_leap_year = lambda year: (
    ((year % 4 == 0) and (year % 100 != 0)) or (year % 400 == 0)
)
leap_count_between_years = (
    lambda start, end, include_both=False: [
        is_leap_year(year) for year in range(start, end + 1)
    ].count(True)
    if include_both
    else [is_leap_year(year) for year in range(start + 1, end)].count(True)
)
list_of_leap_years_between_years = (
    lambda start, end, include_both=False: [
        year for year in range(start, end + 1) if is_leap_year(year)
    ]
    if include_both
    else [year for year in range(start + 1, end) if is_leap_year(year)]
)
planet_mean_positions_at_epoch_ujjain_1900 = [
    257.4568,
    -1,
    270.22,
    164,
...
planet_speed_at_epoch_ujjain_1900 = [
    0.9856,
    -1,
    0.524,
    4.0923,
...
planet_correction_factors_per_year_since_epoch = [
    (1, 0, 0),
    (1, 0, 0),
    (1, 0, 0),
    (1, 6.67, -0.00133),
...
planet_longitude_correction = lambda planet_index, years_since_epoch: planet_correction_factors_per_year_since_epoch[
    planet_index
][
    0
] * (
    planet_correction_factors_per_year_since_epoch[planet_index][1]
    + planet_correction_factors_per_year_since_epoch[planet_index][2]
    * years_since_epoch
)
...
def harsha_bala(dob, tob, place, divisional_factor=1):
    ...
    planet_positions = charts.divisional_chart(
        jd, place, divisional_chart_factor=divisional_factor
    )
    ...
    for p in range(7):
        ...
        if (
            const.house_strengths_of_planets[p][h_p] > const._FRIEND
            or h_p in const.house_lords_dict[p]
        ):  # Exhalted or Own
            harsha_bala[p] += 5
        " Rule-3 Feminine"
        if p in const.feminine_planets and h_f_a in const.harsha_bala_feminine_houses:
            ...
            p in const.masculine_planets and h_f_a in const.harsha_bala_masculine_houses
        ):
            harsha_bala[p] += 5
        "Rule-4 "
...
def _kshetra_bala(p_to_h_of_rasi_chart):
    ...
    for p in range(7):
        ...
        if const.house_strengths_of_planets[p][h_p] > const._FRIEND:
            ...
            const.house_strengths_of_planets[p][h_p] == const._ENEMY
    ...
    for dcf in sv:
        pp = (
            charts.divisional_chart(
                jd, place, ayanamsa_mode=ayanamsa_mode, divisional_chart_factor=dcf
            )
            if dcf != 2
...
def _sapthavargaja_bala_2(planet_positions, dcf, compound_relations):
    ...
    for p, (h, _) in planet_positions[1:8]:
        ...
        if h == const.moola_trikona_of_planets[p] and dcf == 1:  # Moola Trinkona Rasi
            ...
            const.house_strengths_of_planets[p][h] == const._OWNER_RULER
        ):  # Swastha Rasi
            sb[p] = 30
...
def _sapthavargaja_bala_1(planet_positions, dcf):
    ...
    sb_fac = {
        const._ADHISATHRU_GREATENEMY: 1.875,
        const._SATHRU_ENEMY: 3.75,
        const._SAMAM_NEUTRAL: 7.5,
        const._MITHRA_FRIEND: 15,
...
def _kendra_bala(rasi_planet_positions):
    ...
    for p, (h, _) in rasi_planet_positions[
        1:8
    ]:  # exclude 0th element Lagnam and Rahu/Ketu
...
def _uchcha_bala(planet_positions):
    ...
    for p, (h, long) in planet_positions[
        ...
        if const.use_saravali_formula_for_uccha_bala:
            ...
            ub.append(
                ubv
            )  # Saravali formula #https://saravali.github.io/astrology/bala_sthana.html#uchcha
...
def pancha_vargeeya_bala(jd, place):
    ...
    p_to_h_of_rasi_chart = utils.get_planet_house_dictionary_from_planet_positions(
        rasi_chart
    )
    ...
    p_to_h_of_drekkana_chart = utils.get_planet_house_dictionary_from_planet_positions(
        drekkana_chart
    )
    db = _drekkana_bala(p_to_h_of_drekkana_chart)
    navamsa_chart = charts.divisional_chart(jd, place, divisional_chart_factor=9)
    p_to_h_of_navamsa_chart = utils.get_planet_house_dictionary_from_planet_positions(
        navamsa_chart
    )
...
def dwadhasa_vargeeya_bala(jd, place):
    ...
    for dvf in range(1, 13):  # D1-D12 charts
        planet_positions = charts.divisional_chart(
            jd, place, divisional_chart_factor=dvf
        )
...
def _paksha_bala(jd, place, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE):
    ...
    cht_benefics, cht_malefics = charts.benefics_and_malefics(
        jd, place, ayanamsa_mode=ayanamsa_mode, exclude_rahu_ketu=True
    )
...
def _tribhaga_bala(jd, place):
    ...
    if tobh >= srh and tobh < srh + dlinc:  # 1st part of day
        ...
        tbp[1] = 60  # Moon
    elif (tobh >= ssh + nlinc and tobh < 24) or (
        tobh >= 0 and tobh < srh - nlinc
    ):  # 2nd part of night
        tbp[5] = 60  # Venus
...
def _abdadhipathi(jd, place):
    ...
    day = (
        int(_ahargana_days // 360) * 3 + 1
    ) % 7  # Add 1 get 1st day of the next kali year
...
def _yuddha_bala(jd, place):
    ...
    dia_diff = abs(
        const.planets_disc_diameters[indices[0]]
        - const.planets_disc_diameters[indices[1]]
    )
...
def _ishta_phala(jd, place):
    ...
    ip_score = {
        const._ADHIMITRA_GREATFRIEND: 22,
        const._MITHRA_FRIEND: 15,
        const._SAMAM_NEUTRAL: 8,
        const._ADHISATHRU_GREATENEMY: 4,
...
def _subha_rashmi(jd, place):
    planet_positions = charts.rasi_chart(jd, place)
    cr = _cheshta_rashmi(jd, place)
    ur = _uccha_rashmi(planet_positions)
...
def __drik_bala_calc_1(dk_p1_p2, p1, p2):
    ...
    if _DEBUG_:
        print("final", p1, p2, "drishti value", dk_p1_p2_new)
    return dk_p1_p2_new
...
def planet_aspect_relationship_table(planet_positions, include_houses=False):
    ...
    for p1 in range(9):  # Aspected Planet
        ...
        for p2 in range(9):  # Aspecting Planet
            ...
            if _DEBUG_:
                print("drishti angle", p2, p2_long, p1, p1_long, dk_p1_p2)
...
def _drik_bala(jd, place, ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE):
    ...
    dkp = [0 for _ in range(7)]
    dkm = [0 for _ in range(7)]
...
def _bhava_dig_bala(jd, place):
    ...
    bm = drik.bhaava_madhya(jd, place)
    brl = {
        0: const.nara_rasi_longitudes,
        3: const.jalachara_rasi_longitudes,
        9: const.chatushpada_rasis,
        6: const.keeta_rasis,
    }
    chk = []
    for k, v in brl.items():
        chk += list(
            set(
                [
                    ((k + h) % 12, abs(60 - abs(h) * 10))
                    for h in range(-7, 7)
                    for l1, l2 in v
                    if bm[(k + h) % 12] >= l1 and bm[(k + h) % 12] <= l2
                ]
            )
        )
...
def _bhava_drik_bala(jd, place):
    ...
    for planet in range(7):
        ...
        planet_house_aspects[planet] = [
            int(p)
            for p in planet_house_aspects[planet]
            if p not in [const._ascendant_symbol, "7", "8"]
        ]
    bm = drik.bhaava_madhya(jd, place)
    for h in range(12):  # Aspected Planet
        h_mid = bm[h]
    ...
    dkp = [0 for _ in range(12)]
    dkm = [0 for _ in range(12)]
    dk_final = [0 for _ in range(12)]
    for row in range(12):
        for col in range(7):
...
def get_planet_mean_longitude_using_epoch_table(jd, place, planet_index=0):
    ...
    year_jd = utils.jd_to_gregorian(jd)[0]
    ...
    tens_and_units = (
        whole_days_from_epoch % 100
    )  # Combine tens and units if tens is missing
    ...
    value_ten_thousands = _ujjain_epoch_table_for_planets.get(
        ten_thousands, [0, 0, 0, 0]
    )[
        -1
    ]  # 4th column
    value_thousands = _ujjain_epoch_table_for_planets.get(thousands, [0, 0, 0, 0])[
        -2
    ]  # 3rd column
    value_hundreds = _ujjain_epoch_table_for_planets.get(hundreds, [0, 0, 0, 0])[
        -3
    ]  # 2nd column
    ...
    if has_tens:
        ...
        value_tens = _ujjain_epoch_table_for_planets.get(tens, [0, 0, 0, 0, 0])[
            1
        ]  # 5th column (tens)
        value_units = _ujjain_epoch_table_for_planets.get(units, [0, 0, 0, 0])[
            0
        ]  # 1st column (units)
        ...
        units_row_value = _ujjain_epoch_table_for_planets.get(units_row, [0, 0, 0, 0])[
            0
        ]  # Units column
        combined_units_value = 10 * units_row_value

    value_decimal = (
        decimal_part * _ujjain_epoch_table_for_planets.get(1, [0, 0, 0, 0])[0]
    )  # Decimal uses 1st column
    ...
    _planet_longitude_correction = planet_longitude_correction(
        planet_index, years_since_epoch
    )
...
def get_planet_mean_longitude(jd, place, planet_index=0):
    ...
    planet_mean_position_at_jd = (
        (
            planet_mean_positions_at_epoch_ujjain_1900[planet_index]
            + days_from_epoch * planet_speed_at_epoch
        )
        + _planet_longitude_correction
    ) % 360
...
def _cheshta_bala_new(jd, place, use_epoch_table=False):
    ...
    for p in [
        ...
        p_id = drik.planet_list.index(p)
        mean_long = (
            get_planet_mean_longitude_using_epoch_table(jd, place, p_id)
            if use_epoch_table
...
if __name__ == "__main__":
    ...
    tob = (1, 30, 0)
    place = drik.Place("VPJainExample", 28 + 39 / 60, 77 + 13 / 60, 5.5)
    dob = drik.Date(1974, 7, 29)
    tob = (5, 5, 0)
    place = drik.Place("UserExample", 27 + 18 / 60, 78.0, 5.5)
    jd = utils.julian_day_number(dob, tob)
    ...
    pp = charts.rasi_chart(jd, place)
    print("BVRamanExample Planet Positions", pp)
    print(_cheshta_bala_new(jd, place, use_epoch_table=True))
    print(_cheshta_bala_new(jd, place, use_epoch_table=False))
