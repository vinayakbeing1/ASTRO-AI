...
class Calendar:
    ...
    def __init__(
        self,
        date_as_tuple: tuple = None,
        time_as_tuple: tuple = None,
        place_as_tuple: tuple = None,
    ):
        ...
        if place_as_tuple:
            ...
            self._place_timezone = utils.get_place_timezone_offset(
                self._place_latitude, self._place_longitude
            )
        self._place = Place(
            self._place_name,
            self._place_latitude,
        ...
        self._julian_day_at_time = swe.julday(
            self._date[0],
            self._date[1],
            self._date[2],
            self._time[0] + self._time[1] / 60.0 + self._time[2] / 3600.0,
        )
    ...
    def samvastara(self, north_indian_tradition=False):
        return lunar_year(
            self._julian_day_at_time,
            self.lunar_month()[0],
            north_indian_tradition=north_indian_tradition,
        )
    ...
    def get_ayanamsa(self):
        self._ayanamsa_value = utils.to_dms(
            get_ayanamsa_value(self._julian_day_at_time), is_lat_long="plong"
        )
    ...
    def sidereal_longitude(self, planet: int):
        if planet in planet_list:
            ...
            w = (
                "Planet should be in the range of "
                + str(min(planet_list))
                + " and "
                + str(max(planet_list))
            )
...
def tithi(jd, place):
    ...
    approx_end = utils.inverse_lagrange(x, y, degrees_left)
    start_time = (
        rise + approx_end - jd_utc
    ) * 24 + place.timezone  # jd changed to jd_utc 2.0.3
    if start_time < 0:
...
def raasi(jd, place):
    ...
    nirayana_long = lunar_longitude(
        jd
    )  # longitudes[0] # Fixed 1.1.0 lunar longitude at JD and NOT at RISE
...
def nakshatra(jd, place):
    ...
    offsets = [0.0, 0.25, 0.5, 0.75, 1.0]
    longitudes = [
        sidereal_longitude(rise + t, const._MOON) for t in offsets
    ]  # Fixed 1.1.0 lunar longitude from sunrise to next sunrise
    nirayana_long = lunar_longitude(
        jd
    )  # longitudes[0] # Fixed 1.1.0 lunar longitude at JD and NOT at RISE
    ...
    ends = (
        rise - jd_utc + approx_end
    ) * 24 + tz  # """ Changed to jd_utc to get correct end time for the jd -  2.0.3 """
...
def _raasi(jd, place):
    ...
    raasi_ends_at = (
        sun_rise_time + raasi_remain_degrees * 24 / (moon_daily_motion)
    ) % 360
...
def durmuhurtam(jd, place):
    ...
    srise = swe.rise_trans(
        ...
        const._SUN,
        geopos=(lon, lat, 0.0),
        rsmi=_rise_flags + const._CALC_RISE,
    )[1][0]
    night_dur = srise - sset
...
def planetary_positions(jd, place):
    ...
    for planet in planet_list:
        ...
        if planet == const._KETU:
            nirayana_long = ketu(
                sidereal_longitude(jd, const._RAHU)
            )  # V2.3.1 planetary positions at JD not JD_UTC
        else:  # Ketu
            nirayana_long = sidereal_longitude(
                jd, planet
            )  # V2.3.1 planetary positions at JD not JD_UTC
...
def dasavarga_from_long(longitude, divisional_chart_factor):
    ...
    if divisional_chart_factor not in const.division_chart_factors:
        raise ValueError(
            "Wrong divisional_chart_factor",
            divisional_chart_factor,
            " Valid value:",
            const.division_chart_factors,
        )
...
def _vimsottari_bhukti(maha_lord, start_date):
    ...
    for i in range(9):
        ...
        factor = (
            const.mahadasa[lord]
            * const.mahadasa[maha_lord]
            / const.human_life_span_for_dhasa
        )
...
def _vimsottari_antara(maha_lord, bhukti_lord, start_date):
    ...
    for i in range(9):
        retval[lord] = start_date
        factor = const.mahadasa[lord] * (
            const.mahadasa[maha_lord] / const.human_life_span_for_dhasa
        )
...
def udhayadhi_nazhikai(birth_time, sunrise_time):
    ...
    sunrise_time_hrs = (
        sunrise_time[0] + sunrise_time[1] / 60.0 + sunrise_time[2] / 3600.0
    )
...
"""
kaala_longitude = lambda dob, tob, place, divisional_chart_factor=1: upagraha_longitude(
    dob,
    tob,
    place,
    planet_index=0,
...
mrityu_longitude = (
    lambda dob, tob, place, divisional_chart_factor=1: upagraha_longitude(
        dob,
        tob,
        place,
        planet_index=2,
...
def upagraha_longitude(
    dob, tob, place, planet_index, divisional_chart_factor=1, upagraha_part="middle"
):
    """
...
bhava_lagna = lambda jd, place, time_of_birth_in_hours, divisional_chart_factor: special_ascendant(
    jd,
    place,
    time_of_birth_in_hours,
    lagna_rate_factor=1.0,
    divisional_chart_factor=divisional_chart_factor,
)
hora_lagna = lambda jd, place, time_of_birth_in_hours, divisional_chart_factor=1: special_ascendant(
    jd,
    place,
    time_of_birth_in_hours,
...
ghati_lagna = lambda jd, place, time_of_birth_in_hours, divisional_chart_factor=1: special_ascendant(
    jd,
    place,
    time_of_birth_in_hours,
...
def special_ascendant(
    jd, place, time_of_birth_in_hours, lagna_rate_factor=1.0, divisional_chart_factor=1
):
    """
    ...
    srise = sunrise(jd_utc, place)[1]
    srise_jd = sunrise(jd, place)[
        2
    ]  # V2.3.1 Get sunrise JD - as we need sun longitude at sunrise
    [sun_rise_hours, sun_rise_minute, sun_rise_second] = [
        int(ss) for ss in srise.replace(" AM", "").replace(" PM", "").split(":")
    ]
    ...
    sun_long = solar_longitude(
        srise_jd
    )  # V2.3.1 Use JD at sunrise to get sun longitude at sunrise
...
def next_sankranti_date(panchanga_date, place):
    ...
    return (
        utils.panchanga_date_to_tuple(sank_date),
        solar_hour1,
        tamil_month,
        tamil_day,
