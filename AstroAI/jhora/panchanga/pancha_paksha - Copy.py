...
_DAYNIGHT_INDEX = 3
_MAIN_BIRD_INDEX = 4
_MAIN_ACTIVITY_INDEX = 5
_SUB_BIRD_INDEX = 6
_SUB_ACTIVITY_INDEX = 7
_DURATION_FACTOR = 8
...
pancha_pakshi_duration_list = [
    [ele / 144 for ele in row] for row in _pancha_pakshi_duration_list
]
...
_next_list_item = lambda _list, _current, dir=1, step=1: _list[
    (_list.index(_current) + dir * step) % len(_list)
]
...
class PanchaPakshiSastraWidget(QWidget):
    def __init__(
        ...
        birth_bird_index,
        headers=None,
        parent_level_list=None,
        child_level_list=None,
        hide_headers=True,
    ):
        super().__init__()
        ...
        if parent_level_list:
            column_count = max(
                len(row)
                for row in parent_level_list
                + [item for sublist in child_level_list for item in sublist]
            )
        else:
        ...
        self._tob_text.setToolTip(
            "Enter time in the format HH:MM:SS if afternoon use 12+ hours"
        )
        ...
        self.bird_combo.addItems(
            [utils.resource_strings[b + "_str"] for b in pancha_pakshi_birds]
        )
    ...
    def _calculate_results(self):
        ...
        (
            ...
            top_level_list,
            child_level_list,
        ) = _construct_pancha_pakshi_information(
            dob, tob, place, nakshathra_bird_index=bird_index
        )
        self.populate_tree(
            headers, top_level_list, child_level_list, hide_headers=False
        )
...
def _create_pancha_paksha_db():
    _pp_db = []
    print(
        "nak_bird_index,week_day_index,paksha_index,daynight_index,main_bird_index,main_activity_index,sub_bird_index,sub_activity_index,duration_factor"
    )
    for nak_bird_index, _day_list in enumerate(pancha_paksha_activity_list):
        for week_day_index, week_day_list in enumerate(_day_list):
            _sp_day_acts = week_day_list[:5]
            _sp_night_acts = week_day_list[5:10]
            _kp_day_acts = week_day_list[10:15]
            _kp_night_acts = week_day_list[15:20]
            main_bird_index = nak_bird_index
            paksha_index = 0
            daynight_index = 0
            for timestep1_index, main_activity_index in enumerate(_sp_day_acts):
                sub_acts = (
                    _sp_day_acts[timestep1_index:] + _sp_day_acts[:timestep1_index]
                )
                sub_bird_index = main_bird_index
                for sub_activity_index in sub_acts:
                    duration_factor = pancha_pakshi_duration_list[daynight_index - 1][
                        sub_activity_index - 1
                    ]
                    print(
                        nak_bird_index,
                        week_day_index,
                    ...
                    sub_bird_index = _next_list_item(
                        bird_list, sub_bird_index, dir=1, step=1
                    )
            daynight_index = 1
            for timestep1_index, main_activity_index in enumerate(_sp_night_acts):
                sub_acts = (
                    _sp_night_acts[timestep1_index:] + _sp_night_acts[:timestep1_index]
                )
                sub_bird_index = main_bird_index
                for sub_activity_index in sub_acts:
                    duration_factor = pancha_pakshi_duration_list[daynight_index - 1][
            ...
            for timestep1_index, main_activity_index in enumerate(_kp_day_acts):
                sub_acts = (
                    _kp_day_acts[timestep1_index:] + _kp_day_acts[:timestep1_index]
                )
                sub_bird_index = main_bird_index
                for sub_activity_index in sub_acts:
                    duration_factor = pancha_pakshi_duration_list[daynight_index - 1][
                    ...
                    sub_bird_index = _next_list_item(
                        bird_list, sub_bird_index, dir=1, step=3
                    )
            daynight_index = 1
            for timestep1_index, main_activity_index in enumerate(_kp_night_acts):
                sub_acts = (
                    _kp_night_acts[timestep1_index:] + _kp_night_acts[:timestep1_index]
                )
                sub_bird_index = main_bird_index
                for sub_activity_index in sub_acts:
                    duration_factor = pancha_pakshi_duration_list[daynight_index - 1][
...
def get_matching_pancha_pakshi_data_from_db(bird_index, weekday_index, paksha_index):
    print(
        "bird_index,weekday_index,paksha_index",
        bird_index - 1,
        weekday_index - 1,
        paksha_index - 1,
    )
    # Open db
    pp_db = pd.read_csv(
        PP_DB_FILE, header=None, index_col=None, encoding="utf-8", dtype=dtype_dict
    )
...
def _construct_pancha_pakshi_information(
    dob=None, tob=None, place=None, nakshathra_bird_index=None
):
    _DEBUG_ = False
    jd = utils.julian_day_number(dob, tob)
    ...
    day_inc = day_length / 5.0
    night_inc = night_length / 5.0
    if _DEBUG_:
        print(
            "nakshathra_bird_index",
            nakshathra_bird_index,
            "weekday_index",
            weekday_index,
            "paksha_index",
    ...
    print(
        utils.resource_strings[pancha_pakshi_birds[nakshathra_bird_index - 1] + "_str"],
        utils.DAYS_LIST[weekday_index - 1],
        utils.PAKSHA_LIST[paksha_index - 1],
    )
    if _DEBUG_:
        print(
            "day_length", day_length, "night_length", night_length, day_inc, night_inc
        )
    result_list = get_matching_pancha_pakshi_data_from_db(
        nakshathra_bird_index, weekday_index, paksha_index
    )
    headers = [
        "From_Time",
        "To_Time",
    ...
    for row in range(0, len(result_list), 5):
        ...
        if _DEBUG_:
            print("row=", row, nbi, wdi, pi, dni, mbi, mai, sbi, sai, df)
        ...
        time_from1 = (
            ("day_sun.png", time_from)
            if dni == 0
            else ("moon_with_star.png", time_from)
        )
        duration = (
            str(round(time_inc * 24, 2)) + " " + utils.resource_strings["hours_str"]
        )
        ...
        tlist = [
            ...
            time_to,
            duration,
            (main_bird_image, main_bird),
            (main_act_image, main_act),
            sub_bird,
            sub_act,
            power_factor,
            effect,
        ...
        for irow in range(row, row + 5):
            ...
            if _DEBUG_:
                print("c-row=", irow, nbi, wdi, pi, dni, mbi, mai, sbi, sai, df)
            time_inc = day_inc / 24 if dni == 0 else night_inc / 24
            ...
            duration = (
                str(round(df * time_inc * 24 * 60))
                + " "
                + utils.resource_strings["minutes_str"]
            )
            sub_bird = utils.resource_strings[pancha_pakshi_birds[int(sbi)] + "_str"]
            sub_bird_image = pancha_pakshi_images[int(sbi)]
            sub_act = utils.resource_strings[
                pancha_pakshi_activities[int(sai)] + "_str"
            ]
            ...
            cll = [
                ...
                (main_bird_image, main_bird),
                (main_act_image, main_act),
                (sub_bird_image, sub_bird),
                (sub_act_image, sub_act),
                power_factor,
                effect,
                rating,
            ]
            if _DEBUG_:
                print("c-row", irow, cll)
            clist.append(cll)
...
if __name__ == "__main__":
    ...
    print(
        "bird",
        bird_index,
        utils.resource_strings[pancha_pakshi_birds[bird_index - 1] + "_str"],
    )
    dt = datetime.datetime.now().timetuple()
    dob = dt[0:3]
    tob = dt[3:6]
    headers, top_level_list, child_level_list = _construct_pancha_pakshi_information(
        dob, tob, place, nakshathra_bird_index=bird_index
    )
    app = QApplication(sys.argv)
    demo = PanchaPakshiSastraWidget(
        birth_bird_index=bird_index,
        headers=headers,
        parent_level_list=top_level_list,
