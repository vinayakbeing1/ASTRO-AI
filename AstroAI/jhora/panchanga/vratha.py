...
_yoga_tag = "Yoga"
_sukla_paksha_tag = "Sukla Paksha"
_krishna_paksha_tag = "Krishna Paksha"
_tithi_tag = "Tithi"
...
amavasya_dates = (
    lambda panchanga_place, panchanga_start_date, panchanga_end_date: tithi_dates(
        panchanga_place,
        panchanga_start_date,
        panchanga_end_date,
        _amavasya_tithi,
        tag_t="amavasya",
    )
...
sankatahara_chathurthi_dates = (
    lambda panchanga_place, panchanga_start_date, panchanga_end_date: tithi_dates(
        panchanga_place,
        panchanga_start_date,
        panchanga_end_date,
        _sankatahara_chathurthi_tithi,
        tag_t="sankatahara_chathurthi",
    )
)
shivarathri_dates = (
    lambda panchanga_place, panchanga_start_date, panchanga_end_date: tithi_dates(
...
srartha_yoga_dates = (
    lambda panchanga_place, panchanga_start_date, panchanga_end_date: yoga_dates(
        panchanga_place,
        panchanga_start_date,
        panchanga_end_date,
        _srartha_yogas,
...
_manvaadhi_tithis = {
    "swayambhuva_manvadhi": (3, 1),
    "swarochisha_manvadhi": (15, 1),
    "uttama_manvadhi": (15, 8),
    "tamasa_manvadhi": (12, 8),
    "raivata_manvadhi": (10, 4),
    "chakshusha_manvadhi": (15, 4),
    "vaivaswata_manvadhi": (15, 3),
    "savarni_manvadhi": (15, 12),
    "daksha_savarni_manvadhi": (9, 7),
    "brahma_savarni_manvadhi": (7, 11),
    "dharma_savarni_manvadhi": (11, 10),
    "rudra_savarni_manvadhi": (3, 6),
    "daiva_savarni_manvadhi": (30, 5),
...
ashtaka_dates = lambda panchanga_place, panchanga_start_date, panchanga_end_date=None: _ashtaka_manvaadhi_dates(
    panchanga_place,
    panchanga_start_date,
    panchanga_end_date,
    _ashtaka_tithis,
...
manvaadhi_dates = lambda panchanga_place, panchanga_start_date, panchanga_end_date=None: _ashtaka_manvaadhi_dates(
    panchanga_place,
    panchanga_start_date,
    panchanga_end_date,
    _manvaadhi_tithis,
...
yugadhi_dates = lambda panchanga_place, panchanga_start_date, panchanga_end_date=None: _ashtaka_manvaadhi_dates(
    panchanga_place,
    panchanga_start_date,
    panchanga_end_date,
    _yugadhi_tithis,
...
vinayaka_chathurthi_dates = lambda panchanga_place, panchanga_start_date, panchanga_end_date=None: _ashtaka_manvaadhi_dates(
    panchanga_place,
    panchanga_start_date,
    panchanga_end_date,
    _vinayaka_chathurthi_tithis,
    tag="vinayaka_chathurthi",
)


def _ashtaka_manvaadhi_dates(
    panchanga_place,
    panchanga_start_date,
    panchanga_end_date=None,
    tithi_tamil_month_tuples=None,
    tag=None,
):
    ...
    if _debug_print:
        print("_ashtaka_manvaadhi_dates before sorting", results)
    if panchanga_end_date == None:
        results = results[:1]
    results = sorted([results], key=lambda x: x[0])
    results = utils.flatten_list(results)
    if _debug_print:
        print("_ashtaka_manvaadhi_dates after sorting and flattening", results)
    return results


def special_vratha_dates(
    panchanga_place,
    panchanga_start_date,
    panchanga_end_date=None,
    vratha_type=None,
    vratha_index_list=None,
):
    """
    ...
    if vratha_type.lower() == "tithi" or vratha_type.lower() == "nakshathra":
        ...
        return eval(vratha_function)(
            panchanga_place, panchanga_start_date, panchanga_end_date
        )
...
def pradosham_dates(panchanga_place, panchanga_start_date, panchanga_end_date=None):
    ...
    for pdate, _, _, tag_t in pdates:
        ...
        day = panchanga.vaara(cur_jd)
        tag = _pradosha_list[day] + " " + res["pradosham_str"] + " / " + tag_t
...
def tithi_dates(
    ...
    res = utils.resource_strings
    if tag_t != "":
        tag_t = " / " + res[tag_t + "_str"]
    _start_date = panchanga.Date(
        panchanga_start_date.year, panchanga_start_date.month, panchanga_start_date.day
    )
    if panchanga_end_date == None:
        _end_date = utils.next_panchanga_day(_start_date, 365)
    else:
        _end_date = panchanga.Date(
            panchanga_end_date.year, panchanga_end_date.month, panchanga_end_date.day
        )
    cur_date = _start_date
    cur_jd = swe.julday(
        panchanga_start_date.year,
        panchanga_start_date.month,
        panchanga_start_date.day,
        sunrise_hours,
    )
...
def nakshathra_dates(
    panchanga_place,
    panchanga_start_date,
    panchanga_end_date=None,
    nakshathra_index_list=None,
):
    res = utils.resource_strings
    _start_date = panchanga.Date(
        panchanga_start_date.year, panchanga_start_date.month, panchanga_start_date.day
    ...
    while cur_jd < end_jd:
        ...
        cur_date = panchanga.jd_to_gregorian(cur_jd)[0:3]
        if (
            current_nakshathra[0] in nakshathra_index_list
            and cur_date not in special_vratha_dates
        ):
            ...
            if ends_at < 0:
                ...
                new_cur_date = utils.previous_panchanga_day(
                    panchanga.Date(cur_date[0], cur_date[1], cur_date[2]),
                    minus_days=days,
                )
...
def yoga_dates(
    ...
    panchanga_start_date,
    panchanga_end_date=None,
    yoga_index_list=None,
    tag_y="",
):
    res = utils.resource_strings
    if tag_y != "":
        tag_y = " / " + res[tag_y + "_str"]
    _start_date = panchanga.Date(
        panchanga_start_date.year, panchanga_start_date.month, panchanga_start_date.day
    )
    if panchanga_end_date == None:
    ...
    while cur_jd < end_jd:
        ...
        if cur_yoga[0] in yoga_index_list:
            ends_at = cur_yoga[1]
            tag = utils.YOGAM_LIST[cur_yoga[0] - 1] + " " + res["yogam_str"]
            if tag_y not in tag:
                tag += tag_y
...
def _get_planets_in_conjunction_same_house(
    planet_positions, minimum_separation_longitude
):
    """Exlcude Lagnam, Sun, Moon, Rahu and Ketu  planet_positions[3:8]"""
    planets_in_conjunction = []
    ...
    for h, pls in enumerate(h_to_p):
        ...
        for p1, p2 in coms:
            p1_long = (
                planet_positions[p1 + 1][1][0] * 30 + planet_positions[p1 + 1][1][1]
            )
            p2_long = (
                planet_positions[p2 + 1][1][0] * 30 + planet_positions[p2 + 1][1][1]
            )
            if abs(p1_long - p2_long) < minimum_separation_longitude:
...
def conjunctions(
    ...
    panchanga_start_date,
    panchanga_end_date,
    minimum_separation_longitude,
    planets_in_same_house=False,
):
    ...
    while cur_jd < end_jd:
        ...
        if planets_in_same_house:
            result_local = _get_planets_in_conjunction_same_house(
                planet_positions, minimum_separation_longitude
            )
        else:
            result_local = _get_planets_in_conjunction(
                planet_positions, minimum_separation_longitude
            )
        if result_local:
...
def _get_conjunction_time_1(jd, place, p1, p2):
    ...
    n = 24
    offsets = [s / n for s in range(-n, n)]
    ...
    for t in offsets:
        ...
        print(
            utils.to_dms((jd + t - jd_utc) * 24 + tz),
            utils.to_dms(p1_long, is_lat_long="plong"),
            utils.to_dms(p2_long, is_lat_long="plong"),
        )
        if abs(p1_long - p2_long) < 0.01:
...
def _get_conjunction_time(jd, place, p1, p2):
    ...
    n = 10
    offsets = [s / n for s in range(0, n)]
    rise = panchanga.sunrise(jd, place)[2]
    p1_long = []
    p2_long = []
    for t in offsets:
...
def search(
    ...
    if panchanga_end_date == None:
        ...
        _panchanga_end_date = panchanga.Date(
            panchanga_end_date.year, panchanga_end_date.month, panchanga_end_date.day
        )
    # Search for all festivals if all panchanga indices are None
    if all(
        x is None
        for x in (tithi_index, nakshathra_index, yoga_index, tamil_month_index)
    ):
        if _debug_print:
            print("search for festival from csv")
        fest_data = get_festivals_between_the_dates(
            panchanga_start_date,
            panchanga_end_date,
            panchanga_place,
            festival_name_contains=festival_name_contains,
        )
        if len(fest_data) > 0:
            for fdate, fest in fest_data:
                ...
                jd = utils.julian_day_number(
                    panchanga.Date(fdate[0], fdate[1], fdate[2]), (fdate[3], 0, 0)
                )
                sunrise_hours = panchanga.sunrise(jd, panchanga_place)[0]
                if len(fest) != 0:
                    _special_vratha_dates.append(
                        [
                            (
                                fdate,
                                sunrise_hours,
                                f["Festival_" + const._DEFAULT_LANGUAGE],
                            )
                            for f in fest
                        ][0]
                    )
    # Tithi search
    if tithi_index != None:
        tithi_results = tithi_dates(
            panchanga_place, panchanga_start_date, _panchanga_end_date, [tithi_index]
        )
        if _debug_print:
            print("tithi_results", tithi_results)
        if len(tithi_results) == 0:
            return []
    ...
    if nakshathra_index != None:
        if _debug_print:
            print("finding nakshathra dates ")
        if len(_special_vratha_dates) == 0:
            nakshathra_results = nakshathra_dates(
                panchanga_place,
                panchanga_start_date,
                panchanga_end_date,
                [nakshathra_index],
            ...
            if _debug_print:
                print("finding nakshathra dates from vratha dates")
            for t_date in _special_vratha_dates:
                ...
                if nakshathra_index in [nr[0], nr[3]]:
                    ...
                    nakshathra_results.append(
                        (p_date1, start_time, end_time, vratha_tag + " / " + nak_tag)
                    )  # (t_date,tag))
            if len(nakshathra_results) == 0:
                return []
            _special_vratha_dates = nakshathra_results
    ...
    if yoga_index != None:
        if _debug_print:
            print("finding yoga dates")
        if len(_special_vratha_dates) == 0:
            yoga_results = yoga_dates(
                panchanga_place, panchanga_start_date, panchanga_end_date, [yoga_index]
            )
            ...
            if _debug_print:
                print("finding yoga from vratha dates")
            for t_date in _special_vratha_dates:
                vratha_tag = t_date[-1]
                p_date1 = panchanga.Date(t_date[0][0], t_date[0][1], t_date[0][2])
                cur_jd = swe.julday(p_date1.year, p_date1.month, p_date1.day, 0.0)
                ...
                if yoga_index == yr[0]:
                    ...
                    end_time = yr[1]
                    yoga_results.append(
                        (p_date1, end_time, vratha_tag + " / " + yoga_tag)
                    )
    ...
    if tamil_month_index != None:
        if _debug_print:
            print("finding also tamil month dates", _special_vratha_dates)
        for sv_result in _special_vratha_dates:  # V2.3.0
            ...
            if _debug_print:
                print(t_date)  # ,ts_time,te_time)
            panchanga_date = panchanga.Date(t_date[0], t_date[1], t_date[2])
            tamil_month_day = panchanga.tamil_solar_month_and_date(
                panchanga_date, panchanga_place
            )
            if _debug_print:
                print(
                    t_date,
            ...
            if tamil_month_day[0] + 1 == tamil_month_index:
                ...
                tm_results.append(
                    (t_date, ts_time, te_time, vratha_tag + " / " + tm_tag)
                )  # V2.3.0
                if _debug_print:
                    print(
                        "found", t_date, tamil_month_day[0] + 1, tamil_month_day[1]
                    )  # ,lunar_month)
        if _debug_print:
    ...
    if _debug_print:
        print("_special_vratha_dates", _special_vratha_dates)
    _special_vratha_dates = [
        tup[:-1] + (tup[-1] + " / " + description,) for tup in _special_vratha_dates
    ]
    end_time = datetime.datetime.now()
    if _debug_print:
        print("search cpu time elapsed", end_time - start_time, " seconds")
...
def sankranti_dates(place, start_date, end_date=None):
    ...
    results = []
    jd_start = swe.julday(
        start_date.year, start_date.month, start_date.day, 9.0
    )  # get around cur_sunrise
    if end_date == None:
...
def sathyanarayana_puja_dates(
    panchanga_place, panchanga_start_date, panchanga_end_date=None
):
    return search(
        panchanga_place,
        panchanga_start_date,
        panchanga_end_date,
        tithi_index=_pournami_tithi[0],
...
def kaalashtami_dates(panchanga_place, panchanga_start_date, panchanga_end_date=None):
    return search(
        ...
        panchanga_start_date,
        panchanga_end_date,
        tithi_index=_krishna_ashtami,
        description=utils.resource_strings["kaalashtami_str"],
    )


def mahalaya_paksha_dates(
    panchanga_place, panchanga_start_date, panchanga_end_date=None
):
    res = utils.resource_strings
    mpds = search(
        ...
        panchanga_start_date,
        panchanga_end_date,
        tithi_index=_amavasya_tithi[0],
        tamil_month_index=_purattaasi,
    )
    if len(mpds) > 0:
        mpds = mpds[0]
    else:
    ...
    for d in range(1, _mahalaya_paksha_days):
        ...
        mpd.append(
            (
                panchanga.jd_to_gregorian(jd - d),
                cur_tithi[1],
                cur_tithi[2],
                res["mahalaya_paksha_str"] + " / " + utils.TITHI_LIST[cur_tithi[0] - 1],
            )
        )  # V2.3.0
        if (
            panchanga_end_date == None
            or cur_tithi[0] == _pournami_tithi[0]
            or (len(cur_tithi) > 3 and cur_tithi[3] == _pournami_tithi[0])
        ):
            mpd = sorted(mpd)
            return mpd
    mpd = sorted(mpd)
...
def srartha_dates(panchanga_place, panchanga_start_date, panchanga_end_date=None):
    ...
    if _debug_print:
        print("amavasya dates results", results)
    results += sankranti_dates(
        panchanga_place, panchanga_start_date, panchanga_end_date
    )  # ,True)
    if _debug_print:
        print("sankranthi dates results", results)
    results += mahalaya_paksha_dates(
        panchanga_place, panchanga_start_date, panchanga_end_date
    )
    if _debug_print:
        print("mahalaya dates results", results)
    results += yoga_dates(
        panchanga_place, panchanga_start_date, panchanga_end_date, _srartha_yogas
    )
    if _debug_print:
        print("yoga dates results", results)
    results += manvaadhi_dates(
        panchanga_place, panchanga_start_date, panchanga_end_date
    )
    if _debug_print:
        print("manvaadhi dates results", results)
    results += ashtaka_dates(panchanga_place, panchanga_start_date, panchanga_end_date)
    if _debug_print:
        print("ashtaka dates results", results)
    results += yugadhi_dates(panchanga_place, panchanga_start_date, panchanga_end_date)
    if _debug_print:
        print("yugadhi dates results", results)
    results = [utils.flatten_list([results])]
    if _debug_print:
        print("srartha dates results", results)
    results = utils.flatten_list(results)
    results = sorted(results, key=lambda x: x[0])
    if panchanga_end_date == None:
        return results[:1]
...
def moondraam_pirai_dates(
    panchanga_place, panchanga_start_date, panchanga_end_date=None
):
    res = utils.resource_strings
    c_dates = tithi_dates(
        panchanga_place, panchanga_start_date, panchanga_end_date, _third_crescent_tithi
    )  # V2.2.2
    results = []
    for c_date, _, _, tag_t in c_dates:
        jd = utils.gregorian_to_jd(panchanga.Date(c_date[0], c_date[1], c_date[2]))
...
def _sankalpa_mantra(
    panchanga_date,
    panchanga_place,
    ritu_per_solar_tamil_month=const.ritu_per_solar_tamil_month,
):  # V2.3.0
    ...
    if ritu_per_solar_tamil_month:
        tamil_maasa_index, _ = panchanga.tamil_solar_month_and_date(
            panchanga_date, panchanga_place
        )
        ritu = ritu_list[panchanga.ritu(tamil_maasa_index + 1)]
    samvastra = utils.YEAR_LIST[
        panchanga.samvatsara(panchanga_date, place, maasa_index, zodiac=0) - 1
    ]
    ...
    mantra_str = (
        "{0} nAma samvathsarE, {1} {7} rithou, {2} mAsE, {3} pakshE, "
    ...
    return mantra_str.format(
        samvastra, solistice, lunar_month, paksha, tithi, vasara, nakshathra, ritu
    )


def tithi_pravesha(
    birth_date: panchanga.Date = None,
    birth_time: tuple = None,
    birth_place: panchanga.Place = None,
    year_number=None,
    plus_or_minus_duration_in_days=30,
):
    """
    ...
    current_date_start = utils.previous_panchanga_day(
        current_date_start, plus_or_minus_duration_in_days
    )
    current_date_end = utils.next_panchanga_day(
        current_date_start, 2 * plus_or_minus_duration_in_days
    )
    ...
    t = panchanga.tithi(jd, birth_place)
    tm = panchanga.tamil_solar_month_and_date(birth_date, birth_place)
...
def _get_criteria_for_the_day(jd, place, use_purnimanta_system=None):
    ...
    date_in = panchanga.Date(y, m, d)
    _tithi_returned = panchanga.tithi(jd, place)
    _tithis = (
        [_tithi_returned[0], _tithi_returned[3]]
        if len(_tithi_returned) > 3
        else [_tithi_returned[0]]
    )
    ...
    td = None
    adhik_maasa = None
    _vaara = panchanga.vaara(jd) + 1
    day_id = panchanga.vaara(jd)
    if use_purnimanta_system is None:
        tm, td = panchanga.tamil_solar_month_and_date(date_in, place)
    else:
        tm, td, _, adhik_maasa, _ = panchanga.lunar_month_date(
            jd, place, use_purnimanta_system=use_purnimanta_system
        )
        tm -= 1
    criteria = {
        "Tithi": _tithis,
...
def get_festivals_between_the_dates(
    start_date: panchanga.Date,
    end_date: panchanga.Date,
    ...
    while start_jd <= end_jd:
        mfd = get_festivals_of_the_day(
            start_jd, place, festival_name_contains=festival_name_contains
        )
...
def get_festivals_of_the_day(jd, place, festival_name_contains=None):
    ...
    if len(festival_data) == 0:
        load_festival_data(const._FESTIVAL_FILE)
    matching_festivals = []
    criteria_list = [
        _get_criteria_for_the_day(jd, place, use_purnimanta_system=c)
        for c in [None, False, True]
    ]
    check_rows = (
        festival_data
        if festival_name_contains is None
        else [
            row
            for row in festival_data
            if festival_name_contains.casefold() in row["Festival_en"].casefold()
        ]
    )
            ...
            if row.get(key):
                try:
                    ...
                    if isinstance(value, list):
                        match = match and any(
                            [float(row[key]) == float(v) for v in value]
                        )  # == float(value)
                    else:
                    ...
                    if festival_name_contains is not None:
                        match = (
                            match
                            and festival_name_contains.casefold()
                            in row["Festival_en"].casefold()
                        )
...
if __name__ == "__main__":
    ...
    start_date = panchanga.Date(2025, 1, 1)
    end_date = panchanga.Date(2025, 12, 31)
    start_time = datetime.datetime.now()
    ...
    dob = (1996, 12, 7)
    tob = (5, 34, 0)
    place = panchanga.Place("Chennai", 13.0878, 80.2785, 5.5)
    ...
    _festival_file = (
        const._DATA_DIR + const._sep + "hindu_festivals_multilingual_unicode_bom.csv"
    )
    ...
    p1 = 3
    p2 = 6
    cdate_jd, p_long = panchanga.previous_conjunction_of_planet_pair(
        p1, p2, place, start_date
    )
    yc, dc, mc, fhc = utils.jd_to_gregorian(cdate_jd)
    print(
        "Conjunction of planets",
        utils.PLANET_NAMES[p1],
        "and",
        utils.PLANET_NAMES[p2],
        "is",
        yc,
        dc,
        mc,
        utils.to_dms(fhc, as_string=True),
        "at longitude",
        utils.to_dms(p_long, is_lat_long="plong"),
        "Before date:",
        start_date,
    )
    cdate_jd, p_long = panchanga.next_conjunction_of_planet_pair(
        p1, p2, place, start_date
    )
    yc, dc, mc, fhc = utils.jd_to_gregorian(cdate_jd)
    print(
    ...
    tz = -5.0  # 5.5#
    place = panchanga.Place(
        "PlacePerLatLong", lat, lon, tz
    )  # ('Chennai,IN',lat, lon, tz)
    # """
    ...
    for vdate in vdates:
        ...
        print(
            sdate + "\t" + utils.DAYS_LIST[v] + "\t" + vdate[2] + "\t upto " + vdate[1],
            "\n",
    ...
    exit()
    sr_dates = special_vratha_dates(
        place, start_date, end_date, "srartha", vratha_index_list=None
    )
    print(sr_dates)
    ...
    print(
        special_vratha_dates(place, start_date, end_date, "nakshathra", _pournami_tithi)
    )
