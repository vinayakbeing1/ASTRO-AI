...
def get_matching_pancha_pakshi_data_from_db(bird_index, weekday_index, paksha_index):
    pp_db = pd.read_csv(
        ...
        index_col=None,
        encoding="utf-8",
        usecols=range(_LAST_COL_FOR_READING + 1),
    )
    search_criteria = (
...
def construct_pancha_pakshi_information(
    ...
    result_list = pancha_paksha.get_matching_pancha_pakshi_data_from_db(
    ...
    for row in range(0, len(result_list), 5):
        ...
        time_from = (
            ...
            + "{:02d}".format(bd)
            + " "
            + utils.to_dms(bfh, use_24hour_format=True)
        )
        ...
        rtng = ""
        nkb = (
            utils.resource_strings["nakshathra_pakshi_str"]
            + " : "
            + utils.resource_strings[pancha_pakshi_birds[int(mbi)] + "_str"]
        )
        nkbi = pancha_pakshi_images[int(mbi)]
        ppbi = pancha_pakshi_images[int(ppi)]
        ppb = (
            utils.resource_strings["padu_pakshi_str"]
            + " : "
            + utils.resource_strings[pancha_pakshi_birds[int(ppi)] + "_str"]
        )
        bpb = (
            utils.resource_strings["bharana_pakshi_str"]
            + " : "
            + utils.resource_strings[pancha_pakshi_birds[int(bpi)] + "_str"]
        )
        bpbi = pancha_pakshi_images[int(bpi)]
        kp_icon = "shukla_paksha.png" if int(pi) == 0 else "krishna_paksha.png"
        kpstr = (utils.PAKSHA_LIST[int(pi)], kp_icon)
        wstr = (utils.DAYS_LIST[int(wdi)], "")
        tstr = (
            (utils.resource_strings["daytime_str"], "day_sun.png")
            if dni == 0
            else (utils.resource_strings["nighttime_str"], "moon_with_star.png")
        )
        parent_level_labels.append(
            [kpstr, wstr, tstr, (nkb, nkbi), (ppb, ppbi), (bpb, bpbi)]
        )
        tlist = [
            time_from1,
            time_to,
            ...
            (main_act_image, main_act, main_act_color),
            sub_bird,
            sub_act,
            rel,
            pf,
        ...
        for irow in range(row, row + 5):
            ...
            rel = (
                "",
                utils.resource_strings[pp_relations[int(reli)] + "_str"],
                pp_relation_colors[int(reli)],
            )
            eff = (
                "",
                utils.resource_strings[pp_effect[int(efi)] + "_str"],
                pp_effect_colors[int(efi)],
            )
            cll = [
                time_from1,
                time_to,
                ...
                (sub_act_image, sub_act, sub_act_color),
                rel,
                pf,
                eff,
                rtng,
