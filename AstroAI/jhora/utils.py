...
def use_database_for_world_cities(enable_database=False):
    ...
    if enable_database:
        with open(const._world_city_csv_file, "r", encoding="ISO-8859-1") as file:
            world_cities_dict = {
                row[1].lower(): idx for idx, row in enumerate(csv.reader(file))
            }
...
sort_tuple = lambda tup, tup_index, reverse=False: sorted(
    tup, key=lambda x: x[tup_index], reverse=reverse
)
...
def save_location_to_database_old(location_data):
    ...
    _world_city_db_df.to_csv(
        const._world_city_csv_file, mode="w", header=None, index=False
    )  # ,quoting=None)
...
def save_location_to_database(location_data):
    ...
    print("writing ", location_data, " to ", const._world_city_csv_file)
    with open(
        const._world_city_csv_file, mode="a", newline="", encoding="ISO-8859-1"
    ) as csvfile:
    ...
    try:
        ...
        if g == None or g == "":
            ...
            try:
                ...
                print(
                    "No latitude/longitude provided. Could not guess location from IP Address"
                )
                return []
        else:
            ...
            time_zone_offset = get_place_timezone_offset(latitude, longitude)
            print(
                "Location obtained from IP Address:",
                place,
...
def _validate_data(
    place, latitude, longitude, time_zone_offset, dob, tob, division_chart_factor
):
    country = ""
    ...
    if division_chart_factor not in const.division_chart_factors:
        ...
        w_msg = (
            "\nInvalid value for dhasa varga factor. "
            + str(division_chart_factor)
            + "\nAllowed values "
            + str_dvf
            + "\n"
...
def get_location(place_name=None):
    ...
    if place_index is not None and place_index >= 0:
        ...
        print(
            place_name,
            "not in " + const._world_city_csv_file + ".Trying to get from Google",
        )
            ...
            if "," in place_name:
                ...
                if _city not in world_cities_dict.keys():
                    ...
                    save_location_to_database(
                        [_country, _city, _latitude, _longitude, _tz_str, _time_zone]
                    )
        else:
            print(
                "Could not get",
                place_name,
                "from google.Trying to get from OpenStreetMaps",
            )
            place_found = False
    ...
    while True:
        try:
            ...
            address, (latitude, longitude) = geolocator.geocode(
                "city:" + place_with_country_code, featuretype="city"
            )
            break
        except (RuntimeError, TypeError, NameError):
            # print('except')
            ValueError(
                "City:" + place_with_country_code + " not found in OpenStreetMap"
            )
            address = ""
            break
    if address:
...
def _get_timezone_from_pytz(timezone_str_from_geocoder):
    ...
    tz = (
        datetime.datetime.now(timezone(timezone_str_from_geocoder))
        .utcoffset()
        .total_seconds()
        / 60
        / 60
...
def get_planet_to_house_dict_from_chart(house_to_planet_list):
    ...
    p_to_h = {
        p: h
        for p in [*range(9)] + [const._ascendant_symbol]
        for h, planets in enumerate(house_to_planet_list)
        if str(p) in planets
    }
    return p_to_h
...
def set_language(language=const._DEFAULT_LANGUAGE):
    ...
    if language in const.available_languages.values():
        ...
        language_list_file = (
            const._LANGUAGE_PATH + const._DEFAULT_LANGUAGE_LIST_STR + language + ".txt"
        )
        # print('language_list_file',language_list_file)
        language_message_file = (
            const._LANGUAGE_PATH + const._DEFAULT_LANGUAGE_MSG_STR + language + ".txt"
        )

        get_resource_lists(language_list_file)
        resource_strings = get_resource_messages(
            language_message_file=language_message_file
        )
...
def _read_resource_messages_from_file(message_file):
    if not os.path.exists(message_file):
        print(
            "Error: List Types File:"
            + message_file
            + " does not exist. Script aborted."
        )
        exit()
    cal_key_list = {}
...
def get_resource_messages(
    language_message_file=const._LANGUAGE_PATH
    + const._DEFAULT_LANGUAGE_MSG_STR
    + const._DEFAULT_LANGUAGE
    + ".txt",
):
    """
...
def get_resource_lists(
    language_list_file=const._LANGUAGE_PATH
    + const._DEFAULT_LANGUAGE_LIST_STR
    + const._DEFAULT_LANGUAGE
    + ".txt",
):
    """
...
from_dms_to_str = (
    lambda dms_list: str(dms_list[0])
...
def _function(point):
    ...
    # Place Citra at 180°
    fval = swe.fixstar_ut("Citra", point, flag=swe.FLG_SWIEPH | swe.FLG_SIDEREAL)[0] - (
        180
    )
...
def julian_day_utc(julian_day, place):
    return julian_day - (place.timezone / 24.0)
...
def julian_day_number(date_of_birth_as_tuple, time_of_birth_as_tuple):
    ...
    """
    tob_in_hours = (
        time_of_birth_as_tuple[0]
        + time_of_birth_as_tuple[1] / 60.0
        + time_of_birth_as_tuple[2] / 3600.0
    )
    jd = swe.julday(
        date_of_birth_as_tuple[0],
        date_of_birth_as_tuple[1],
        date_of_birth_as_tuple[2],
        tob_in_hours,
    )
    return jd
...
def deeptaamsa_range_of_planet(planet, planet_longitude_within_raasi):
    ...
    """
    return (
        planet_longitude_within_raasi - const.deeptaamsa_of_planets[planet],
...
def _convert_to_tamil_date_and_time(panchanga_date, time_of_day_in_hours, place=None):
    ...
    if place != None:  # if solar time > sunset time move to next day
        ...
        sunset_jd = drig_panchanga.sunset(jd, place)[0] - (place.timezone / 24.0)
        sunset_time = from_dms_str_to_degrees(
            drig_panchanga.sunset(sunset_jd, place)[1]
        )
...
def previous_panchanga_day(panchanga_date, minus_days=1):
    ...
    if len(p_date_str) == 4:
        p_date = drig_panchanga.Date(
            -int(p_date_str[1]), int(p_date_str[2]), int(p_date_str[3])
        )
    else:
        p_date = drig_panchanga.Date(
            int(p_date_str[0]), int(p_date_str[1]), int(p_date_str[2])
        )
    return p_date
...
def date_diff_in_years_months_days(
    start_date_str, end_date_str, date_format_str="%Y-%m%-d"
):
...
def udhayadhi_nazhikai(jd, place):
    ...
    return [
        str(naazhigai) + ":" + str(vinadigal) + ":" + str(tharparai),
        tharparai1 / 3600.0,
    ]


closest_element_from_list = lambda list_array, value: list_array[
    min(range(len(list_array)), key=lambda i: abs(list_array[i] - value))
]
...
count_stars = (
    lambda from_star, to_star, dir=1, total=27: ((to_star + total - from_star) % total)
    + 1
    if dir == 1
    else ((from_star + total - to_star) % total) + 1
)
count_rasis = (
    lambda from_rasi, to_rasi, dir=1, total=12: ((to_rasi + total - from_rasi) % total)
    + 1
    if dir == 1
    else ((from_rasi + total - to_rasi) % total) + 1
)
...
def __varga_non_cyclic(
    dcf, base_rasi=0, start_sign_variation=1, count_from_end_of_sign=False
):
    """
    ...
    for sign in range(12):
        ...
        if start_sign_variation == 1 and sign in const.even_signs:
            start_sign = (seed + 6) % 12
        elif start_sign_variation == 2 and sign in const.even_signs:
            start_sign = (seed + 8) % 12
        elif start_sign_variation == 3 and sign in const.even_signs:
            start_sign = (seed + 4) % 12
        elif start_sign_variation == 4 and sign in const.even_signs:
            start_sign = (seed + 10) % 12
        elif start_sign_variation == 5 and sign in const.even_signs:
            start_sign = (seed + 2) % 12
...
get_prasna_lagna_KP_249_for_rasi_chart = lambda kp_no: const.prasna_kp_249_dict[kp_no][
    0
]
get_prasna_lagna_KP_249_for_varga_chart = (
    lambda kp_no, varga_no=1: int(
        0.5
        * (const.prasna_kp_249_dict[kp_no][2] + const.prasna_kp_249_dict[kp_no][3])
        // (30.0 / varga_no)
    )
    % 12
    if varga_no > 1
...
get_prasna_lagna_108_for_navamsa = (
    lambda kp_no: 11 if (kp_no % 12 == 0) else (kp_no % 12) - 1
)
get_prasna_lagna_108_for_varga_chart = (
    lambda nadi_no, varga_no=1: get_prasna_lagna_108_for_rasi_chart(nadi_no)
    if varga_no == 1
    else (
        get_prasna_lagna_108_for_navamsa(nadi_no)
        if varga_no == 9
        else int(nadi_no // (108 / varga_no)) % 12
    )
)
get_prasna_lagna_nadi_for_rasi_chart = lambda nadi_no: nadi_no // 150
get_prasna_lagna_nadi_for_varga_chart = (
    lambda nadi_no, varga_no=1: int(nadi_no // (150 / varga_no)) % 12
    if varga_no > 1
    else get_prasna_lagna_nadi_for_rasi_chart(nadi_no)
)
# Get row,col of the element that contains a string
get_2d_list_index = lambda matrix, search_string, contains_in_element=False: next(
    (i, j)
    for i, row in enumerate(matrix)
    for j, element in enumerate(row)
    if (contains_in_element and search_string in element)
    or (not contains_in_element and search_string == element)
)
get_1d_list_index = lambda matrix, search_string, contains_in_element=False: next(
    (
        i
...
get_KP_details_from_planet_longitude = lambda planet_longitude: {
    kp_no: [r, n, sd, ed, rl, sl, ssl]
    for kp_no, [r, n, sd, ed, rl, sl, ssl] in const.prasna_kp_249_dict.items()
...
def search_replace(input_list, s1, s2):
    if isinstance(input_list[0], list):  # Check if the input_list is a 2D list
        return [
            [element.replace(s1, s2) if s1 in element else element for element in row]
            for row in input_list
        ]
    else:  # It's a 1D list
        return [
            element.replace(s1, s2) if s1 in element else element
...
cyclic_count_of_stars_with_abhijit_in_22 = lambda lst, from_star, to_star: (
    lambda start_idx, end_idx: (end_idx - start_idx + 1) % len(lst)
    if start_idx <= end_idx
    else len(lst) - start_idx + end_idx + 1
)(lst.index(from_star), lst.index(to_star))
cyclic_count_of_stars_with_abhijit = (
    lambda from_star, count, direction=1, star_count=28: (
        (from_star - 1 + (count - 1) * direction) % star_count
    )
    + 1
)
cyclic_count_of_stars = (
    lambda from_star, count, direction=1: cyclic_count_of_stars_with_abhijit(
        from_star, count, direction, star_count=27
    )
)
cyclic_count_of_stars_without_abhijit = (
    lambda from_star, count, direction=1: cyclic_count_of_stars_with_abhijit(
        from_star, count, direction, star_count=27
    )
)
cyclic_count_of_numbers = (
    lambda from_number, to_number, dir=1, number_count=30: (
        (from_number - 1 + (to_number - 1) * dir) % number_count
    )
    + 1
)
...
def get_nakshathra_list_with_abhijith():
    return (
        [NAKSHATRA_LIST[s] for s in range(20)]
        + [NAKSHATRA_LIST[27]]
        + [NAKSHATRA_LIST[s] for s in range(20, 27)]
    )


karana_lord = lambda karana_index: [
    _karana_lord
    for _karana_lord, kar_list in const.karana_lords.items()
    if karana_index in kar_list[0]
][0]
...
def vaakya_tamil_month(year, month_number):
    ...
    diff = kddays - 1826555
    month_birthday = datetime.datetime(1900, 1, 1, 0, 0, 0) + datetime.timedelta(
        days=diff
    )
    ...
    next_mon_birthday = datetime.datetime(1900, 1, 1, 0, 0, 0) + datetime.timedelta(
        days=next_mon_kd - 1826555
    )
    num_days_in_this_month = (next_mon_birthday - month_birthday).days

    return (
        tamil_month_names[month_number - 1],
        datetime.datetime.strftime(month_birthday, "%d-%m-%Y"),
        weekday,
        num_days_in_this_month,
        month_start_kd,
    )
