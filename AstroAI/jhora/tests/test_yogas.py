...
def brahma_yoga(h_to_p):
    ...
    ly2 = p_to_h[4] in quadrants_of_the_house(
        p_to_h[house.house_owner(h_to_p, (asc_house + 8) % 12)]
    )
    ly3 = p_to_h[5] in quadrants_of_the_house(
        p_to_h[house.house_owner(h_to_p, (asc_house + 10) % 12)]
    )
    ly4_1 = p_to_h[3] in quadrants_of_the_house(
        p_to_h[house.house_owner(h_to_p, (asc_house) % 12)]
    )
    ly4_2 = p_to_h[3] in quadrants_of_the_house(
        p_to_h[house.house_owner(h_to_p, (asc_house + 9) % 12)]
    )
...
def vishnu_yoga(h_to_p):
    ...
    vy1 = (
        ...
        and p_to_h[house.house_owner(h_to_p, (asc_house + 9) % 12)]
        == (asc_house + 1) % 12
    )
    if not vy1:
        return False
    lord_of_ninth_in_navamsa = house.house_owner(
        h_to_p, p_to_h_navamsa[ninth_lord_in_rasi]
    )
...
def saarada_yoga(h_to_p):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    sy1 = (
        p_to_h[house.house_owner(h_to_p, (asc_house + 9) % 12)] == (asc_house + 4) % 12
    )
    if not sy1:
        return False
...
def bhaarathi_yoga(h_to_p):
    ...
    ninth_lord = house.house_owner(h_to_p, (asc_house + 8) % 12)
    navamsa_lords = [
        house.house_owner(h_to_p, p_to_h_navamsa[second_lord]),
        house.house_owner(h_to_p, p_to_h_navamsa[fifth_lord]),
...
def devendra_yoga(h_to_p):
    ...
    if not dy1:
        return False
    dy2 = (
        p_to_h[house.house_owner(h_to_p, (asc_house + 1) % 12)] == (asc_house + 9) % 12
        and p_to_h[house.house_owner(h_to_p, (asc_house + 9) % 12)]
        == (asc_house + 1) % 12
    )
    if not dy2:
        return False
    dy3 = (
        p_to_h[house.house_owner(h_to_p, asc_house)] == (asc_house + 10) % 12
        and p_to_h[house.house_owner(h_to_p, (asc_house + 10) % 12)] == asc_house
    )
...
def indra_yoga(h_to_p):
    ...
    if not iy1:
        return False
    iy2 = (
        p_to_h[house.house_owner(h_to_p, (asc_house + 4) % 12)] == (asc_house + 10) % 12
        and p_to_h[house.house_owner(h_to_p, (asc_house + 10) % 12)]
        == (asc_house + 4) % 12
    )
    return iy1 and iy2
...
def ravi_yoga(h_to_p):
    ...
    if not ry1:
        return False
    ry2 = (
        p_to_h[house.house_owner(h_to_p, (asc_house + 9) % 12)] == (asc_house + 2) % 12
...
def gandharva_yoga(h_to_p):
    ...
    lagna_lord = house.house_owner(h_to_p, asc_house)
    gy1 = p_to_h[
        house.house_owner(h_to_p, (asc_house + 9) % 12)
...
def chapa_yoga(h_to_p):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    cy1 = (
        p_to_h[house.house_owner(h_to_p, (asc_house + 3) % 12)] == (asc_house + 9) % 12
        and p_to_h[house.house_owner(h_to_p, (asc_house + 9) % 12)]
        == (asc_house + 3) % 12
    )
    lagna_lord = house.house_owner(h_to_p, asc_house)
...
def makuta_yoga(h_to_p):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    my1 = (
        p_to_h[4] == (p_to_h[house.house_owner(h_to_p, (asc_house + 8) % 12)] + 8) % 12
    )
    my2 = any(
        [
            np in ["5", "6"]
...
def harsha_yoga(h_to_p):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    return (
        p_to_h[house.house_owner(h_to_p, (asc_house + 5) % 12)] == (asc_house + 5) % 12
    )
...
def sarala_yoga(h_to_p):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    return (
        p_to_h[house.house_owner(h_to_p, (asc_house + 7) % 12)] == (asc_house + 7) % 12
    )
...
def vimala_yoga(h_to_p):
    ...
    asc_house = p_to_h[const._ascendant_symbol]
    return (
        p_to_h[house.house_owner(h_to_p, (asc_house + 11) % 12)]
        == (asc_house + 11) % 12
    )
