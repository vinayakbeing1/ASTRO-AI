...
class VedicCalendar(QWidget):
        ...
        if link == "show_more":
            try:
                ...
                panchanga_widget = PanchangaInfoDialog(
                    language=self._language, jd=jd, place=place
                )
    ...
    def _get_days_panchanga_info(self, row, col):
        ...
        if jd is not None:
            ...
            _tithi_icon = (
                ...
                if (
                    _tit == 15
                    or (len(_tithi_returned) > 3 and _tithi_returned[3] == 15)
                )
                else (
                    _amavasai_icon
                    if (
                        _tit == 30
                        or (len(_tithi_returned) > 3 and _tithi_returned[3] == 30)
                    )
                    else ""
                )
            )
            ...
            _panchanga_dict = {
                "top_left": (kp_icon, _tithi),
                "middle_left": (_tithi_icon, str(td)),
                "middle_right": ("", str(d)),
                "bottom_left": _nak,
                "top_right": (_sunrise_icon, _srise),
                "bottom_right": (_sunset_icon, _sset),
            }
            header_text = (
                year_str
                + " "
    ...
    def cell_clicked(self, row, col):
        try:
            ...
            font = QFont()
            font.setPointSize(_info_label1_font_size)
