...
class WidgetDialog(QDialog):
    ...
    def create_ui(self):
        ...
        self.setMinimumSize(
            self.sizeHint()
        ) if self._fit_to_widget_contents else self.setMinimumSize(700, 650)
...
class InfoDialog(QDialog):
    ...
    def __init__(self, title="", info_text="", button_texts=[]):
        ...
        self._button_texts = (
            button_texts if len(button_texts) > 0 else [self.res["accept_str"]]
        )
        self.create_ui()
...
class OptionDialog(QDialog):
    ...
    def __init__(
        ...
        self._option_label = (
            self.res["select_one_of_options_from_str"]
...
if __name__ == "__main__":
    ...
    _title = (
        "/".join([utils.resource_strings[ol + "_str"] for ol in _title_list])
        + " "
        + utils.resource_strings["month_str"]
    )
    option_list = [
        utils.resource_strings[ol + "_str"] + " " + utils.resource_strings["month_str"]
        for ol in _title_list
    ]
    lm_dlg = OptionDialog(
        title=_title,
        option_label="Select options from",
