...
class LabelGrid(QWidget):
    def __init__(
        ...
        self._colors = colors
        self._set_labels_bold = set_labels_bold
        ...
        self.labels = [
            [QLabel(data[r][c]) for c in range(columns)] for r in range(rows)
        ]
        ...
        self._color_styles = [
            ["color: black; " for _ in range(self._columns)] for _ in range(self._rows)
        ]
        if (
            self._colors != None
            and len(self._colors) == self._rows
            and len(self._colors[0]) == self._columns
        ):
            self._color_styles = [
                ["color: " + self._colors[r][c] + "; " for c in range(self._columns)]
                for r in range(self._rows)
            ]
        [
            [
                self.labels[r][c].setStyleSheet(
                    self._label_style + self._border_style + self._color_styles[r][c]
                )
                for r in range(self._rows)
            ]
        ...
        [
            self.labels[r][0].setStyleSheet(
                self._label_style
                + self._header_style
                + self._border_style
                + self._color_styles[r][0]
            )
            for r in range(self._rows)
        ]
        [
            self.labels[0][c].setStyleSheet(
                self._label_style
                + self._header_style
                + self._border_style
                + self._color_styles[0][c]
            )
            for c in range(self._columns)
        ]
        ...
        [
            [
                self._grid_layout.addWidget(self.labels[r][c], r, c)
        ...
        if self._fit_to_contents:
            self._grid_layout.setSizeConstraint(
                QGridLayout.SizeConstraint.SetMinimumSize
            )
    ...
    def setSize(self, rows=2, columns=2):
        ...
        [
            [
                self.labels[r][c].setStyleSheet(
                    self._border_style + self._color_styles[r][c]
                )
                for r in range(self._rows)
            ]
            for c in range(self._columns)
        ...
        [
            self.labels[r][0].setStyleSheet(
                self._header_style + self._border_style + self._color_styles[r][0]
            )
            for r in range(self._rows)
        ]
        [
            self.labels[0][c].setStyleSheet(
                self._header_style + self._border_style + self._color_styles[0][c]
            )
            for c in range(self._columns)
        ]
        self._grid_layout.setSpacing(0)
    ...
    def setData(self, data=None, colors=None):
        if data == None:
            data = [["" for _ in range(self._columns)] for _ in range(self._rows)]
        [
            [self.labels[r][c].setText(data[r][c]) for c in range(self._columns)]
            for r in range(self._rows)
        ]
        [
            [
