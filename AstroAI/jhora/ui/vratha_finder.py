...
_CALCULATE_WAIT_MSG = (
    "<b>Calculating... Results will be updated here. Please wait...</b>"
)
...
class VrathaFinderDialog(QDialog):
    ...
    def __init__(self, current_date_jd, place: drik.Place, chart_title=""):
        ...
        self._chart_title = (
            self.res["search_festival_str"] if chart_title == "" else chart_title
        )
        self._vratha_result_dates = []
        self._tithi_list = (
            [self.res["tithi_str"]]
            + [utils.PAKSHA_LIST[0] + "-" + t for t in utils.TITHI_LIST[0:14]]
            + [utils.TITHI_LIST[14]]
            + [utils.PAKSHA_LIST[1] + "-" + t for t in utils.TITHI_LIST[0:14]]
            + [utils.TITHI_LIST[29]]
        )
    ...
    def _create_ui(self):
        ...
        self._special_combo.addItems(
            [
                self.res[k + "_str"]
                for k in list(vratha.special_vratha_map.keys()) + ["customized"]
            ]
        )
        ...
        max_width = max(
            font_metrics.horizontalAdvance(self._special_combo.itemText(i))
            for i in range(self._special_combo.count())
        )
        ...
        self._from_date_label = QLabel(self.res["starts_at_str"])
        h_layout.addWidget(self._from_date_label)
        y, m, d, _ = utils.jd_to_gregorian(self._current_date_jd)
        self._from_date_text = QLineEdit(str(y) + "," + str(m) + "," + str(d))
        h_layout.addWidget(self._from_date_text)
        self._to_date_label = QLabel(self.res["ends_at_str"])
        h_layout.addWidget(self._to_date_label)
        y, m, d, _ = utils.jd_to_gregorian(self._current_date_jd + 365)
        self._to_date_text = QLineEdit(str(y) + "," + str(m) + "," + str(d))
        h_layout.addWidget(self._to_date_text)
        ...
        self._tithi_combo.setVisible(False)
        self._nakshatra_combo.setVisible(False)
        self._yogam_combo.setVisible(False)
        self._tm_combo.setVisible(False)
        self._tithi_combo.setEnabled(False)
        self._nakshatra_combo.setEnabled(False)
        self._yogam_combo.setEnabled(False)
        self._tm_combo.setEnabled(False)
        ...
        self._results_list.setSizeAdjustPolicy(
        ...
        h_layout = QHBoxLayout()
        self._find_button = QPushButton(self.res["compute_str"])
        h_layout.addWidget(self._find_button)
        ...
        self._close_button = QPushButton(self.res["cancel_str"])
    ...
    def _enable_combos(self):
        ...
        if vrata_index == len(
            vratha.special_vratha_map.keys()
        ):  # vratha_type in ['tithi','nakshatra','yogam']:
            ...
            if self._search_group.checkedId() == 1:
                self._tithi_combo.setVisible(True)
                self._nakshatra_combo.setVisible(True)
                self._yogam_combo.setVisible(True)
                self._tm_combo.setVisible(True)
                self._tithi_combo.setEnabled(True)
                self._nakshatra_combo.setEnabled(True)
                self._yogam_combo.setEnabled(True)
                self._tm_combo.setEnabled(True)
            else:
                self._tithi_combo.setVisible(False)
                self._nakshatra_combo.setVisible(False)
                self._yogam_combo.setVisible(False)
    ...
    def _calculate_clicked_1(self):
        QToolTip.showText(
            self._accept_button.mapToGlobal(self._accept_button.rect().bottomLeft()),
    ...
    def _calculate_clicked(self):
        self._accept_button.setEnabled(False)
        _start_date = self._from_date_text.text().split(",")
        _start_date = drik.Date(
            int(_start_date[0]), int(_start_date[1]), int(_start_date[2])
        )
        _end_date = self._to_date_text.text().split(",")
        _end_date = drik.Date(int(_end_date[0]), int(_end_date[1]), int(_end_date[2]))
        vrata_index = self._special_combo.currentIndex()
        vdates = []
        if vrata_index == len(vratha.special_vratha_map.keys()):
            festival_selected = (
                None
                if self._search_group.checkedId() != 2
                else self._search_festival_edit.text()
            )
            _tithi_index = (
                None
                if self._search_group.checkedId() in [0, 2]
                or self._tithi_combo.currentIndex() == 0
                else self._tithi_combo.currentIndex()
            )
            _nakshatra_index = (
                None
                if self._search_group.checkedId() in [0, 2]
                or self._nakshatra_combo.currentIndex() == 0
                else self._nakshatra_combo.currentIndex()
            )
            _yogam_index = (
                None
                if self._search_group.checkedId() in [0, 2]
                or self._yogam_combo.currentIndex() == 0
                else self._yogam_combo.currentIndex()
            )
            _tm_index = (
                None
                if self._search_group.checkedId() in [0, 2]
                or self._tm_combo.currentIndex() == 0
                else self._tm_combo.currentIndex()
            )
            # print(_tithi_index,_nakshatra_index,_yogam_index,_tm_index,festival_selected)
            vdates = vratha.search(
                self.Place,
                _start_date,
                _end_date,
                _tithi_index,
                ...
                _yogam_index,
                _tm_index,
                festival_name_contains=festival_selected,
            )
        else:
            vratha_type = list(vratha.special_vratha_map.keys())[vrata_index]
            vdates = vratha.special_vratha_dates(
                self.Place, _start_date, _end_date, vratha_type
            )
        if vdates is not None and len(vdates) > 0:
            unique_first_items = len(set(item[0] for item in vdates))
            self._stats_label.setText(
                str(unique_first_items) + " " + self.res["matching_dates_found_str"]
            )
    ...
    def _accept_and_close(self):
        ...
        if selected_date_index != -1:
            ...
            self._selection_date_jd = utils.julian_day_number(
                selected_date, (selected_time, 0, 0)
            )
