...
class SapthaNaadi(QWidget):
    def __init__(self, planet_positions=[], planets_in_retrograde=[]):
        ...
        self._star_indices = {
            self._grid_label_values[i][j]: (i, j)
            for i in range(self._xdim)
            for j in range(self._ydim)
            if isinstance(self._grid_label_values[i][j], int)
        }
        self._rasi_color = "Red"
        self._planet_color = "Brown"
    ...
    def createUI(self):
        ...
        self._grid_labels = [
            [QLabel("") for j in range(self._ydim)] for i in range(self._xdim)
        ]
        for i in range(self._xdim):
            for j in range(self._ydim):
                ...
                )  # Optional: Add a border to each cell
    ...
    def _update_with_planet_labels(self):
        for p, (h, long) in self._planet_positions:
            ...
            cell_value = (
                self._grid_labels[r][c].text()
                + "<br>"
                + color_label(pstr + rstr, self._planet_color)
            )
    ...
    def _update_labels_with_stars_and_rasis(self):
        for i in range(self._xdim):
            for j in range(self._ydim):
                ...
                if star_cell:
                    cell_value = color_label(
                        utils.NAKSHATRA_SHORT_LIST[cell_value - 1], self._star_color
                    )
                    self._grid_labels[i][j].setText(cell_value)
                    self._grid_labels[i][j].setAlignment(Qt.AlignmentFlag.AlignTop)
                else:
                    cell_value = color_label(
                        utils.resource_strings[cell_value], self._text_color
                    )
                    self._grid_labels[i][j].setText(cell_value)
...
class PanchaShalaka(QWidget):
    def __init__(
        self, planet_positions=[], planets_in_retrograde=[], label_font_size=6
    ):
        super().__init__()
        self.x_dim = 7
        self.y_dim = 7
        ...
        self.setWindowTitle(
            utils.resource_strings["saptha_shalaka_str"]
            + " "
    ...
    def _update_with_planet_labels(self):
        for p, (h, long) in self._planet_positions:
            ...
            nak = drik.nakshatra_pada(p_long)
            p_star = nak[0]
            if p_star > const._ABHIJITH_STAR_INDEX:
                p_star += 1
            rstr = const._retrogade_symbol if p in self._planets_in_retrograde else ""
            pstr = (
                utils.resource_strings["ascendant_short_str"]
    ...
    def createUI(self):
        ...
        for _, (x, y, star_planets) in self.rasi_labels.items():
            ...
            painter.drawText(
                QRectF(int(x * x_gap), int((y) * y_gap), 300, 300), str(star_planets)
            )
            painter.setPen(QPen())
...
class SapthaShalaka(QWidget):
    ...
    def createUI(self):
        ...
        for line in self.lines:
            start, end = line
            painter.drawLine(
                start[0] * x_gap, (start[1]) * y_gap, end[0] * x_gap, (end[1]) * y_gap
            )
...
class ChandraKalanala(QWidget):
    def __init__(
        ...
        self.setWindowTitle(
            utils.resource_strings["chandra_kalanala_str"]
            + " "
            + utils.resource_strings["chakra_str"]
        )
        self.lines = [
...
class Tripataki(QWidget):
    ...
    def _update_with_planet_labels(self):
        self.rasi_labels = [
            (x, y, utils.RAASI_SHORT_LIST[h])
            for h, (x, y) in enumerate(self.rasi_labels)
        ]
    ...
    def createUI(self):
        ...
        for x, y, rasi_planets in self.rasi_labels:
            ...
            painter.setPen(QColor(color))
            painter.drawText(
                QRectF(int(x * x_gap), int((y) * y_gap), 300, 300), str(rasi_planets)
            )
            painter.setPen(QPen())
...
class SuryaKalanala(QWidget):
    def __init__(
        ...
        self.setGeometry(100, 100, 500, 500)
        self.setWindowTitle(
            utils.resource_strings["surya_kalanala_str"]
            + " "
            + utils.resource_strings["chakra_str"]
        )
        self.lines = [
...
class Shoola(QWidget):
    def __init__(
        ...
        self.setWindowTitle(
            utils.resource_strings["shoola_str"]
            + " "
            + utils.resource_strings["chakra_str"]
        )
        self.lines = [
    ...
    def createUI(self):
        ...
        for line in self.lines:
            ...
            painter.drawLine(
                start[0] * x_gap,
                (self.y_dim + 1 - start[1]) * y_gap,
                end[0] * x_gap,
                (self.y_dim + 1 - end[1]) * y_gap,
            )
        ...
        for _, (x, y, star_planets) in self.rasi_labels.items():
            ...
            painter.setPen(QColor(color))
            painter.drawText(
                QRectF(int(x * x_gap), int((self.y_dim + 1 - y) * y_gap), 300, 300),
                str(star_planets),
            )
            painter.setPen(QPen())
...
class Sarvatobadra(QWidget):
    def __init__(self, planet_positions=[], planets_in_retrograde=[]):
        ...
        self._star_indices = {
            ...
            and isinstance(self._grid_label_values[i][j], int)
        }
        self._rasi_color = "Red"
        self._planet_color = "Brown"
        self._text_color = "Green"
        ...
        for i in range(9):
            for j in range(9):
                ...
                if border_cell and isinstance(cell_value, int):
                    ...
                    cell_value = color_label(
                        utils.RAASI_SHORT_LIST[cell_value - 1], self._rasi_color
                    )
                    self._grid_labels[i][j].setText(cell_value)
                    self._grid_labels[i][j].setAlignment(Qt.AlignmentFlag.AlignCenter)
                else:
...
class KaalaChakra(QWidget):
    ...
    def get_kaala_chakra_star_labels(self):
        ...
        _abhijit_list = [_star_list[i] for i in const.abhijit_order_of_stars]
        self.stars_inner = [
            ((self._base_star - 1) + (ele - 1)) % 28 for ele in self.stars_inner
        ]
        self.labels_inner = [_abhijit_list[i] for i in self.stars_inner]
        self.stars_outer_divisions = [
            [((self._base_star - 1) + (ele - 1)) % 28 for ele in row]
            for row in self.stars_outer_divisions
        ]
        self.labels_outer_divisions = [
            [_abhijit_list[i] for i in row] for row in self.stars_outer_divisions
        ]
    ...
    def setData(
        ...
        self._planets_in_retrograde = planets_in_retrograde
        self.labels_custom = [
            utils.resource_strings[r]
            for r in [
                "kaala_chakra_southeast_str",
                "kaala_chakra_east_str",
                "kaala_chakra_northeast_str",
                "kaala_chakra_north_str",
                "kaala_chakra_northwest_str",
                "kaala_chakra_west_str",
                "kaala_chakra_southwest_str",
                "kaala_chakra_south_str",
            ]
        ]
        ...
        try:
            ...
            for i, angle in enumerate(angles_outer):
                ...
                for j in range(1, 4):
                    ...
                    color = (
                        self._planet_color
                        if "\n" in self.labels_outer_divisions[i][j - 1]
                        else self._star_color
                    )
                    qp.setPen(QColor(color))
                    qp.drawText(
                        QRectF(x_div, y_div, 300, 300),
                        Qt.AlignmentFlag.AlignLeft,
                        self.labels_outer_divisions[i][j - 1],
                    )
                    qp.setPen(QPen())
                ...
                qp.drawText(
                    QRectF(x_label, y_label, 300, 300),
                    Qt.AlignmentFlag.AlignLeft,
                    self.labels_custom[i],
                )
                qp.setPen(QPen())
        except Exception as e:
            print(f"Error in drawCircle: {e}")
...
class KotaChakra(QWidget):
    def __init__(
        ...
        self.labels = self.get_planet_star_labels(
            planet_positions, birth_star, planets_in_retrograde
        )
        self._title_font_size = title_font_size
        self._label_font_size = label_font_size
        self._square_color_rgbs = square_color_rgbs
        if len(square_captions) == 0:
            self._square_captions = [
                utils.resource_strings[r]
                for r in [
                    "baahya_str",
                    "praakaara_str",
        ...
        self.setWindowTitle(
            utils.resource_strings["kota_str"]
            + " "
            + utils.resource_strings["chakra_str"]
        )
        if len(self._planet_positions) > 0:
            ...
            self._kota_lord = utils.PLANET_NAMES[
                house.house_owner_from_planet_positions(planet_positions, moon_house)
            ]
            self._kota_paala = utils.PLANET_NAMES[
                const.kota_paala_lord_for_star_paadha[birth_star - 1][
                    birth_star_padha - 1
                ]
            ]
    ...
    def setData(
        ...
        self._kota_paala = utils.PLANET_NAMES[
            const.kota_paala_lord_for_star_paadha[birth_star - 1][birth_star_padha - 1]
        ]
        # print('kota lord',self._kota_lord,'kota paala',self._kota_paala)
        self.createUI()
    ...
    def drawLabels(self, painter, rect, index, num_squares):
        ...
        xadj = int(0.02 * rect.width())
        yadj = int(0.03 * rect.height())
        if index == num_squares - 1:  # Innermost square
            label_positions = [
                (
                    QPoint(rect.center().x() + xadj, rect.top() + yadj),
                    square_labels[0],
                ),  # Inner Top Middle
                (
                    QPoint(rect.right() - xadj, rect.center().y()),
                    square_labels[1],
                ),  # Inner Right
                (
                    QPoint(rect.center().x() + xadj, rect.bottom() - 3 * yadj),
                    square_labels[2],
                ),  # Inner Bottom
                (
                    QPoint(rect.left() + 3 * xadj, rect.center().y()),
                    square_labels[3],
                ),  # Inner Left
            ]
        else:
            label_positions = [
                ...
                (
                    rect.bottomRight() + QPoint(-xadj, -3 * yadj),
                    square_labels[4],
                ),  # Bottom Right
                (
                    QPoint(rect.center().x() + xadj, rect.bottom() - 3 * yadj),
                ...
                (
                    rect.bottomLeft() + QPoint(4 * xadj, -3 * yadj),
                    square_labels[6],
                ),  # Bottom Left
                (
                    QPoint(rect.left() + 4 * xadj, rect.center().y()),
                    square_labels[7],
                ),  # Center Left
            ]
        font = QFont()
        ...
        for pos, text in label_positions:
            painter.drawText(
                QRectF(pos.x() - 30, pos.y() - 10, 60, 200),
                Qt.AlignmentFlag.AlignLeft,
                text,
            )
        self.update()
    ...
    def drawCaptions(self, painter, rect, index, num_squares):
        ...
        if index == num_squares - 1:  # Innermost square
            pos = QPoint(
                rect.bottomRight().x() - xadj, rect.bottomRight().y() - int(yadj / 2)
            )
        else:
            pos = QPoint(
                rect.bottomRight().x() - 10 * xadj, rect.bottom() - int(yadj / 2)
            )
        font = QFont()
        font.setPointSize(self._label_font_size)
        font.setWeight(QFont.Weight.Bold)
        painter.setFont(font)
        painter.drawText(
            QRectF(pos.x() - 30, pos.y() - 10, 300, 300),
            Qt.AlignmentFlag.AlignLeft,
            _caption,
        )
    ...
    def get_kota_chakra_star_list(self, birth_star=15):
        ...
        return [
            [star_list_1[((birth_star - 1) + (ele - 1)) % 28] for ele in row]
    ...
    def get_planet_star_labels(
        self, planet_positions, birth_star=15, planets_in_retrograde=[]
    ):
        from jhora.panchanga import drik
...
if __name__ == "__main__":
    ...
    planet_positions = [
        ["L", (9, 22.44575902738461)],
        [0, (7, 21.565282200247736)],
        [1, (6, 6.959489445251251)],
        [2, (4, 25.539747317445176)],
        [3, (8, 9.936449034377262)],
        [4, (8, 25.82805158497206)],
        [5, (6, 23.71713310535756)],
        [6, (11, 6.807276353388318)],
        [7, (5, 10.553787374448945)],
        [8, (11, 10.553787374448916)],
        [9, (9, 8.2)],
        [10, (9, 2.2)],
        [11, (7, 9.7)],
    ]
