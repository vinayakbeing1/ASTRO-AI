...
class ConjunctionDialog(QDialog):
    ...
    def __init__(
        ...
        place: drik.Place,
        planet1=6,
        chart_title="Planetary Conjunctions",
        entry_type=0,
        planet2=4,
        raasi=None,
    ):
    ...
    def _create_ui(self):
        ...
        h_layout.addWidget(self._after_before_combo)
        self._after_before_combo.addItems(
            [self.res["next_str"], self.res["previous_str"]]
        )
        ...
        if (
            self._planet1 == const._ascendant_symbol
            or utils.PLANET_NAMES[self._planet1] in self._planet1_list
        ):
        ...
        v_layout.addLayout(h_layout)
        self._results_text = QLabel(
            self._after_before_combo.currentText() + "occurs at:"
        )
    ...
    def _update_planet2_list(self):
        ...
        if (
            self._planet1_combo.currentIndex() == 8
            or self._planet1_combo.currentIndex() == 9
        ):
    ...
    def _find_retrogression_change_date(self):
        ...
        cur_jd, ret_sign = drik.next_planet_retrograde_change_date(
            self._planet1, panchanga_start_date, panchanga_place, direction=direction
        )
        ...
        results += (
            "{0:4d}-{1:2d}-{2:2d}".format(y, m, d)
            + " "
            + utils.to_dms(fh, as_string=True)
        )
        ...
        self._dob_text.setText(str(y) + "," + str(m) + "," + str(d))
        self._tob_text.setText(str(hh) + ":" + str(mm) + ":" + str(ss))
        QApplication.restoreOverrideCursor()
    ...
    def _find_conjunction_date(self):
        ...
        QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
        self._planet1 = (
            const._ascendant_symbol
            if self._planet1_combo.currentIndex() == 0
            else utils.PLANET_NAMES.index(self._planet1_combo.currentText())
        )
        self._planet2 = (
            const._ascendant_symbol
            if self._planet2_combo.currentIndex() == 0
            else utils.PLANET_NAMES.index(self._planet2_combo.currentText())
        )
        panchanga_place = self.Place
        direction = 1 if self._after_before_combo.currentIndex() == 0 else -1
        start_jd = self._current_date_jd + direction * (1 / (24 * 60 * 60))
        ret = drik.next_conjunction_of_planet_pair(
            self._current_date_jd,
            panchanga_place,
            self._planet1,
            self._planet2,
            direction=direction,
        ...
        self._conjunction_date_jd = cur_jd
        self._current_date_jd = cur_jd
        results = (
            self._planet1_combo.currentText()
            + "/"
            + self._planet2_combo.currentText()
            + " "
        )
        y, m, d, fh = utils.jd_to_gregorian(cur_jd)
        results += (
        ...
        pstr1 = (
            self.res["ascendant_str"]
            if self._planet1 == const._ascendant_symbol
            else utils.PLANET_NAMES[self._planet1]
        )
        pstr2 = (
            self.res["ascendant_str"]
            if self._planet2 == const._ascendant_symbol
            else utils.PLANET_NAMES[self._planet2]
        )
        results += (
            " "
            + self.res["longitude_str"]
            + " "
            + pstr1
            + ":"
    ...
    def _find_transit_date(self):
        ...
        self._raasi = (
            None
            if self._raasi_combo.currentIndex() == 0
            else self._raasi_combo.currentIndex()
        )
        cur_jd, p1_long = drik.next_planet_entry_date(
            self._planet1,
            start_jd,
            panchanga_place,
            direction=direction,
...
def show(
    ...
    dlg = ConjunctionDialog(
        current_date_jd, place, planet1, chart_title, entry_type, planet2
    )
    ret = dlg.exec()
...
if __name__ == "__main__":
    ...
    exit()
    chart = ConjunctionDialog(
        jd, place, entry_type=entry_type, planet1=const._ascendant_symbol
    )
    chart.show()
    sys.exit(App.exec())
