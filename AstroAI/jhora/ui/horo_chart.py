...
class ChartSimple(QWidget):
    def __init__(self, chart_type="south indian", calculation_type: str = "drik"):
        ...
        if self._calculation_type == "ss":
            print(
                "ChartSimple:init: Forcing ayanamsa to SURYASIDDHANTA for the SURYA SIDDHANTA calculation type"
            )
    ...
    def _add_footer_to_chart(self):
        ...
        self._footer_label.setText(
            self._footer_title
        )  # "Copyright © Dr. Sundar Sundaresan, Open Astro Technologies, USA.")
    ...
    def _validate_ui(self):
        all_data_ok = (
            ...
            and self._name_text.text().strip() != ""
            and re.match(
    ...
    def compute_horoscope(self):
        ...
        (
            ...
            self._horoscope_charts,
            _,
        ) = self._horo.get_horoscope_information()
        _place = drik.Place(
    ...
    def _fill_information_label1(self, format_str):
        ...
        key = (
            ...
            + "-"
            + self._calendar_key_list["ascendant_str"]
        )
    ...
    def _fill_information_label2(self, format_str):
        ...
        db_list = []
        key = (
            "&nbsp;&nbsp;" + dhasa + " " + self._calendar_key_list["balance_str"] + " :"
        )
        ...
        info_str += format_str % (key, value)
        key = (
            self._calendar_key_list["ayanamsam_str"] + " (" + self._ayanamsa_mode + ") "
        )
    ...
    def _update_chart_ui_with_info(self):
        ...
        rasi_1d = self._horoscope_charts[
            0
        ]  # 0 for raasi chart from horoscope charts array
        rasi_1d = [x[:-1] for x in rasi_1d]  # remove last \n character
        # print('rasi_1d',rasi_1d)
        jd = (
            self._horo.julian_day
        )  # For ascendant and planetary positions, dasa buthi - use birth time
        place = drik.Place(
            self._place_name,
        ...
        if self._chart_type.lower() == "north indian":
            ...
            row, col = _get_row_col_string_match_from_2d_list(
                rasi_2d, self._calendar_key_list["ascendant_str"]
            )
            ...
            chart_1d = sudharsana_chakra.sudharshana_chakra_chart(
                jd, place, self._date_of_birth
            )  # , years_from_dob, divisional_chart_factor)
        ...
        if self._chart_type.lower() == "north indian":
            ...
            ascendant_navamsa = drik.dasavarga_from_long(
            ...
            row, col = _get_row_col_string_match_from_2d_list(
                nava_2d, self._calendar_key_list["ascendant_str"]
            )
            ...
            for z, pls in chart_1d:
                ...
                for p in tmp:
                    if p == const._ascendant_symbol:
                        pl_str += (
                            self._calendar_key_list["ascendant_short_str"] + "/"
