...
_DAYNIGHT_INDEX = 2
_NAK_BIRD_INDEX = 3
_NAK_ACTIVITY_INDEX = 4
_SUB_BIRD_INDEX = 5
_SUB_ACTIVITY_INDEX = 6
_DURATION_FACTOR = 7
_RELATION = 8
_POWER_FACTOR = 9
_EFFECT = 10
...
pp_relations = ["enemy", "same", "friend"]
pp_relation_colors = ["orange", "yellow", "light green"]
...
def add_text_above_parent_row(tree, parent_level_labels=None):
    ...
    intermediate_item.setFlags(
        intermediate_item.flags() & ~Qt.ItemFlag.ItemIsSelectable
    )
    ...
    for (text, icon_path) in parent_level_labels:
        ...
        if icon_path:
            ...
            icon_label.setPixmap(
                QIcon(IMAGE_PATH + icon_path).pixmap(
                    _DEFAULT_ICON_SIZE, _DEFAULT_ICON_SIZE
                )
            )
...
class PanchaPakshiSastraWidget(QWidget):
    def __init__(
        self,
        birth_bird_index=None,
        headers=None,
        parent_level_list=None,
        child_level_list=None,
        hide_headers=True,
        parent_level_labels=None,
        tree_font_size=9,
        expand_one_row_at_a_time=False,
        synch_with_local_clock=False,
    ...
    def show_remove_sandclock(self, row_item, column_index, remove_clock=False):
        ...
        self.tree.setItemWidget(
            row_item, column_index, widget
        )  # Set in the 6th column (index 5)
    ...
    def expand_row_with_datetime(self, search_datetime=None):
        ...
        if _DEBUG_:
            print(
                "expand_row_with_datetime",
                search_datetime,
                "isValid:",
                search_datetime.isValid(),
            )
        ...
        for i in range(root.childCount()):
            ...
            if _DEBUG_:
                print("Parent item texts:", from_datetime_string, to_datetime_string)
            ...
            if _DEBUG_:
                print("Parsed datetimes:", from_datetime, to_datetime)
            if _DEBUG_:
                print(
                    f"From isValid: {from_datetime.isValid()}, To isValid: {to_datetime.isValid()}"
                )

            try:
                if _DEBUG_:
                    print(
                        f"Comparison: {from_datetime} <= {search_datetime} <= {to_datetime}"
                    )
                if (
                    from_datetime.isValid()
                    and to_datetime.isValid()
                    and from_datetime <= search_datetime <= to_datetime
                ):
                    if _DEBUG_:
                        print(
                            "Expanding parent",
                    ...
                    for j in range(parent_item.childCount()):
                        ...
                        child_from_datetime = QDateTime.fromString(
                            child_from_datetime_string, _DATE_FORMAT
                        )
                        child_to_datetime = QDateTime.fromString(
                            child_to_datetime_string, _DATE_FORMAT
                        )
                        if _DEBUG_:
                            print(
                                "Checking child:",
                        ...
                        if _DEBUG_:
                            print(
                                f"Child Comparison: {child_from_datetime} <= {search_datetime} <= {child_to_datetime}"
                            )
                        if self._display_sand_clock:
                            self.show_remove_sandclock(
                                child_item, _FROM_DATE_DISP, remove_clock=True
                            )
                        if (
                            child_from_datetime.isValid()
                            and child_to_datetime.isValid()
                            and child_from_datetime
                            <= search_datetime
                            <= child_to_datetime
                        ):
                            if _DEBUG_:
                            ...
                            if self._display_sand_clock:
                                self.show_remove_sandclock(
                                    child_item, _FROM_DATE_DISP, remove_clock=False
                                )
                ...
                print(
                    "Error processing search_datetime", search_datetime, "Exception:", e
                )
    ...
    def populate_tree(
        ...
        for parent_index, parent_row in enumerate(parent_level_list):
            ...
            for child_row in child_level_list[parent_index]:
                ...
                power_factor_progress_bar1.setValue(
                    int(child_row[_POWER_FACTOR_DISP] * 100)
                )  # Convert to percentage
                power_factor_progress_bar1.setStyleSheet(
                    "QProgressBar::chunk {background-color: lightgreen;}"
                )
                power_factor_progress_bar1.setFixedWidth(60)
                self.tree.setItemWidget(
                    child_item, _POWER_FACTOR_DISP, power_factor_progress_bar1
                )
                rating_progress_bar2 = QProgressBar()
                column_value = int(
                    child_row[_RATING_DISP]
                )  # Assuming column values are integers between 1 and 10
                rating_progress_bar2.setValue(
                    column_value * 10
                )  # Convert to percentage
                rating_progress_bar2.setStyleSheet(
                    "QProgressBar::chunk {background-color: lightgreen;}"
                )
