...
def get_planet_entry_data(start_jd, place):
    ...
    for planet in list(retrogression_planets_periods.keys())[:5]:
        npe_data[planet] = {}
        jd_utc = start_jd - place.timezone / 24.0
        ...
        for i, t in enumerate(theta[:-1]):
            r = int(rasi + i) % 12
            npe = drik.next_planet_entry_date(
                planet, start_jd, place, raasi=None, direction=1
            )  # V4.5.0
            y, m, d, _ = utils.jd_to_gregorian(npe[0])
            p_long = npe[1]
...
class VakraGathiPlot(QtWidgets.QMainWindow):
    ...
    def _change_language(self):
        ...
        self._lang_combo.addItems(
            [msgs[l.lower() + "_str"] for l in const.available_languages.keys()]
        )
    ...
    def onMouseMoved(self, evt):
        ...
        if (
            self.plot_graph.plotItem.vb.mapSceneToView(evt)
            and self.line.cursor() == Qt.CursorShape.PointingHandCursor
        ):
    ...
    def _update_chart(self):
        ...
        if self._planet_combo.currentIndex() > 4:
            planet = self._planet_combo.currentIndex() + _starting_planet_index + 2
        if planet not in retrogression_planets_periods.keys():
            return
        # Draw retrograde graph
        self.title = (
            utils.resource_strings["retrograde_as_seen_from_earth_str_1"]
            + " "
            + utils.PLANET_NAMES[planet]
            + " "
            + utils.resource_strings["retrograde_as_seen_from_earth_str_2"]
        )
        ...
        p_long = 0
        if self._start_jd != None and self._place != None:
            jd_utc = self._start_jd - self._place.timezone / 24.0
            p_long = drik.sidereal_longitude(jd_utc, planet)
            ...
            th = p_long * pi / 180.0
            x_label = (d1 + d2) * cos(th) * 0.5
            y_label = (d1 + d2) * sin(th) * 0.5
        ...
        for i, t in enumerate(theta[:-1]):
            ...
            if planet <= 6:
                ...
                y, m, d = npe[0]
                p_long = utils.to_dms(npe[1], is_lat_long="plong")
                date_str = str(y) + "-" + str(m) + "-" + str(d)
                p_date = drik.Date(y, m, d)
            # Write Raasi Label
            ...
            x_label = (d1 + d2) * cos(th) * label_factor
            y_label = (d1 + d2) * sin(th) * label_factor
            lbl.setPos(x_label, y_label)
            # Draw Raasi Separation Line
            x_line = (d1 + d2) * cos(t) * label_factor
            y_line = (d1 + d2) * sin(t) * label_factor
            line = pg.LineROI(
                pos1=(0, 0),
                pos2=(x_line, y_line),
                width=0.5,
