...
class VedicAnalogClock(QWidget):
    ...
    def __init__(
        self, jd=None, place=None, language="ta", force_equal_day_night_ghati=False
    ):
        super().__init__()
        ...
        # Load the icon
        self.icon = QPixmap(_IMAGE_ICON).scaled(
    ...
    def mouseMoveEvent(self, event):
        pos = event.pos()
        if (
            abs(pos.x() - self.sunrise_icon_pos[0]) <= self.sunrise_icon.width()
            and abs(pos.y() - self.sunrise_icon_pos[1]) <= self.sunrise_icon.height()
        ):
            srise_tip = (
                self.res["sunrise_str"] + " (" + utils.to_dms(self._today_sunrise) + ")"
            )
            QToolTip.showText(QCursor.pos(), srise_tip, self)
        elif (
            abs(pos.x() - self.sunset_icon_pos[0]) <= self.sunset_icon.width()
            and abs(pos.y() - self.sunset_icon_pos[1]) <= self.sunset_icon.height()
        ):
            sset_tip = (
                self.res["sunset_str"] + " (" + utils.to_dms(self._today_sunset) + ")"
            )
    ...
    def _get_drik_info(self):
        ...
        if self.jd is None:
            self.jd = utils.julian_day_number(
                drik.Date(
        ...
        self._day_length = drik.day_length(self.jd, self.place)
        self._night_length = drik.night_length(self.jd, self.place)
        _day_duration = self._day_length + self._night_length
        self._vedic_hour_per_local_hour = _VEDIC_HOURS_PER_DAY / _day_duration
        self._today_sunrise = drik.sunrise(self.jd, self.place)[0]
        self._today_sunset = drik.sunset(self.jd, self.place)[0]
        ...
        local_time_zone_hours = self.place.timezone
        time_zone = QTimeZone(int(local_time_zone_hours * 3600))
        self.qtime_tomorrow_sunrise = QDateTime(
            tomorrow_date, tomorrow_sunrise_time, time_zone
        )
        self._day_vedic_hour_per_local_hour = (
            _HALF_VEDIC_HOURS_PER_DAY / self._day_length
        )
        self._night_vedic_hour_per_local_hour = (
            _HALF_VEDIC_HOURS_PER_DAY / self._night_length
        )
        ssvt = (
            self.get_vedic_time_equal_day_night_ghati(self._today_sunset)
            if self.force_equal_day_night_ghati
            else self.get_vedic_time(self._today_sunset)
        )
        ssgt = (
            ssvt[0]
            + ssvt[1] / _VEDIC_HOURS_PER_DAY
            + ssvt[2] / _VEDIC_HOURS_PER_DAY / _VEDIC_HOURS_PER_DAY
        )
    ...
    def get_vedic_time_equal_day_night_ghati(self, float_hours=None):
        ...
        if float_hours is None:
            ...
            float_hours = (
                ...
                + current_datetime.time().msec() / 3600000
            )
        if float_hours <= self._today_sunset and float_hours >= self._today_sunrise:
            ...
            total_vedic_hours = (
                _HALF_VEDIC_HOURS_PER_DAY
                + (float_hours - self._today_sunset)
                * self._night_vedic_hour_per_local_hour
                if float_hours >= self._today_sunset
                else _VEDIC_HOURS_PER_DAY
                - (self._today_sunrise - float_hours)
                * self._night_vedic_hour_per_local_hour
            )
        vedic_hours = int(total_vedic_hours)
        vedic_minutes = int((total_vedic_hours - vedic_hours) * _VEDIC_HOURS_PER_DAY)
        vedic_seconds = int(
            ((total_vedic_hours - vedic_hours) * _VEDIC_HOURS_PER_DAY - vedic_minutes)
            * _VEDIC_HOURS_PER_DAY
        )
    ...
    def paintEvent(self, event):
        try:
            ...
            available_height = (
                self.height() - 50
            )  # Reserve 50 pixels for text at the bottom
            ...
            self.drawTickMarks(
                painter, _CENTER_X, _CENTER_Y, _OUTER_CIRCLE_RADIUS, _OUTER_TICK_STEP
            )  # Outer tick marks
            ...
            vedic_hours, vedic_minutes, vedic_seconds = (
                self.get_vedic_time_equal_day_night_ghati()
                if self.force_equal_day_night_ghati
            ...
            self.drawLabels(
                ...
                _CENTER_X,
                _CENTER_Y,
                _INNER_CIRCLE_RADIUS,
                _INNER_LABEL_STEP,
                _INNER_LABEL_COLOR,
            )  # Inner labels
            self.drawTickMarks(
                painter, _CENTER_X, _CENTER_Y, _INNER_CIRCLE_RADIUS, _INNER_TICK_STEP
            )  # Inner tick marks
    ...
    def drawCircles(self, painter):
        ...
        painter.drawEllipse(
            _CENTER_X - _OUTER_CIRCLE_RADIUS,
            _CENTER_Y - _OUTER_CIRCLE_RADIUS,
            2 * _OUTER_CIRCLE_RADIUS,
            2 * _OUTER_CIRCLE_RADIUS,
        )
        ...
        )  # fill with the widget's background color
        painter.drawEllipse(
            _CENTER_X - _INNER_CIRCLE_RADIUS,
            _CENTER_Y - _INNER_CIRCLE_RADIUS,
            2 * _INNER_CIRCLE_RADIUS,
            2 * _INNER_CIRCLE_RADIUS,
        )

        # Draw "Day" label
        day_angle = _SUNRISE_LABEL_DEGREES + _DAY_NIGHT_ANGLE_OFFSET
        day_angle_rad = day_angle * (math.pi / 180)
        day_x = int(
            _CENTER_X
            + (
                _INNER_CIRCLE_RADIUS
                + _day_night_radius_factor
                * (_OUTER_CIRCLE_RADIUS - _INNER_CIRCLE_RADIUS)
                / 2
            )
            * math.cos(day_angle_rad)
        )
        day_y = int(
            _CENTER_Y
            + (
        ...
        sunrise_x = int(
            _CENTER_X
            + (_OUTER_CIRCLE_RADIUS + 20) * math.cos(start_angle / 16 * math.pi / 180)
        )
        sunrise_y = int(
            _CENTER_Y
            + (_OUTER_CIRCLE_RADIUS + 20) * math.sin(start_angle / 16 * math.pi / 180)
        )
        self.sunrise_icon_pos = (
            int(sunrise_x) - self.sunrise_icon.width() // 2,
            int(sunrise_y) - self.sunrise_icon.height() // 2,
        )
        painter.drawPixmap(
            QPoint(self.sunrise_icon_pos[0], self.sunrise_icon_pos[1]),
            self.sunrise_icon,
        )
        ...
        sunset_x = int(
            _CENTER_X
            + (_OUTER_CIRCLE_RADIUS + 20) * math.cos(sunset_angle / 16 * math.pi / 180)
        )
        sunset_y = int(
            _CENTER_Y
            + (_OUTER_CIRCLE_RADIUS + 20) * math.sin(sunset_angle / 16 * math.pi / 180)
        )
        self.sunset_icon_pos = (
            int(sunset_x) - self.sunset_icon.width() // 2,
            int(sunset_y) - self.sunset_icon.height() // 2,
        )
        painter.drawPixmap(
            QPoint(self.sunset_icon_pos[0], self.sunset_icon_pos[1]), self.sunset_icon
        )
    ...
    def drawTickMarks(self, painter, centerX, centerY, radius, step):
        for i in range(0, _VEDIC_HOURS_PER_DAY, step):
            ...
            if i % 5 == 0:  # Label tick mark
                x2 = centerX + int(
                    (radius - _LABEL_TICK_MARK_LENGTH) * math.cos(math.radians(angle))
                )
                y2 = centerY + int(
                    (radius - _LABEL_TICK_MARK_LENGTH) * math.sin(math.radians(angle))
                )
            else:  # Regular tick mark
                x2 = centerX + int(
                    (radius - _TICK_MARK_LENGTH) * math.cos(math.radians(angle))
                )
                y2 = centerY + int(
                    (radius - _TICK_MARK_LENGTH) * math.sin(math.radians(angle))
                )
    ...
    def drawLabels(self, painter, centerX, centerY, radius, step, label_color):
        ...
        for i in range(0, _VEDIC_HOURS_PER_DAY + 1, step):
            ...
            x = (
                centerX
                + int((radius - _LABEL_OFFSET) * math.cos(math.radians(angle)))
                - 10
            )
            y = (
                centerY
                + int((radius - _LABEL_OFFSET) * math.sin(math.radians(angle)))
                + 10
            )
    ...
    def drawFixedCaptions(self, painter):
        ...
        for i in range(num_captions):
            angle = math.radians(
                _SUNRISE_LABEL_DEGREES + i * angle_step + _CAPTION_ANGLE_SHIFT
            )
            angle_degrees = i * angle_step

            # Calculate the position just outside the outer circle
            x = _CENTER_X + int(
                (_OUTER_CIRCLE_RADIUS + _CAPTION_RADIUS_INCREMENT) * math.cos(angle)
            )
            y = _CENTER_Y + int(
                (_OUTER_CIRCLE_RADIUS + _CAPTION_RADIUS_INCREMENT) * math.sin(angle)
            )
    ...
    def drawHand(self, painter, centerX, centerY, length, angle, color, arrowSize=10):
        ...
        endX = centerX + int(
            length * math.cos(math.radians(angle + _SUNRISE_LABEL_DEGREES))
        )
        endY = centerY + int(
            length * math.sin(math.radians(angle + _SUNRISE_LABEL_DEGREES))
        )
...
_CAPTION_FONT_SIZE = 12
_CLOCK_FONT_SIZE = 12
_LOCAL_CLOCK_COLOR = "red"
_VEDIC_CLOCK_COLOR = "green"
...
class VedicDigitalClock(QWidget):
    ...
    def __init__(
        ...
        jd=None,
        place=None,
        adjust_label_widths=False,
        show_local_clock=False,
        show_clock_caption=False,
        show_clock_label=True,
        ...
        self.show_clock_caption = show_clock_caption
        self.show_clock_label = show_clock_label
    ...
    def initUI(self):
        ...
        if self.show_clock_caption:
            if self.show_local_clock:
                self.local_caption = QLabel(self.res["local_clock_str"], self)
            self.vedic_caption = QLabel(utils.resource_strings["vedic_clock_str"], self)
        if self.show_clock_label:
            if self.show_local_clock:
                self.local_format_label = QLabel(
                    f"{self.res['hours_str']}:{self.res['minutes_str']}:{self.res['seconds_str']}",
                    self,
                )
            self.vedic_format_label = QLabel(
                f"{self.res['ghati_str']}:{self.res['pala_str']}:{self.res['vighati_str']}",
                self,
            )
        if self.show_local_clock:
        ...
        if self.show_clock_caption and self.show_clock_label:
            for label in [
                self.local_caption,
                self.local_format_label,
                self.vedic_caption,
                self.vedic_format_label,
            ]:
                if label:
        ...
        self.vedic_time_label.setFont(
            QFont("Arial", _CLOCK_FONT_SIZE, QFont.Weight.Bold)
        )
        self.vedic_time_label.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed
        )
        self.vedic_time_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.vedic_time_label.setStyleSheet(
            "color: " + _LOCAL_CLOCK_COLOR + "; min-width: 120px; max-width: 120px;"
        )
        if self.show_local_clock:
            self.local_time_label.setFrameStyle(QFrame.Shape.Box | QFrame.Shadow.Raised)
            self.local_time_label.setLineWidth(2)
            self.local_time_label.setFont(
                QFont("Arial", _CLOCK_FONT_SIZE, QFont.Weight.Bold)
            )
            self.local_time_label.setSizePolicy(
                QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed
            )
        ...
        self.vedic_time_label.setStyleSheet(
            "color: " + _VEDIC_CLOCK_COLOR + "; min-width: 120px; max-width: 120px;"
        )
        if self.show_clock_caption:
            layout.addWidget(self.local_caption)
        if self.show_local_clock:
            if self.show_clock_label:
                layout.addWidget(self.local_format_label)
            layout.addWidget(self.local_time_label)
            layout.addWidget(separator)
        if self.show_clock_caption:
            layout.addWidget(self.vedic_caption)
        if self.show_clock_label:
            layout.addWidget(self.vedic_format_label)
    ...
    def update_time(self):
        ...
        self.vedic_time_label.setText(
            f"{vedic_time[0]:02}:{vedic_time[1]:02}:{vedic_time[2]:02}"
        )
        if self.show_local_clock:
            self.local_time_label.setText(current_datetime.time().toString("HH:mm:ss"))
...
if __name__ == "__main__":
    ...
    clock_widget = VedicAnalogClock(
        language="ta", jd=jd, place=place
