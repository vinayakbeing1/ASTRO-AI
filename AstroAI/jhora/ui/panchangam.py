...
_info_label2_font_size = (
    4.87 if not _INFO_LABEL_HAS_SCROLL else 6
)  # 4.87# if _SHOW_MUHURTHA_OR_SHUBHA_HORA==0 else 5.9
...
def set_scrollbar_stylesheet(scroll_widget):
    scroll_widget.verticalScrollBar().setStyleSheet(
        f"""
        QScrollBar:vertical {{
...
class PanchangaInfoDialog(QWidget):
    def __init__(
        ...
        self._ayanamsa_mode = (
            ayanamsa_mode if ayanamsa_mode is not None else const._DEFAULT_AYANAMSA_MODE
        )
        ...
        if self.start_jd is None:
            ...
            self.start_jd = utils.julian_day_number(
                dob, (int(tob[0]), int(tob[1]), int(tob[2]))
            )
    ...
    def initUI(self):
        ...
        if self.info_labels_have_scroll:
            ...
            self._info_label2.setHorizontalScrollBarPolicy(
                Qt.ScrollBarPolicy.ScrollBarAlwaysOff
            )
            self._info_label2.setVerticalScrollBarPolicy(
                Qt.ScrollBarPolicy.ScrollBarAsNeeded
            )
            set_scrollbar_stylesheet(self._info_label2)
        self._info_label2.setMinimumHeight(self._info_label2_height)
        self._info_label2.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.MinimumExpanding
        )
        self._info_label2.setStyleSheet(
            "border: 1px solid black;"
            + " font-size:"
            + str(self._info_label2_font_size)
            + "pt"
        )
        ...
        if self.info_labels_have_scroll:
            ...
            self._info_label3.setHorizontalScrollBarPolicy(
                Qt.ScrollBarPolicy.ScrollBarAlwaysOff
            )
            self._info_label3.setVerticalScrollBarPolicy(
                Qt.ScrollBarPolicy.ScrollBarAsNeeded
            )
            set_scrollbar_stylesheet(self._info_label3)
        self._info_label3.setMinimumHeight(self._info_label3_height)
        self._info_label3.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.MinimumExpanding
        )
        self._info_label3.setStyleSheet(
            "border: 1px solid black;"
            + " font-size:"
            + str(self._info_label3_font_size)
            + "pt"
        )
    ...
    def update_panchangam_info(
        self, jd=None, place: drik.Place = None, ayanamsa_mode=None
    ):
        try:
            if jd is not None:
                self.start_jd = jd
            if place is not None:
                self.place = place
            self._ayanamsa_mode = (
                ayanamsa_mode if ayanamsa_mode is not None else self._ayanamsa_mode
            )
            ...
            info_list = self._fill_information_label1(show_more_link=False).split(
                sep_str
            )
    ...
    def _fill_information_label1(
        self, show_more_link=True, jd=None, place=None, ayanamsa_mode=None
    ):
        try:
            ...
            date_str2 = (
                str(year) + " " + utils.MONTH_LIST_EN[month - 1] + " " + str(day)
            )
            key = self.res["date_of_birth_str"]
            value = (
                date_str2
                + "  "
                + self.res["time_of_birth_str"]
                + " "
                + utils.to_dms(birth_time_hrs)
            )
            info_str += format_str.format(key, value)
            keys = [
                self.res["solar_str"]
                ...
                + "/"
                + self.res["month_str"],
                self.res["lunar_year_month_str"],
                self.res["lunar_year_month_str"],
            ]
            _calendar_type_str = [
                "",
                " (" + self.res["amantha_str"] + ")",
                " (" + self.res["purnimantha_str"] + ")",
            ]
            for _calendar_type in range(3):
                _month, _day, _year, adhik_maasa, nija_maasa = drik.vedic_date(
                    jd, place, calendar_type=_calendar_type
                )
                adhik_maasa_str = ""
                nija_month_str = ""
                if adhik_maasa:
            ...
            if len(_festival_list) > 0:
                ...
                for row in _festival_list:
                    info_str += format_str.format(
                        "\t",
                        row["Festival_" + const.available_languages[self._language]],
                    )
            # """
            ...
            value = (
                ...
                + ") "
                + self.res["paadham_str"]
                + str(nak[1])
                + " "
                + utils.to_dms(nak[3])
                + " "
                + self.res["ends_at_str"]
                + frac_str
            )
            info_str += format_str.format(key, value)
            if nak[3] < 24:
                ...
                value = (
                    utils.NAKSHATRA_LIST[_next_nak - 1]
                    + " "
                    + " ("
            ...
            if _SHOW_SPECIAL_TITHIS:
                for t in range(1, 13):
                    ...
                    frac_left = 100 * utils.get_fraction(
                        _tithi_returned[1], _tithi_returned[2], birth_time_hrs
                    )
                    frac_str = (
                        " ("
                        + "{0:.2f}".format(frac_left)
                    ...
                    if _tithi_returned[2] < 24:
                        _paksha = 0
                        if (_tithi_returned[0]) % 30 + 1 > 15:
                            _paksha = 1  # V3.1.1
                        value = (
                            utils.PAKSHA_LIST[_paksha]
                            + " "
                            + utils.TITHI_LIST[(_tithi_returned[0]) % 30]
                            + " ("
                ...
                frac_left = 100 * utils.get_fraction(
                    _tithi[1], _tithi[2], birth_time_hrs
                )
                frac_str = (
                    " ("
                    + "{0:.2f}".format(frac_left)
            ...
            yoga_lord = (
                " ("
                + utils.PLANET_SHORT_NAMES[
                    const.yogam_lords_and_avayogis[yogam[0] - 1][0]
                ]
                + "/"
                + utils.PLANET_SHORT_NAMES[
                    const.yogam_lords_and_avayogis[yogam[0] - 1][1]
                ]
                + ") "
            )
            value = (
            ...
            if yogam[2] < 24:
                yoga_lord = (
                    " ("
                    + utils.PLANET_SHORT_NAMES[
                        const.yogam_lords_and_avayogis[(yogam[0]) % 27][0]
                    ]
                    + "/"
                    + utils.PLANET_SHORT_NAMES[
                        const.yogam_lords_and_avayogis[(yogam[0]) % 27][1]
                    ]
                    + ") "
                )
                value = (
                    utils.YOGAM_LIST[(yogam[0]) % 27]
                    + yoga_lord
                    + "  "
                    + utils.to_dms(yogam[2])
                    + " "
            ...
            if karanam[2] < 24:
                ...
                value = (
                    utils.KARANA_LIST[_next_karana - 1]
                    + " ("
                    + karana_lord
                    + ") "
                    + utils.to_dms(karanam[2])
            ...
            value = (
                self.res["ay_" + const.anandhaadhi_yoga_names[ay[0]] + "_str"]
                + " "
                + utils.to_dms(ay[1])
                + " "
                + self.res["starts_at_str"]
            ...
            value = (
                utils.to_dms(_day_length).replace(" AM", "").replace(" PM", "")
                + " "
                + self.res["hours_str"]
            )
            info_str += format_str.format(key, value)
            ...
            value = (
                utils.to_dms(_night_length).replace(" AM", "").replace(" PM", "")
                + " "
                + self.res["hours_str"]
            )
            info_str += format_str.format(key, value)
            ...
            value += (
                " (" + self.res[const.tamil_yoga_names[tg[3]] + "_yogam_str"] + ")"
                if len(tg) > 3 and tg[0] != tg[3]
                else ""
            )
            value += (
                "&nbsp;&nbsp;"
            ...
            if len(value) > 0:
                key = (
                    self.res["dwi_pushkara_yoga_str"]
                    if value[0] == 1
                    else self.res["tri_pushkara_yoga_str"]
                )
            ...
            if len(value) > 0:
                ...
                value = (
                    utils.to_dms(value[0])
                    + " "
                    + self.res["starts_at_str"]
                    + " "
                    + utils.to_dms(value[1])
                    + " "
                    + self.res["ends_at_str"]
                )
                info_str += format_str.format(key, value)
            ...
            value = (
                self.res["shiva_vaasa_str" + str(sv[0])]
                + " "
                + utils.to_dms(sv[1])
                + " "
                + self.res["ends_at_str"]
            ...
            value = (
                self.res["agni_vaasa_str" + str(av[0])]
                + " "
                + utils.to_dms(av[1])
                + " "
                + self.res["ends_at_str"]
            ...
            key = self.res["yogini_vaasa_str"]
            value = self.res[directions[yv] + "_str"]
            info_str += format_str.format(key, value)
            ds = drik.disha_shool(jd)
            key = self.res["disha_shool_str"]
            value = self.res[directions[ds] + "_str"]
            info_str += format_str.format(key, value)
            car, ca_jd = drik.chandrashtama(jd, place)
            key = self.res["chandrashtamam_str"]
            value = (
                utils.RAASI_LIST[car - 1]
                + " "
                + utils.julian_day_to_date_time_string(ca_jd)
                + " "
                + self.res["ends_at_str"]
            )
            # """
            ...
            value = (
                utils.to_dms(value, as_string=True, is_lat_long="lat")
                .replace("N", "")
                .replace("S", "")
            )
            info_str += format_str.format(key, value)
            ...
            bird_index = pancha_paksha._get_birth_bird_from_nakshathra(
                nak[0], paksha_index
            )
            key = (
                self.res["pancha_pakshi_sastra_str"]
                + " "
                + self.res["main_bird_str"].replace("\\n", " ")
                + " : "
            )
            value = utils.resource_strings[
                pancha_paksha.pancha_pakshi_birds[bird_index - 1] + "_str"
            ]
            info_str += format_str.format(key, value)
            ...
            tb = sys.exc_info()[2]
            print(
                f"PanchangaInfoDialog - An error occurred: {e}",
                "line number",
                tb.tb_lineno,
            )
    ...
    def _fill_information_label2(self):
        try:
            ...
            info_str += format_str.format(key, value)
            info_str += header.format(
                self.res["daytime_str"] + " " + self.res["gauri_choghadiya_str"] + ":"
            )
            ...
            for g, (gt, st, et) in enumerate(gc):
                if g == 8:  # V4.3.6
                    info_str += header.format(
                        self.res["nighttime_str"]
                        + " "
                        + self.res["gauri_choghadiya_str"]
                        + ":"
                    )
            ...
            info_str += header.format(
                self.res["daytime_str"] + " " + self.res["muhurtha_str"] + ":"
            )
            mh = drik.muhurthas(jd, place)
            for mi, (mn, ma, (ms, me)) in enumerate(mh):
                if mi == 15:
                    info_str += header.format(
                        self.res["nighttime_str"] + " " + self.res["muhurtha_str"] + ":"
                    )
                key = (
                    "&nbsp;&nbsp;"
            ...
            # else:
            info_str += header.format(
                self.res["daytime_str"] + " " + self.res["shubha_hora_str"] + ":"
            )
            ...
            for g, (gt, st, et) in enumerate(gc):
                ...
                if g == 12:
                    info_str += header.format(
                        self.res["nighttime_str"]
                        + " "
                        + self.res["shubha_hora_str"]
                        + ":"
                    )
                key = (
                    "&nbsp;&nbsp;"
                    + utils.PLANET_NAMES[gt]
                    + " "
                    + self.res["shubha_hora_" + str(gt)]
                )
                value = (
                    st
                    + " "
            ...
            for prc, pr_beg, pr_end in self.panchaka_rahitha[:1]:
                key = (
                    self.res["muhurtha_str"] + " (" + self.res["good_str"] + ") "
                    if prc == 0
                    else self.res[bad_panchakas[prc] + "_panchaka_str"]
                )
                value1 = (
                    utils.to_dms(pr_beg) + " " + utils.resource_strings["starts_at_str"]
                )
                value2 = (
                    utils.to_dms(pr_end) + " " + utils.resource_strings["ends_at_str"]
                )
    ...
    def _fill_information_label3(self):
        try:
            ...
            star_list = [utils.NAKSHATRA_LIST[t - 1] for t in tb]
            knt = 6
            star_list = [
                " ".join(map(str, star_list[i : i + knt]))
                for i in range(0, len(star_list), knt)
            ]
            for sl in star_list:
                info_str += format_str.format("", sl)
            cb = drik.chandrabalam(jd, place)
            info_str += header.format(self.res["chandrabalam_str"] + " :")
            star_list = [utils.RAASI_LIST[t - 1] for t in cb]
            knt = 5
            star_list = [
                " ".join(map(str, star_list[i : i + knt]))
                for i in range(0, len(star_list), knt)
            ...
            for ulr, ulb, ule in ulm:
                ...
                ulb_str = utils.to_dms(ulb)
                ule_str = utils.to_dms(ule)
                value = (
                    ulb_str
                    + " "
                    + self.res["starts_at_str"]
            ...
            value = _paksha + " " + utils.TITHI_LIST[kt[0] - 1]
            _t_deity = utils.TITHI_DEITIES[kt[0] - 1]
            value_str = (
                '<span style="color:'
                + _VALUE_COLOR
                + ';">'
...
class PanchangaWidget(QWidget):
    def __init__(
        ...
        time_of_birth=None,
        place_of_birth=None,
        show_vedic_digital_clock=False,
        ...
        self.show_vedic_analog_clock = (
            show_vedic_analog_clock and show_vedic_digital_clock
        )
    ...
    def _update_clock_tab_text(self):
        ...
        if self.show_local_clock:
            lc = (
                self.resources["present_str"]
                + " "
                + f"{self.resources['time_of_birth_str']} "
            )
            local_time = lc + current_datetime.time().toString("HH:mm:ss") + " / "
        vedic_time = drik.float_hours_to_vedic_time(
            jd, place, current_time, vedic_hours_per_day=_VEDIC_HOURS_PER_DAY
        )
        vc1 = self.resources["vedic_clock_str"] + " "
        vc = (
            vc1
            + f"{self.resources['ghati_str']}:{self.resources['pala_str']}:{self.resources['vighati_str']} "
        )
        vedic_time_str = (
            local_time
            + vc
            + f"{vedic_time[0]:02}:{vedic_time[1]:02}:{vedic_time[2]:02}"
        )
        self.tabWidget.setTabText(1, vedic_time_str)
        self.tabWidget.tabBar().setStyleSheet(
            "QTabBar::tab { color: green; font-weight: bold; }"
        )
    ...
    def _ayanamsa_selection_changed(self):
        ...
        drik.set_ayanamsa_mode(
            self._ayanamsa_mode, jd=self.julian_day
        ) if self._ayanamsa_mode.upper() == "SUNDAR_SS" else drik.set_ayanamsa_mode(
            self._ayanamsa_mode
        )
    ...
    def _save_info_labels_as_images(self, image_id, image_files):
        ...
        for page in range(max_pages):
            label1.verticalScrollBar().setValue(
                int(
                    min(page, pages_label1 - 1)
                    * label1.verticalScrollBar().maximum()
                    / (pages_label1 - 1)
                )
            )
            label2.verticalScrollBar().setValue(
                int(
                    min(page, pages_label2 - 1)
                    * label2.verticalScrollBar().maximum()
                    / (pages_label2 - 1)
                )
            )
            label3.verticalScrollBar().setValue(
                int(
                    min(page, pages_label3 - 1)
                    * label3.verticalScrollBar().maximum()
                    / (pages_label3 - 1)
                )
            )
