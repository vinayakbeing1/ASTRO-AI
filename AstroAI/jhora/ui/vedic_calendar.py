...
_shukla_paksha_icon = (
    os.path.abspath(const._IMAGES_PATH) + const._sep + "shukla_paksha.png"
)
_krishna_paksha_icon = (
    os.path.abspath(const._IMAGES_PATH) + const._sep + "krishna_paksha.png"
)
_INFO_LABELS_HAVE_SCROLL = True
_info_label1_font_size = 4.87  # if not _INFO_LABELS_HAVE_SCROLL else 6
_info_label2_font_size = (
    4.87 if (not _INFO_LABELS_HAVE_SCROLL or _SHOW_MUHURTHA_OR_SHUBHA_HORA == 1) else 5
)
...
_top_left_font_size = 9
_top_right_font_size = 9
_bottom_left_font_size = 9
_bottom_right_font_size = 9
_center_left_font_size = 12
_center_right_font_size = 16
_calendar_cell_width = 100
_calendar_cell_height = 80
_label_offset_x = 2
_label_offset_y = 1
_info_label1_width = 325
_HEADER_CELL_HEIGHT = 20
_HEADER_COLOR = "green"
_HEADER_LABEL_HEIGHT = 50
_HEADER_LABEL_COLOR = "maroon"
_KEY_COLOR = "brown"
_VALUE_COLOR = "blue"
_KEY_LENGTH = 50
_VALUE_LENGTH = 50
...
previous_month_color = "orange"
default_color = "lightyellow"
input_day_color = "lightgreen"
next_month_color = "lightblue"
_top_left_label_color = "magenta"
_top_right_label_color = "blue"
_middle_left_label_color = "red"
_middle_right_label_color = "black"
_bottom_left_label_color = "green"
_bottom_right_label_color = "blue"
_cell_border_line_color = "brown"
_cell_border_line_thickness = 1
...
class CustomLabel(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        self.font_sizes = {
            "top_left": _top_left_font_size,
            "top_right": _top_right_font_size,
            "middle_left": _center_left_font_size,
            "middle_right": _center_right_font_size,
            "bottom_left": _bottom_left_font_size,
            "bottom_right": _bottom_right_font_size,
            "center": _center_right_font_size,
        }
        self.colors = {
            "top_left": QColor(_top_left_label_color),
            "top_right": QColor(_top_right_label_color),
            "middle_left": QColor(_middle_left_label_color),
            "middle_right": QColor(_middle_right_label_color),
            "bottom_left": QColor(_bottom_left_label_color),
            "bottom_right": QColor(_bottom_right_label_color),
            "center": QColor(_middle_right_label_color),
        }
        self.setStyleSheet(
            "border: "
            + str(_cell_border_line_thickness)
            + "px solid "
            + _cell_border_line_color
            + ";"
        )
        ...
        try:
            ...
            for pos, value in self.texts.items():
                ...
                cell_y = (
                    ...
                    if "middle" in pos
                    else 2 * self.height() // 3
                )
                cell_y = self.height() // 2 if "center" in pos else cell_y

                text_rect = QRect(cell_x, cell_y, cell_width, cell_height)
                rect = QRect(
                    cell_x + _label_offset_x,
                    cell_y + _label_offset_y,
                    text_rect.width() - _label_offset_x,
                    text_rect.height() - _label_offset_y,
                )
                ...
                if pos == "center" and isinstance(value, list):
                    ...
                    for item in value:
                        ...
                        default_size = (
                            _ICON_SIZE_MEDIUM if len(value) < 5 else _ICON_SIZE_SMALL
                        )
                        icon_rect = QRect(icon_x, cell_y, default_size, default_size)

                        if self.hovered_icon_rect and self.hovered_icon_rect.contains(
                            icon_rect.center()
                        ):
                    ...
                    if isinstance(value, tuple):
                        ...
                        icon_size = (
                            _ICON_SIZE_MEDIUM if "center" in pos else _ICON_SIZE_SMALL
                        )
                        icon_height = (
                            int(0.8 * self.height()) if "bottom" in pos else cell_y
                        )
        ...
        try:
            ...
            for rect, tooltip in self.icon_tooltips:
                if rect.contains(event.pos()):
                    ...
                    tooltip_pos = event.globalPosition().toPoint() + QPoint(
                        _TOOLTIP_ZOOM_OFFSET, _TOOLTIP_ZOOM_OFFSET
                    )
...
class VedicCalendar(QWidget):
    def __init__(
        self,
        start_date: drik.Date = None,
        place: drik.Place = None,
        language="ta",
        use_purnimanta_system=None,
        ...
        self.previous_month_cell = None
        self.next_month_cell = None
    ...
    def initUI(self):
        ...
        _calendar_list = [
            self.res[s] + " " + self.res["calendar_str"]
            for s in ["solar_str", "amantha_str", "purnimantha_str"]
        ]
        self._calendar_combo.addItems(_calendar_list)
        self._calendar_combo.activated.connect(
            self._change_calendar_type
        )  # self._change_language)
        _calendar_index = (
            0
            if self._use_purnimanta_system is None
            else (2 if self._use_purnimanta_system else 1)
        )
        ...
        self._info_label1.setStyleSheet(
            "border: 1px solid black; font-size: " + str(_info_label1_font_size) + " pt"
        )
        ...
        self._info_label1.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextBrowserInteraction
        )
        self._info_label1.setOpenExternalLinks(
            False
        )  # Disable opening links in the default browser
        h_layout.addWidget(self._info_label1)
        v_layout = QVBoxLayout()
        self.header_label = QLabel("")
        self.header_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.header_label.setStyleSheet(
            "border: 1px solid black; font-weight: bold; font-size: " + str(12) + "pt"
        )
        ...
        headers = [
            '<b><span style="color:' + _HEADER_COLOR + ';">' + str(h) + "</span></b>"
            for h in utils.DAYS_SHORT_NAMES
        ]
        self.day_labels = ["" for _ in utils.DAYS_SHORT_NAMES]
        for col, header in enumerate(headers):
            self.day_labels[col] = QLabel(header)
            self.day_labels[col].setFixedHeight(
                _HEADER_CELL_HEIGHT
            )  # Set a fixed height for the header
            self.day_labels[col].setAlignment(
        ...
        for row in range(1, 8):
            ...
            for col in range(7):
                ...
                week.append(cell)
                jd_week.append(None)
            self.cells.append(week)
            self.jd.append(jd_week)
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        try:
            ...
            def _check_if_valid_cell(next_cell):
                ...
                _invalid_cell = (
                    (next_row == row_min and next_col < previous_month_col)
                    or (next_row == row_max and next_col > next_month_col)
                    or (next_row < row_min or next_row > row_max)
                    or (next_col < col_min or next_col > col_max)
                )
            ...
            if event.key() == Qt.Key.Key_Up:
                next_cell = (
                    ...
                    else (row_max, current_col)
                )
            elif event.key() == Qt.Key.Key_Down:
                next_cell = (
                    (current_row + 1, current_col)
                    if current_row != row_max
                    else (row_min, current_col)
                )
            elif event.key() == Qt.Key.Key_Left:
                next_cell = (
                    (current_row, current_col - 1)
                    if current_col != col_min
                    else (current_row - 1, col_max)
                )
            elif event.key() == Qt.Key.Key_Right:
                next_cell = (
                    (current_row, current_col + 1)
                    if current_col != col_max
                    else (current_row + 1, col_min)
                )
            else:
            ...
            tb = sys.exc_info()[2]
            print(
                f"VedicCalendar:keyEventPressed: - An error occurred: {e}",
                "line number",
                tb.tb_lineno,
            )
    ...
    def _on_show_more_link_clicked(self, link, jd, place):
        if link == "show_more":
            try:
                ...
                panchanga_widget = PanchangaInfoDialog(
                ...
                print(
                    f"VedicCalendar:_on_show_more_link_clicked: An error occurred: {e}"
                )
    ...
    def _get_days_panchanga_info(self, row, col):
        ...
        if jd is not None:
            ...
            date_in = drik.Date(y, m, d)
            _tithi_returned = drik.tithi(jd, place)
            _tit = _tithi_returned[0]
            ...
            _srise = utils.to_dms(
                drik.sunrise(jd, place)[0], round_to_minutes=True
            ).strip()
            _sset = utils.to_dms(
                drik.sunset(jd, place)[0], round_to_minutes=True
            ).strip()
            _naks = drik.nakshatra(jd, place)
            _nak_id = _naks[0]
            ...
            if self._use_purnimanta_system is None:
                ...
                tm, td, _lunar_year, adhik_maasa, nija_maasa = drik.lunar_month_date(
                    jd, place, use_purnimanta_system=self._use_purnimanta_system
                )
                tm -= 1
                adhik_maasa_str = ""
                if adhik_maasa:
        ...
        calendar_type = (
            0
            if self._use_purnimanta_system == None
            else (2 if self._use_purnimanta_system else 1)
        )
        ...
        if len(_tithi_returned) > 3:
            kp_ttip += (
                ...
                + self.res["after_str"]
                + " "
                + utils.TITHI_LIST[_tithi_returned[3] - 1]
            )
        nak_ttip = (
            utils.NAKSHATRA_LIST[_naks[0] - 1]
            + " "
            + " ("
            + utils.PLANET_SHORT_NAMES[utils.nakshathra_lord(_naks[0])]
            + ") "
            + self.res["paadham_str"]
            + str(_naks[1])
            + " "
            + utils.to_dms(_naks[3])
            + " "
            + self.res["ends_at_str"]
        )
        if _naks[3] < 24:
        ...
        _panchanga_dict = {
            ...
            "middle_left": (
                _tithi_icon,
                str(td),
                utils.TITHI_LIST[_tit - 1] if _tit in _tithi_icons.keys() else "",
            ),
            "middle_right": ("", str(d)),
            ...
            "top_right": (
                _sunrise_icon,
                _srise,
                self.res["sunrise_str"] + " " + str(_srise),
            ),
            "bottom_right": (
                _sunset_icon,
                _sset,
                self.res["sunset_str"] + " " + str(_sset),
            ),
        }
        header_text = (
            year_str
            ...
            + str(y)
            + " "
            + utils.MONTH_LIST_EN[m - 1]
            + " "
            + str(d)
            + " "
            + utils.DAYS_LIST[drik.vaara(jd)]
            + " "
            + spl_month_text
            + " "
            + utils.PAKSHA_LIST[_paksha]
    ...
    def computeCalendar(self):
        try:
            ...
            year, month, day = utils.get_year_month_day_from_date_format(
                date_str
            ...
            for row in range(7):
                for col in range(7):
                    ...
                    if row * 7 + col >= start_day:
                        ...
                        sunrise_hours = drik.sunrise(_jd, self.start_place)[0]
                        _jd = utils.julian_day_number(
                            (_year, _month, _day), (sunrise_hours, 0, 0)
                        )
                        self.jd[row][col] = _jd
                        self.row_max = row
                        cell.setVisible(True)
                        panchanga_dict, _ = self._get_days_panchanga_info(row, col)
                        if panchanga_dict is not None:
                            cell.set_texts(panchanga_dict)
                        _cell_style = (
                            "border: "
                            + str(_cell_border_line_thickness)
                            + "px solid "
                        ...
                        current_date = utils.next_panchanga_day(
    ...
    def cell_clicked(self, row, col):
        try:
            ...
            font = QFont()
            font.setPointSizeF(_info_label1_font_size)
            self._info_label1.setFont(font)
            ...
            if (
                self.selected_cell == self.previous_month_cell
                or self.selected_cell == self.next_month_cell
            ):
                self.computeCalendar()
            ...
            tb = sys.exc_info()[2]
            print(
                f"VedicCalendar:cell_clicked: An error occurred: {e}",
                "line number",
                tb.tb_lineno,
            )
    ...
    def _fill_information_label1(self, jd, place, show_more_link=True):
        try:
            ...
            info_str = PanchangaInfoDialog._fill_information_label1(
                self,
