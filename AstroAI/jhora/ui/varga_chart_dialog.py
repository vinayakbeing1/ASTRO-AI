...
def get_varga_option_dict():
    ...
    if const.TREAT_STANDARD_CHART_AS_CUSTOM:
        ...
        for dcf in const.division_chart_factors[1:]:
            _opt_count = len(
                [k for k in _res.keys() if "d" + str(dcf) + "_option" in k]
            )
            _varga_option_dict[dcf] = (_opt_count, 1, None, None)
        for dcf in [
            d
            for d in range(2, const.MAX_DHASAVARGA_FACTOR + 1)
            if d not in const.division_chart_factors
        ]:
            _opt_count = len([k for k in _res.keys() if "dn_custom_option" in k])
            _varga_option_dict[dcf] = (_opt_count, 0, None, None)
...
class VargaChartOptionsDialog(QDialog):
    ...
    def __init__(
        ...
        if varga_factor == None and (
            chart_index == None or chart_index < self._custom_chart_index
        ):
    ...
    def create_ui(self):
        ...
        self._custom_non_cycle_option = QRadioButton(
            self.res["dn_custom_non_cyclic_str"]
        )
        ...
        self._custom_count_from_end_of_sign_check = QCheckBox(
            self.res["dn_custom_count_from_end_of_sign_str"]
        )
        if self._count_from_end_of_sign != None:
            self._custom_count_from_end_of_sign_check.setChecked(
                self._count_from_end_of_sign
            )
        if self._varga_factor not in const.division_chart_factors:
            ...
            self._custom_dvf_combo.addItems(
                _dvf_list
            )  # setRange(2,const.MAX_DHASAVARGA_FACTOR+1)
            # self._custom_dvf_combo.setSingleStep(1)
            if (
                str(self._varga_factor) in _dvf_list
            ):  # range(2,const.MAX_DHASAVARGA_FACTOR+1):
                self._custom_dvf_combo.setCurrentText(str(self._varga_factor))
            else:
                self._custom_dvf_combo.setCurrentText(
                    str(const.DEFAULT_CUSTOM_VARGA_FACTOR)
                )
            h_layout.addWidget(self._custom_dvf_combo)
            v_layout.addLayout(h_layout)
            h_layout = QHBoxLayout()
            self._custom_cycle_option.toggled.connect(self._custom_cyclic_changed)
            self._custom_cycle_option.setChecked(
                self._method_index == 0 and self._base_rasi_index == None
            )
            ...
            if self._base_rasi_index != None:
                self._custom_base_combo.setCurrentIndex(self._base_rasi_index)
            v_layout.addLayout(h_layout)
        if self._varga_factor in const.division_chart_factors:
            ...
            self._varga_option_label.setText(
                self.res["varga_option_str"]
                + " "
                + self.res[_chart_names[varga_chart_index]]
            )
        else:
            self._varga_option_label.setText(
                self.res["varga_option_str"] + " " + self.res[_chart_names[-1]]
            )
        v_layout.addWidget(self._varga_option_label)
        for mc in range(self._method_count):
            if self._varga_factor in const.division_chart_factors:
                rb_caption = self.res[
                    "d" + str(self._varga_factor) + "_option" + str(mc + 1) + "_str"
                ]
            else:
                rb_caption = self.res["dn_custom_option" + str(mc) + "_str"]
            self._method_rbs[mc].setText(
                rb_caption
            )  # .append(QRadioButton(rb_caption))
            if (
                self._varga_factor in const.division_chart_factors
                and self._method_index == (mc + 1)
            ):
                self._method_rbs[mc].setChecked(True)
            if (
                self._varga_factor not in const.division_chart_factors
                and self._method_index == (mc)
            ):
                self._method_rbs[mc].setChecked(True)
            v_layout.addWidget(self._method_rbs[mc])
        self._option_string = rb_caption
        self._custom_non_cycle_option.setChecked(
            not self._custom_cycle_option.isChecked()
        )
        self._custom_count_from_end_of_sign_check.setChecked(False)
        if self._count_from_end_of_sign == True:
            self._custom_count_from_end_of_sign_check.setChecked(True)
        if self._varga_factor not in const.division_chart_factors:
    ...
    def _accept_button_clicked(self):
        ...
        for mc, rb in enumerate(self._method_rbs):
            if rb.isChecked():
                ...
                if (
                    self._custom_cycle_option.isVisible()
                    and self._custom_cycle_option.isChecked()
                ):
                    self._method_index = 0
                elif (
                    self._custom_non_cycle_option.isVisible()
                    and self._custom_non_cycle_option.isChecked()
                ):
        ...
        if not self._custom_base_combo.isVisible():
            ...
            bstr = (
                "<br>(Standard) base_rasi=None"
                if self._varga_factor in const.division_chart_factors
                else "<br>(Custom-Cyclic) base_rasi=None"
            )
        else:
            self._base_rasi_index = self._custom_base_combo.currentIndex()
            bstr = (
                "<br>Custom Non-Cyclic base_rasi=" + utils.RAASI_LIST[0]
                if self._base_rasi_index == 0
                else "base is the sign"
            )
    ...
    def _cancel_button_clicked(self):
        ...
        if self._varga_factor in const.division_chart_factors:
            rb_caption = self.res[
                "d"
                + str(self._varga_factor)
                + "_option"
                + str(self._method_index)
                + "_str"
            ]
        else:
