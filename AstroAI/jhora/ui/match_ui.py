...
class MatchWindow(QWidget):
    def __init__(self, chart_type="south indian"):
        ...
        self._results_table.setHorizontalHeaderItem(
        ...
        self._results_table.setSizeAdjustPolicy(
    ...
    def _get_compatibility(self):
        ...
        self._matching_stars_tuple = utils.sort_tuple(
            comp.get_matching_partners(), 4, reverse=True
        )
    ...
    def _get_compatibility_old(self):
        ...
        for p, porutham in enumerate(ettu_poruthham_list):
            ...
            self._results_table.setItem(
                row, 1, QTableWidgetItem(str(ettu_porutham_results[p][0]))
            )
            self._results_table.setItem(
                row, 2, QTableWidgetItem(str(ettu_porutham_results[p][1]))
            )
            row += 1
        ...
        self._results_table.setItem(
            row, 0, QTableWidgetItem("Overall Compatibility Score:")
        )
    ...
    def _update_compatibility_table(self):
        ...
        paadham = (
            "Paadham"
            + str(selected_matching_star_tuple[1])
            + "-"
            + str(selected_matching_star_tuple[2])
        )
        result = ""
