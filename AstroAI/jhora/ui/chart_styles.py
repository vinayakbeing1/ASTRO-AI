...
color_label = (
    lambda label, color: "<span style=color:" + color + ";>" + label + "</span>"
)
_rasi_color = "Red"
_planet_color = "Brown"
_text_color = "Green"
...
def replace_ascendant_with_prasna_lagna(
    planet_list, prasna_list, chart_type="south indian"
):
    ...
    if "south" in chart_type.lower() or "east" in chart_type.lower():
        prasna_2d = utils._convert_1d_house_data_to_2d(
            prasna_list, chart_type=chart_type
        )
...
class PrasnaDialog(QDialog):
    def __init__(self, parent=None, varga_factor=1):
        ...
        self.setWindowTitle(
            utils.resource_strings["prasna_lagna_str"]
            + " "
            + utils.resource_strings["options_str"]
        )
        ...
        # Radio Buttons
        self.radio_prasna108 = QRadioButton(
            utils.resource_strings["prasna_lagna_str"] + "-108"
        )
        self.radio_prasnakp249 = QRadioButton(
            utils.resource_strings["prasna_lagna_str"] + "KP-249"
        )
        self.radio_prasna_nadi = QRadioButton(
            utils.resource_strings["naadi_str"] + "-1800"
        )
        ...
        self._spin_label = QLabel(
            (
                utils.resource_strings["prasna_label_str"]
                + " (1.."
                + str(self._spin_max)
                + ")"
            )
        ...
        self.keep_lagna_radio_button = QRadioButton(
            utils.resource_strings["prasna_replace_lagna_label1_str"]
        )
        ...
        self.replace_lagna_radio_button = QRadioButton(
            utils.resource_strings["prasna_replace_lagna_label2_str"]
        )
    ...
    def on_radio_button_changed(self):
        ...
        if self.random_checkbox.isChecked():
            self.spin_box.setValue(random.randint(1, self._spin_max))
    ...
    def accept_clicked(self):
        ...
        if self.radio_prasna108.isChecked():
            ...
            plag = utils.get_prasna_lagna_KP_249_for_varga_chart(
                kp_no, self._varga_factor
            )
        elif self.radio_prasna_nadi.isChecked():
            plag = utils.get_prasna_lagna_nadi_for_varga_chart(
                kp_no, self._varga_factor
            )
...
class SudarsanaChakraChart(QWidget):
    ...
    def __init__(
        self,
        data=None,
        chart_center_pos: tuple = (175, 175),
        chart_radii: tuple = (75, 125, 175),
        chart_inner_square: tuple = (30, 30),
        label_font_size: int = 8,
        chart_label_radius_factor: float = 0.15,
        chart_size_factor: float = 1.0,
        chart_title_font_size=9,
        chart_title="",
    ):
    ...
    def _draw_sudarsana_chakra_chart(self):
        ...
        if self.chart_title_font_size != None:
            ...
            painter.setFont(font)
        painter.drawText(
            title_rect, Qt.AlignmentFlag.AlignCenter, self._chart_title.strip()
        )
        for i, r in enumerate(r_list[:-1]):
            data_1d = self.data[i]
            r1 = r_list[i]
            r2 = r_list[(i + 1)]
            pr = r1 + self._sc_label_radius_factor * (
                r2 - r1
            )  # r2-self._sc_label_radius_factor*(r2-r1)
            ...
            for i_z in range(12):
                ...
                self._write_planets_inside_houses(
                    painter, pr, t1, t2, data_1d[i_z], i_z
                )
    ...
    def drawNode(self, painter, angle, radius, text):
        ...
        painter.drawText(
            QtCore.QRectF(0, -size / 2.0, size, size),
    ...
    def _write_planets_inside_houses_1(self, painter, radius, data, i_z):
        ...
        rect = QtCore.QRect(
            int(cx + radius * ri * math.cos(a)),
            int(cy - radius * ri * math.sin(a)),
            40,
            40,
        )
...
class WesternChart(QWidget):
    ...
    def __init__(
        self,
        data=None,
        chart_center_pos: tuple = (_west_chart_center_x, _west_chart_center_y),
        chart_radii: tuple = (
            _west_chart_radius_1,
            _west_chart_radius_2,
            _west_chart_radius_3,
            _west_chart_radius_4,
        ),
        label_font_size=_west_label_font_size,
        label_pos_radial_increment=_west_radial_increment,
        chart_size_factor: float = 1.0,
        chart_title_font_size=_west_chart_title_font_size,
        chart_title="",
    ):
        QWidget.__init__(self)
        self._chart_title = chart_title
        self._data_counter = 0
        self._chart_center_pos = tuple(
            [int(x * chart_size_factor) for x in chart_center_pos]
        )
        self._chart_radii = tuple([int(x * chart_size_factor) for x in chart_radii])
        self._label_font_size = label_font_size
        self._label_pos_radial_increment = (
            label_pos_radial_increment * chart_size_factor
        )
    ...
    def _draw_western_chart(self):
        ...
        for i_z in range(12):
            ...
            rect = QtCore.QRect(
                int(cx + r34 * math.cos(house_mid_angle * math.pi / 180.0)),
                int(cy - r34 * math.sin(house_mid_angle * math.pi / 180.0)),
                10,
                10,
            )
        ...
        _chart_title = (
            self._chart_title.strip().split()[0]
            if self._chart_title.strip() != ""
            else ""
        )
    ...
    def _write_planets_inside_houses(self, painter, radius, data, i_z):
        ...
        planet = (
            tmp_arr[0][-2:].strip()
            if const._retrogade_symbol in tmp_arr[0]
            else tmp_arr[0][-1:].strip()
        )
        ...
        house_index = (
            zodiac_index - self._asc_house + 12 + 5
        ) % 12  # +5 to account for 150 degrees for ASC house start
        ...
        for i, c in enumerate(text_new.split()):
            rect = QtCore.QRect(
                int(cx + (radius - (i + 1) * ri) * math.cos(a)),
                int(cy - (radius - (i + 1) * ri) * math.sin(a)),
                20,
                12,
            )
    ...
    def setData(
        self, data, chart_title="", chart_title_font_size=None
    ):  # ,event=None):
        self._chart_title = chart_title
...
class EastIndianChart(QWidget):
    ...
    def __init__(
        ...
        chart_house_size: tuple = (
            _east_chart_house_x,
            _east_chart_house_y,
            _east_chart_house_width,
            _east_chart_house_height,
        ),
        ...
        self._ndl_22_dialog = None
        self._ndl_64_dialog = None
        self._graha_drekkana_dialog = None
        self._nava_thaara_dialog = None
        self._spl_thaara_dialog = None
        if self.data == None:
            self.data = ["", "", "", "", "", "", "", "", "", "", "", ""]
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                action.triggered.connect(
                    lambda checked, key=key: self.set_menu_data(key)
                )
    ...
    def set_menu_data(self, key):
        ...
        if (
            ...
            == utils.resource_strings["prasna_lagna_str"]
            + "("
            + utils.resource_strings["prasna_lagna_short_str"]
            + ")"
        ):
            ...
            if _replace_lagna:
                ...
                (
                    ...
                    alt_data,
                    self._asc_house,
                ) = replace_ascendant_with_prasna_lagna(
                    self._data_original, alt_data, "east indian"
                )
            else:
                # replace 'lagna(plag) with lagna
                lagna = utils.resource_strings["ascendant_str"]
                plag = utils.resource_strings["prasna_lagna_short_str"]
                self.data = utils.search_replace(
                    self.data, lagna + "(" + plag + ")", lagna
                )
            if len(alt_data) > 0:
                action = self.sender()
            ...
            utils.resource_strings["yogi_sphuta_str"]
            + ", "
            + utils.resource_strings["avayogi_sphuta_str"]
            + ", "
            + utils.resource_strings["sahayogi_str"],
            utils.resource_strings["paachakaadi_sambhandha_str"],
            utils.resource_strings["brahma_str"]
            + ","
            + utils.resource_strings["rudra_str"]
            + ","
            + utils.resource_strings["maheshwara_str"],
            utils.resource_strings["combustion_str"]
            + " "
            + utils.resource_strings["planets_str"],
            utils.resource_strings["graha_yudh_str"],
            utils.resource_strings["marana_karaka_sthana_str"]
            + " "
            + utils.resource_strings["planets_str"],
            utils.resource_strings["mrityu_bhaga_str"],
            utils.resource_strings["raasi_str"]
            + " "
            + utils.resource_strings["entry_str"],
            ...
            _title = (
                utils.resource_strings["raasi_str"]
                + "-"
                + utils.resource_strings["graha_str"]
                + "-"
                + utils.resource_strings["drishti_str"]
            )
            if self._drishti_dialog is None and self._drishti_table_widgets is not None:
                self._drishti_dialog = WidgetDialog(
                    title=_title, h_widgets=self._drishti_table_widgets
                )
            ...
            _title = key
            if (
                self._nava_thaara_dialog is None
                and self._nava_thaara_widgets is not None
            ):
                self._nava_thaara_dialog = WidgetDialog(
                    title=_title,
                    h_widgets=self._nava_thaara_widgets,
            ...
            if self._spl_thaara_dialog is None and self._spl_thaara_widgets is not None:
                self._spl_thaara_dialog = WidgetDialog(
                    title=_title,
                    h_widgets=self._spl_thaara_widgets,
                    fit_to_widget_contents=True,
                )
            self._spl_thaara_dialog.exec()
        elif (
            utils.resource_strings["planet_str"]
            + " "
            + utils.resource_strings["drekkanam_str"].replace(" (D3)", "")
            in key
        ):
            from jhora.ui.options_dialog import WidgetDialog
            ...
            _title = key
            if (
                self._graha_drekkana_dialog is None
                and self._graha_drekkana_widgets is not None
            ):
                self._graha_drekkana_dialog = WidgetDialog(
                    title=_title,
                    h_widgets=self._graha_drekkana_widgets,
                    fit_to_widget_contents=True,
                )
            self._graha_drekkana_dialog.exec()
        elif (
            "22nd " + utils.resource_strings["drekkanam_str"].replace(" (D3)", "")
            in key
        ):
            from jhora.ui.options_dialog import WidgetDialog
            ...
            if self._ndl_22_dialog is None and self._ndl_22_widgets is not None:
                self._ndl_22_dialog = WidgetDialog(
                    title=_title,
                    h_widgets=self._ndl_22_widgets,
                    fit_to_widget_contents=True,
                )
            self._ndl_22_dialog.exec()
        elif (
            "64th " + utils.resource_strings["navamsam_str"].replace(" (D9)", "") in key
        ):
            from jhora.ui.options_dialog import WidgetDialog

            _title = key
            if self._ndl_64_dialog is None and self._ndl_64_widgets is not None:
                self._ndl_64_dialog = WidgetDialog(
                    title=_title,
                    h_widgets=self._ndl_64_widgets,
                    fit_to_widget_contents=True,
                )
            ...
            if self._aspect_dialog is None and self._aspect_widgets is not None:
                self._aspect_dialog = WidgetDialog(
                    title=_title,
                    h_widgets=self._aspect_widgets,
                    fit_to_widget_contents=True,
                )
            ...
            _title = key
            if (
                self._planet_info_dialog is None
                and self._planet_info_widgets is not None
            ):
                self._planet_info_dialog = WidgetDialog(
                    title=_title, h_widgets=self._planet_info_widgets
                )
        ...
        self.setData(
            data=self.data,
            arudha_lagna_data=alt_data_2d,
            chart_title=self._chart_title,
            chart_title_font_size=self._chart_title_font_size,
        )
        self.update()
    ...
    def setData(
        ...
        if menu_dict != None:
            ...
            self._ndl_22_widgets = ndl_22_widgets
            self._ndl_64_widgets = ndl_64_widgets
            self._graha_drekkana_widgets = graha_drekkana_widgets
            self._nava_thaara_widgets = nava_thaara_widgets
            self._spl_thaara_widgets = spl_thaara_widgets
        ...
        if (
            self._nava_thaara_dialog is not None
            and self._nava_thaara_widgets is not None
        ):
            self._nava_thaara_dialog = WidgetDialog(
                title=_title,
        ...
        if (
            self._graha_drekkana_dialog is not None
            and self._graha_drekkana_widgets is not None
        ):
            self._graha_drekkana_dialog = WidgetDialog(
                title=_title,
        ...
        if (
            self._planet_info_dialog is not None
            and self._planet_info_widgets is not None
        ):
            self._planet_info_dialog = WidgetDialog(
                title=_title, h_widgets=self._planet_info_widgets
        ...
        for row in range(row_count):
            for col in range(col_count):
                ...
                # write chart title in center of the chart
                    ...
                    font = QFont()
                    painter.setFont(font)
                    rect_image = QtCore.QRect(
                        round(cell_x + cell_width / 2),
                        round(cell_y),
                        round(cell_width / 2),
                        round(cell_height / 2),
                    )
        ...
        if (
            ...
            if _replace_lagna:
                ...
                (
                    ...
                    self._data_original, alt_data, "south indian"
                )
            else:
                # replace 'lagna(plag) with lagna
                lagna = utils.resource_strings["ascendant_str"]
    ...
    def set_south_indian_chart_data(self):  # , data,chart_title=''):
        ...
        for row in range(row_count):
            for col in range(col_count):
                ...
                if row == 0 or row == row_count - 1 or col == 0 or col == col_count - 1:
                    if self.arudha_lagna_data:
                        ...
                        painter.drawText(
                            ...
                            self.arudha_lagna_data[row][col].strip(),
                        )
                    painter.setPen(QPen())
                    painter.drawRect(cell_rect)
                    painter.setPen(QColor(_planet_color))
                    painter.drawText(
                        cell_rect, Qt.AlignmentFlag.AlignCenter, cell_text.strip()
                    )
                    painter.setPen(QColor(_rasi_color))
                    # Write zodiac symbol
                    painter.drawText(
                ...
                if (
                    row == (row_count / 2) - 1
                    and col == (col_count / 2) - 1
                    and chart_title
                ):
                    cell_x = round(self.x + col * cell_width)
                    cell_y = round(self.y + (row) * cell_height)
                    cell_rect = QtCore.QRect(
                        cell_x, cell_y, 2 * cell_width, cell_height
                    )
                    if self._chart_title_font_size != None:
                        font = QFont()
                        font.setPixelSize(self._chart_title_font_size)
...
class NorthIndianChart(QWidget):
    ...
    def __init__(
        ...
        data=None,
        chart_house_size: tuple = (
            _north_chart_house_x,
            _north_chart_house_y,
            _north_chart_house_width,
            _north_chart_house_height,
        ),
        ...
        if (
            ...
            if _replace_lagna:
                ...
                (
                    ...
                    self._data_original, alt_data, "north indian"
                )
            else:
                # replace 'lagna(plag) with lagna
                lagna = utils.resource_strings["ascendant_str"]
    ...
    def setData(
        ...
        self.data = data
        self.arudha_lagna_data = arudha_lagna_data
        self._chart_title_font_size = (
            NorthIndianChart._north_chart_title_font_size
            if chart_title_font_size == None
            else chart_title_font_size
        )
        self._chart_title = chart_title
    ...
    def _draw_north_indian_chart(self):  # ,event):
        ...
        icon_width = int(
            NorthIndianChart._north_chart_icon_width * self._chart_size_factor
        )
        icon_height = int(
            NorthIndianChart._north_chart_icon_height * self._chart_size_factor
        )
        ...
        for l, pos in enumerate(self.label_positions):
            ...
            zodiac_cell_rect = QtCore.QRect(
                zodiac_label_x, zodiac_label_y, cell_width, cell_height
            )
            painter.setPen(QColor(_rasi_color))
            ...
            if self.arudha_lagna_data:
                ...
                arudha_cell_rect = QtCore.QRect(
                    arudha_label_x, arudha_label_y, cell_width, cell_height
                )
                painter.setPen(QColor(_arudha_color))
        ...
        for z, pls in chart_1d:
            ...
            for p in tmp:
                if p == const._ascendant_symbol:
                    ...
                    pl_str += (
                        house.planet_list[int(p)] + ret_str + "/"
                    )  # const._planet_symbols[int(p)]+'/'
            pl_str = pl_str[:-1]
            res.append((z, pl_str))
        result.append(res)
    return result
...
if __name__ == "__main__":
    ...
    _resources = utils.resource_strings
    south_data = [
        ["சனி♄\nகேது☋", "", "", ""],
        ["", "", "", ""],
        ["லக்னம்ℒ\nஅருணா⛢\nவருணா♆", "", "", "செவ்வாய்♂"],
    ...
    print("south data", south_data)
    print("prasna data", prasna_data)
    south_data, prasna_data = replace_ascendant_with_prasna_lagna(
        south_data, prasna_data, chart_type
    )
    print("south data", south_data)
    print("prasna data", prasna_data)
    # """
    ...
    dcf = 1
    arudha_base = 1
    special_menu_list = [_resources["ascendant_short_str"]] + utils.PLANET_SHORT_NAMES[
        : const._planets_upto_ketu
    ]
    ...
    for ab, (key, _) in enumerate(special_menu_dict.items()):
        ...
        for p, r in enumerate(ba):
            ba_chart[r] += (
                _resources["bhava_arudha_a" + str(p + 1) + "_short_str"] + "\n"
                if ab == 0
                else astr + str(p + 1) + "\n"
            )
    ...
    for p, r in enumerate(ga):
        ga_chart[r] += (
            _resources["ascendant_str"] + "\n"
            if p == 0
            else utils.PLANET_NAMES[p - 1] + "\n"
        )
        ...
        for chart_1d in chart_1d_list[:-1]:
            ...
            for z, pls in chart_1d:
                ...
                for p in tmp:
                    if p == const._ascendant_symbol:
                        pl_str += (
                            _resources["ascendant_short_str"] + "/"
    ...
    def _set_chart_data(Chart, chart_type, _chart_title):
        ...
        if "north" in chart_type.lower():
            ...
            Chart.setData(
                ...
                chart_title_font_size=8,
                menu_dict=special_menu_dict,
            )
        elif "east" in chart_type.lower():
            chart_data_2d = utils._convert_1d_house_data_to_2d(
                chart_data_1d, chart_type
            )
            row, col = _get_row_col_string_match_from_2d_list(
                chart_data_2d, _resources["ascendant_str"]
            )
            ...
            Chart.setData(
                _western_data, chart_title=_chart_title, chart_title_font_size=8
            )
            Chart.update()
            ...
            chart_1d = sudharsana_chakra.sudharshana_chakra_chart(
                jd, place, dob, years_from_dob=0, divisional_chart_factor=dcf
            )
            # print('chart_1d',chart_1d)
